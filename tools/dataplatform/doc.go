...
func main() {
	dir := os.Getenv("DOCS_OUT_DATAPLATFORM")
	if dir == "" {
		fmt.Printf("DOCS_OUT_DATAPLATFORM environment variable not set.\n")
		os.Exit(1)
	}
	...
	for _, cmd := range commands.GetRootCmd().SubCommands() {
		// Find dataplatform command
		if cmd.Command != nil && cmd.Command.Parent() != nil && cmd.Name() == "dataplatform" {
			for _, subCmd := range cmd.SubCommands() {
				// Find dataplatform command
				if subCmd.Name() == "dataplatform" {
					err := WriteDocs(subCmd, dir)
					if err != nil {
						fmt.Printf("Error: %v\n", err)
						os.Exit(1)
					}
