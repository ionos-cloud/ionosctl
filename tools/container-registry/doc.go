...
func main() {
	dir := os.Getenv("DOCS_OUT_CONTAINER_REGISTRY")
	if dir == "" {
		fmt.Printf("DOCS_OUT_CONTAINER_REGISTRY environment variable not set.\n")
		os.Exit(1)
	}
	...
	for _, cmd := range commands.GetRootCmd().SubCommands() {
		// Find container-registry command
		if cmd.Command != nil && cmd.Command.Parent() != nil && cmd.Name() == "container-registry" {
