...
func ipblock() *core.Command {
	...
	globalFlags.StringSliceP(config.ArgCols, "", defaultIpBlockCols, utils.ColsMessage(defaultIpBlockCols))
	...
	*/
	...
	get.AddStringFlag(config.ArgIpBlockId, config.ArgIdShort, "", config.IpBlockId, core.RequiredFlagOption())
...
* IpBlock Id`,
	...
	update.AddStringFlag(config.ArgIpBlockId, config.ArgIdShort, "", config.IpBlockId, core.RequiredFlagOption())
...
* IpBlock Id`,
	...
	deleteCmd.AddStringFlag(config.ArgIpBlockId, config.ArgIdShort, "", config.IpBlockId, core.RequiredFlagOption())
...
func getIpBlockPrint(resp *v6.Response, c *core.CommandConfig, ipBlocks []v6.IpBlock) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
			r.ApiResponse = resp
...
func getIpBlocks(ipBlocks v6.IpBlocks) []v6.IpBlock {
	ss := make([]v6.IpBlock, 0)
	if items, ok := ipBlocks.GetItemsOk(); ok && items != nil {
		for _, item := range *items {
			ss = append(ss, v6.IpBlock{IpBlock: item})
		}
	}
	return ss
}

func getIpBlock(ipBlock *v6.IpBlock) []v6.IpBlock {
	ss := make([]v6.IpBlock, 0)
	if ipBlock != nil {
		ss = append(ss, v6.IpBlock{IpBlock: ipBlock.IpBlock})
	}
	return ss
}

func getIpBlocksKVMaps(ss []v6.IpBlock) []map[string]interface{} {
...
func getIpBlockKVMap(s v6.IpBlock) map[string]interface{} {
...
func getIpBlocksIds(outErr io.Writer) []string {
	...
	clierror.CheckError(err, outErr)
	ipBlockSvc := v6.NewIpBlockService(clientSvc.Get(), context.TODO())
