...
func TestPreRunClusterCreate(t *testing.T) {
	...
	core.PreCmdConfigTest(t, w, func(cfg *core.PreCommandConfig) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		viper.Set(config.ArgQuiet, false)
		viper.Set(core.GetFlagName(cfg.NS, dbaaspg.ArgPostgresVersion), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, dbaaspg.ArgVdcId), testClusterVar)
...
func TestRunClusterUpdate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgServerUrl, config.DefaultApiURL)
		viper.Set(core.GetFlagName(cfg.NS, dbaaspg.ArgClusterId), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, dbaaspg.ArgPostgresVersion), testClusterNewVar)
