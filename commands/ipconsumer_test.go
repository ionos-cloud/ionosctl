...
func TestGetIpConsumersCols(t *testing.T) {
	...
	viper.Set(config.ArgOutput, config.DefaultOutputFormat)
	viper.Set(config.ArgQuiet, false)
	viper.Set(core.GetGlobalFlagName("consumer", config.ArgFormat), []string{"Ip"})
	getIpConsumerCols(core.GetGlobalFlagName("consumer", config.ArgFormat), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetIpConsumerColsErr(t *testing.T) {
	...
	viper.Set(config.ArgOutput, config.DefaultOutputFormat)
	viper.Set(config.ArgQuiet, false)
	viper.Set(core.GetGlobalFlagName("consumer", config.ArgFormat), []string{"Unknown"})
	getIpConsumerCols(core.GetGlobalFlagName("consumer", config.ArgFormat), w)
	err := w.Flush()
	assert.NoError(t, err)
