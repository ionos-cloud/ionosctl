...
func LogsListCmd() *core.Command {
	cmd := core.NewCommand(
		context.TODO(), nil, core.CommandBuilder{
			...
			CmdRun: func(c *core.CommandConfig) error {
				...
				logs, _, err := c.DbaasMongoServices.Clusters().LogsList(
					clusterId, directionPtr, limitPtr, startPtr, endPtr,
				)
				if err != nil {
					return err
				}
	...
	cmd.AddDurationFlag(
		flagStartDuration, "", 0*time.Second,
		"The start time, as a duration. This should be negative, i.e. -720h. Valid: h, m, s",
	)
	cmd.AddStringFlag(
		flagStart, "", "",
		"The start time for the query in RFC3339 format. Must not be greater than 30 days ago and less than the end parameter. The default value is 30 days ago.",
	)
	cmd.AddDurationFlag(
		flagEndDuration, "", 0*time.Second,
		"The end time, as a duration. This should be negative and greater than the start time, i.e. -24h. Valid: h, m, s",
	)
	cmd.AddStringFlag(
		flagEnd, "", "",
		"The end time for the query in RFC3339 format. Must not be greater than the start parameter. The default value is the current timestamp.",
	)
	cmd.AddSetFlag(
		flagDirection, "", "", []string{"BACKWARD", "FORWARD"},
		"The direction in which to scan through the logs. The logs are returned in order of the direction",
	)
	cmd.AddIntFlag(
		flagLimit, "", 100,
		"The maximal number of log lines to return. If the limit is reached then log lines will be cut at the end (respecting the scan direction). Must be between 1 - 5000",
	)
