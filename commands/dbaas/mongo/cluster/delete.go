...
ionosctl db m c d --all --name <name>`,
			...
			CmdRun: func(c *core.CommandConfig) error {
				...
				chosenCluster, _, err := client.Must().MongoClient.ClustersApi.ClustersFindById(
					context.Background(), clusterId,
				).Execute()
				if err != nil {
					wrapped := fmt.Errorf("failed trying to find cluster by id: %w", err)
					keepGoing := confirm.FAsk(
						c.Command.Command.InOrStdin(),
						fmt.Sprintf("%s, try deleting %s anyways", wrapped.Error(), clusterId),
					)
					if !keepGoing {
						return wrapped
					}
				...
				ok := confirm.FAsk(
					c.Command.Command.InOrStdin(), confirmStringForCluster(chosenCluster),
				...
				fmt.Fprintf(
					...
					jsontabwriter.GenerateVerboseOutput("Deleting cluster: %s", clusterId),
				)

				_, _, err = client.Must().MongoClient.ClustersApi.ClustersDelete(
					context.Background(), clusterId,
				).Execute()
				return err
			},
...
func deleteAll(c *core.CommandConfig) error {
	...
	return functional.ApplyAndAggregateErrors(
		*xs.GetItems(), func(x sdkgo.ClusterResponse) error {
			yes := confirm.FAsk(
				c.Command.Command.InOrStdin(), confirmStringForCluster(x),
				viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
			)
			if yes {
