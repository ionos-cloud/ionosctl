...
func ClusterCreateCmd() *core.Command {
		...
		context.TODO(), nil, core.CommandBuilder{
			...
			CmdRun: func(c *core.CommandConfig) error {
				...
				if *input.Properties.Location == "" {
					...
					dc, _, err := client.CloudClient.DataCentersApi.DatacentersFindById(
						c.Context, *createConn.DatacenterId,
					).Execute()
					if err != nil {
						return err
					}
	...
	cmd.AddStringVarFlag(
		createProperties.DisplayName, constants.FlagName, constants.FlagNameShort, "", "The name of your cluster",
	)
	cmd.AddStringVarFlag(
		createProperties.Location, constants.FlagLocation, constants.FlagLocationShort, "",
		"The physical location where the cluster will be created. Defaults to the connection's datacenter location",
	)
	_ = cmd.Command.RegisterFlagCompletionFunc(
		constants.FlagLocation,
		func(cmdCobra *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return cloudapiv6completer.LocationIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
		},
	)
	cmd.AddStringVarFlag(
		createProperties.TemplateID, constants.FlagTemplateId, "", "",
	...
	cmd.AddInt32VarFlag(
		createProperties.Instances, constants.FlagInstances, "", 0,
		"The total number of instances in the cluster (one primary and n-1 secondaries)",
	)
	cmd.AddStringVarFlag(
		createProperties.MongoDBVersion, constants.FlagVersion, "", "5.0", "The MongoDB version of your cluster",
	)
	...
	_ = cmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return []string{
				"00:00:00", "08:00:00", "10:00:00", "12:00:00", "16:00:00",
	...
	cmd.AddIntFlag(
		...
		"Timeout option for Request [seconds]",
	)

	// Connections
	_ = allocate.Zero(&createConn)
	cmd.AddStringVarFlag(
		createConn.DatacenterId, constants.FlagDatacenterId, "", "",
	...
	cmd.AddStringVarFlag(
		createConn.LanId, constants.FlagLanId, "", "", "The numeric LAN ID with which you connect your cluster",
		core.RequiredFlagOption(),
	)
	_ = cmd.Command.RegisterFlagCompletionFunc(
		constants.FlagLanId,
	...
	cmd.AddStringSliceVarFlag(
		createConn.CidrList, constants.FlagCidr, "", nil,
		"The list of IPs and subnet for your cluster. All IPs must be in a /24 network. Note the following unavailable IP range: 10.233.114.0/24",
		core.RequiredFlagOption(),
	)
