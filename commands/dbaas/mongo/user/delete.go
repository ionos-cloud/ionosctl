		...
		context.TODO(), nil, core.CommandBuilder{
			...
			PreCmdRun: func(c *core.PreCommandConfig) error {
				return core.CheckRequiredFlagsSets(
					c.Command, c.NS, []string{constants.FlagClusterId, constants.ArgAll},
					[]string{constants.FlagClusterId, constants.FlagName},
				)
			},
			CmdRun: func(c *core.CommandConfig) error {
				...
				yes := confirm.FAsk(
					c.Command.Command.InOrStdin(), fmt.Sprintf("delete user %s", user),
				...
				out, err := jsontabwriter.GenerateOutputPreconverted(
					u, uConverted, tabheaders.GetHeadersAllDefault(allCols, cols),
				)
				if err != nil {
					return err
				}
...
func deleteAll(c *core.CommandConfig, clusterId string) error {
	...
	return functional.ApplyAndAggregateErrors(
		*xs.GetItems(), func(x sdkgo.User) error {
			yes := confirm.FAsk(
				c.Command.Command.InOrStdin(), fmt.Sprintf("delete user %s", *x.Properties.Username),
				viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
			)
			if !yes {
				return fmt.Errorf("user %s skipped by confirmation check", *x.Properties.Username)
			}
			_, _, delErr := client.Must().MongoClient.UsersApi.ClustersUsersDelete(
				c.Context, clusterId, *x.Properties.Username,
			).Execute()
			if delErr != nil {
