	...
	_ = create.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return cloudapiv6completer.LansIds(
				viper.GetString(
					core.GetFlagName(
						create.NS, dbaaspg.ArgDatacenterId,
					),
				),
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return cloudapiv6completer.LansIds(
				viper.GetString(
					core.GetFlagName(
						update.NS, dbaaspg.ArgDatacenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
...
* Backup Id`,
	...
	restoreCmd.AddBoolFlag(constants.ArgForce, constants.ArgForceShort, false, constants.DescForce)
	...
	_ = restoreCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.BackupsIdsForCluster(
				viper.GetString(
					core.GetFlagName(
						restoreCmd.NS, constants.FlagClusterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgName)) {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Filtering after Cluster Name: %v", viper.GetString(core.GetFlagName(c.NS, dbaaspg.ArgName)),
			),
		)
	}
...
func RunClusterGet(c *core.CommandConfig) error {
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			constants.ClusterId, viper.GetString(core.GetFlagName(c.NS, constants.FlagClusterId)),
		),
	)
...
func RunClusterUpdate(c *core.CommandConfig) error {
	...
	if viper.GetBool(core.GetFlagName(c.NS, constants.ArgWaitForState)) {
		fmt.Fprintf(
			...
			jsontabwriter.GenerateVerboseOutput("Wait 10 seconds before checking state..."),
		)
...
func RunClusterRestore(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), fmt.Sprintf("restore cluster with id: %v from backup: %v", clusterId, backupId),
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Restoring Cluster from Backup..."),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("PostgreSQL Cluster successfully restored"),
	)
	return nil
}
...
func RunClusterDelete(c *core.CommandConfig) error {
	if viper.GetBool(core.GetFlagName(c.NS, constants.ArgAll)) {
		...
		fmt.Fprintf(
			...
			jsontabwriter.GenerateLogOutput("PostgreSQL Clusters successfully deleted"),
		)
		return nil
	}
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), fmt.Sprintf("delete cluster with id: %v", clusterId),
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("PostgreSQL Cluster successfully deleted"),
	)
	return nil
}
...
func ClusterDeleteAll(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete ALL clusters", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	for _, cluster := range *dataOk {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateLogOutput(constants.MessageDeletingAll, c.Resource, *idOk),
		)
...
func getCreateClusterRequest(c *core.CommandConfig) (*resources.CreateClusterRequest, error) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("PostgresVersion: %v", pgsqlVersion),
	)
	input.SetPostgresVersion(pgsqlVersion)

	syncMode := strings.ToUpper(viper.GetString(core.GetFlagName(c.NS, dbaaspg.ArgSyncMode)))
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("SynchronizationMode: %v", syncMode),
	)
	...
	storageSize, err := utils2.ConvertSize(
		viper.GetString(core.GetFlagName(c.NS, dbaaspg.ArgStorageSize)), utils2.MegaBytes,
	)
	if err != nil {
		return nil, err
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("StorageSize: %v[MB]", int32(storageSize)),
	)
	...
	if viper.GetString(core.GetFlagName(c.NS, dbaaspg.ArgBackupLocation)) != "" {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Backup Location: %v", viper.GetString(core.GetFlagName(c.NS, dbaaspg.ArgBackupLocation)),
			),
		)
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgLocation)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Getting Location from VDC..."),
		)
		...
		if properties, ok := vdc.GetPropertiesOk(); ok && properties != nil {
			if location, ok := properties.GetLocationOk(); ok && location != nil {
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Location: %v", *location),
				)
				input.SetLocation(*location)
			}
		}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Connection - DatacenterId: %v", vdcId),
	)
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgMaintenanceTime)) ||
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgMaintenanceTime)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("MaintenanceWindow - Time: %v", maintenanceTime),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgMaintenanceDay)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("MaintenanceWindow - DayOfTheWeek: %v", maintenanceDay),
			)
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgBackupId)) ||
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgRecoveryTime)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
					"From Backup - RecoveryTargetTime [RFC3339 format]: %v", recoveryTargetTime,
				),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgBackupId)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("From Backup - BackupId: %v", backupId),
			)
...
func getPatchClusterRequest(c *core.CommandConfig) (*resources.PatchClusterRequest, error) {
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgStorageSize)) {
		...
		input.SetStorageSize(int32(storageSize))
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("StorageSize: %vMB", storageSize),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgName)) {
		displayName := viper.GetString(core.GetFlagName(c.NS, dbaaspg.ArgName))
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("DisplayName: %v", displayName),
		)
		input.SetDisplayName(displayName)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgMaintenanceTime)) || viper.IsSet(
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgMaintenanceTime)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("MaintenanceTime: %v", maintenanceTime),
			)
			maintenanceWindow.SetTime(maintenanceTime)
		}
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgMaintenanceDay)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("MaintenanceDayOfWeek: %v", maintenanceDay),
			)
			maintenanceWindow.SetDayOfTheWeek(sdkgo.DayOfTheWeek(maintenanceDay))
		}
	...
	if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgDatacenterId)) || viper.IsSet(
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(getConnectionMessage(connection)),
		)
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgDatacenterId)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Updated Datacenter Id: %v", lanId),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgLanId)) {
			lanId := viper.GetString(core.GetFlagName(c.NS, dbaaspg.ArgLanId))
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Updated Lan Id: %v", lanId),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, dbaaspg.ArgCidr)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Updated Cidr: %v", cidrId),
			)
...
func getConnectionFromCluster(c *core.CommandConfig, clusterId string) (sdkgo.Connection, error) {
	if c != nil {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Getting properties from cluster with Id: %v", clusterId),
		)
