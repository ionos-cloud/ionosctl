...
func ClusterDeleteCmd() *core.Command {
	...
	deleteCmd := core.NewCommand(ctx, nil, core.CommandBuilder{
		...
		Verb:      "delete",
		Aliases:   []string{"d"},
		ShortDesc: "Delete an In-Memory DB Cluster",
		LongDesc: `Use this command to delete a specified In-Memory DB Cluster from your account.

Required values to run command:

* Cluster Id`,
		Example:    "ionosctl dbaas in-memory-db-v2 cluster delete --cluster-id <cluster-id>",
...
func RunClusterDelete(c *core.CommandConfig) error {
	...
	_, err := client.Must().InMemoryDBClientV2.ClustersApi.ClustersDelete(context.Background(), clusterId).Execute()
	if err != nil {
		return err
	}
	return nil
...
func ClusterDeleteAll(c *core.CommandConfig) error {
	...
	type located struct {
		cluster inmemorydb.ClusterRead
		loc     string
