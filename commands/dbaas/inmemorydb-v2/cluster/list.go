...
func ClusterListCmd() *core.Command {
	...
	listEnv := core.NewCommand(ctx, nil, core.CommandBuilder{
		...
		Verb:       "list",
		Aliases:    []string{"ls"},
		ShortDesc:  "List In-Memory DB Clusters",
		LongDesc:   "Use this command to retrieve a list of In-Memory DB Clusters provisioned under your account. You can filter the result based on Cluster Name using `--name` option.",
		Example:    "ionosctl dbaas in-memory-db-v2 cluster list",
		PreCmdRun:  core.NoPreRun,
		CmdRun:     RunClusterList,
		InitClient: true,
	})
	listEnv.AddStringFlag(constants.FlagName, constants.FlagNameShort, "", "Response filter to list only the In-Memory DB Clusters that contain the specified name in the DisplayName field. The value is case insensitive")
...
func RunClusterList(c *core.CommandConfig) error {
	return c.ListAllLocations(clusterCols, func(cfg *shared.Configuration) (any, error) {
		...
		if fn := core.GetFlagName(c.NS, constants.FlagState); viper.IsSet(fn) {
			req = req.FilterState(inmemorydb.ClusterState(viper.GetString(fn)))
		}
