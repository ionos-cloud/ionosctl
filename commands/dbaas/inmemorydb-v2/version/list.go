...
func VersionListCmd() *core.Command {
	...
	cmd := core.NewCommand(ctx, nil, core.CommandBuilder{
		...
		Verb:       "list",
		Aliases:    []string{"ls"},
		ShortDesc:  "List In-Memory DB Versions",
		LongDesc:   "Use this command to retrieve a list of available In-Memory DB Versions.",
		Example:    "ionosctl dbaas in-memory-db-v2 version list",
...
func RunVersionList(c *core.CommandConfig) error {
	req := client.Must().InMemoryDBClientV2.VersionsApi.VersionsGet(context.Background())
