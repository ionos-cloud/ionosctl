...
func TokenCmd() *core.Command {
	...
	*/
	...
	list.AddIntFlag(
		authv1.ArgContractNo, "", 0,
		"Users with multiple contracts must provide the contract number, for which the tokens are listed",
	)
	...
	*/
	...
	get.AddIntFlag(
		authv1.ArgContractNo, "", 0,
		"Users with multiple contracts must provide the contract number, for which the token information is displayed",
	)
...
* Token Id/CURRENT/EXPIRED/ALL`,
	...
	deleteCmd.AddBoolFlag(
		authv1.ArgCurrent, authv1.ArgCurrentShort, false,
	...
	deleteCmd.AddBoolFlag(
		authv1.ArgExpired, authv1.ArgExpiredShort, false, "Delete the Tokens that are currently expired",
		core.RequiredFlagOption(),
	)
	deleteCmd.AddBoolFlag(
		authv1.ArgAll, authv1.ArgAllShort, false, "Delete the Tokens under your account", core.RequiredFlagOption(),
	)
...
func PreRunTokenDelete(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlagsSets(
		c.Command, c.NS, []string{authv1.ArgTokenId}, []string{authv1.ArgCurrent}, []string{authv1.ArgExpired},
		[]string{authv1.ArgAll},
	)
}
...
func RunTokenGet(c *core.CommandConfig) error {
	...
	token, _, err := c.AuthV1Services.Tokens().Get(
		viper.GetString(core.GetFlagName(c.NS, authv1.ArgTokenId)),
		viper.GetInt32(core.GetFlagName(c.NS, authv1.ArgContractNo)),
	)
	if err != nil {
		return err
	}
...
func RunTokenDelete(c *core.CommandConfig) error {
	...
	if viper.IsSet(core.GetFlagName(c.NS, authv1.ArgCurrent)) && viper.GetBool(
		core.GetFlagName(
			c.NS, authv1.ArgCurrent,
		),
	) {
		return RunTokenDeleteCurrent(c)
	}
	if viper.IsSet(core.GetFlagName(c.NS, authv1.ArgExpired)) && viper.GetBool(
		core.GetFlagName(
			c.NS, authv1.ArgExpired,
		),
	) {
...
func RunTokenDeleteAll(c *core.CommandConfig) error {
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByCriteria(
		"ALL", viper.GetInt32(core.GetFlagName(c.NS, authv1.ArgContractNo)),
	)
	if err != nil {
		return err
	}
...
func RunTokenDeleteExpired(c *core.CommandConfig) error {
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByCriteria(
		"EXPIRED", viper.GetInt32(core.GetFlagName(c.NS, authv1.ArgContractNo)),
	)
	if err != nil {
		return err
	}
...
func RunTokenDeleteCurrent(c *core.CommandConfig) error {
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByCriteria(
		"CURRENT", viper.GetInt32(core.GetFlagName(c.NS, authv1.ArgContractNo)),
	)
	if err != nil {
		return err
	}
...
func RunTokenDeleteById(c *core.CommandConfig) error {
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByID(
		tokenId, viper.GetInt32(core.GetFlagName(c.NS, authv1.ArgContractNo)),
	)
	if err != nil {
		return err
	}
	...
	if c != nil {
		if dcs != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				allTokenCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
