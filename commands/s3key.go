...
func getS3KeyPrint(resp *v5.Response, c *core.CommandConfig, s []v5.S3Key) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
			r.ApiResponse = resp
...
func getS3Keys(S3Keys v5.S3Keys) []v5.S3Key {
	ss := make([]v5.S3Key, 0)
	if items, ok := S3Keys.GetItemsOk(); ok && items != nil {
		for _, s := range *items {
			ss = append(ss, v5.S3Key{S3Key: s})
		}
	}
	return ss
}

func getS3Key(s *v5.S3Key) []v5.S3Key {
	ss := make([]v5.S3Key, 0)
	if s != nil {
		ss = append(ss, v5.S3Key{S3Key: s.S3Key})
	}
	return ss
}

func getS3KeysKVMaps(ss []v5.S3Key) []map[string]interface{} {
...
func getS3KeyKVMap(s v5.S3Key) map[string]interface{} {
