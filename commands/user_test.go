...
var (
	...
	userProperties = v6.UserProperties{
...
func TestRunUserList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		rm.User.EXPECT().List().Return(users, &testResponse, nil)
		err := RunUserList(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunUserGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgUserId), testUserVar)
		rm.User.EXPECT().Get(testUserVar).Return(&userTestGet, &testResponse, nil)
		err := RunUserGet(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunUserUpdate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		rm.User.EXPECT().Update(testUserVar, userNewPut).Return(&userNew, &testResponse, nil)
...
var (
	groupUsersTest = v6.GroupMembers{
		GroupMembers: ionoscloud.GroupMembers{
			Items: &[]ionoscloud.User{userTestGet.User},
		},
	}
	groupUserTest = v6.User{
		User: ionoscloud.User{
			Id: &testUserVar,
		},
	}
...
func TestRunGroupUserList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		rm.Group.EXPECT().ListUsers(testGroupVar).Return(groupUsersTest, &testResponse, nil)
...
func TestRunGroupUserRemove(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgGroupId), testGroupVar)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgUserId), testUserVar)
		rm.Group.EXPECT().RemoveUser(testGroupVar, testUserVar).Return(&testResponse, nil)
