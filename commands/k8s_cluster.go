...
func k8sCluster() *core.Command {
	...
	*/
	...
	get.AddStringFlag(config.ArgK8sClusterId, config.ArgIdShort, "", config.K8sClusterId, core.RequiredFlagOption())
...
* K8s Cluster Id`,
	...
	update.AddStringFlag(config.ArgS3Bucket, "", "", "S3 Bucket name configured for K8s usage. It will overwrite the previous value")
	...
	update.AddStringFlag(config.ArgK8sClusterId, config.ArgIdShort, "", config.K8sClusterId, core.RequiredFlagOption())
...
* K8s Cluster Id`,
	...
	deleteCmd.AddStringFlag(config.ArgK8sClusterId, config.ArgIdShort, "", config.K8sClusterId, core.RequiredFlagOption())
...
func getNewK8sCluster(c *core.CommandConfig) (*v6.K8sClusterForPost, error) {
	var (
		k8sversion string
		err        error
	)
	proper := v6.K8sClusterPropertiesForPost{}
...
func getK8sClusterInfo(oldUser *v6.K8sCluster, c *core.CommandConfig) v6.K8sClusterForPut {
	propertiesUpdated := v6.K8sClusterPropertiesForPut{}
...
func getK8sClusterPrint(resp *v6.Response, c *core.CommandConfig, k8ss []v6.K8sCluster) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
			r.ApiResponse = resp
...
func getK8sClusters(k8ss v6.K8sClusters) []v6.K8sCluster {
	u := make([]v6.K8sCluster, 0)
	if items, ok := k8ss.GetItemsOk(); ok && items != nil {
		for _, item := range *items {
			u = append(u, v6.K8sCluster{KubernetesCluster: item})
		}
	}
	return u
}

func getK8sCluster(u *v6.K8sCluster) []v6.K8sCluster {
	k8ss := make([]v6.K8sCluster, 0)
	if u != nil {
		k8ss = append(k8ss, v6.K8sCluster{KubernetesCluster: u.KubernetesCluster})
	}
	return k8ss
}

func getK8sClustersKVMaps(us []v6.K8sCluster) []map[string]interface{} {
...
func getMaintenanceInfo(c *core.CommandConfig, maintenance *v6.K8sMaintenanceWindow) v6.K8sMaintenanceWindow {
	var day, time string
	if viper.IsSet(core.GetFlagName(c.NS, config.ArgK8sMaintenanceDay)) {
	...
	return v6.K8sMaintenanceWindow{
