...
func RunLanGet(c *core.CommandConfig) error {
	c.Printer.Infof("Lan with id: %v is getting...", viper.GetString(core.GetFlagName(c.NS, config.ArgLanId)))
	l, _, err := c.Lans().Get(
...
func RunLanCreate(c *core.CommandConfig) error {
	...
	c.Printer.Infof("Properties set for creating the Lan: Name: %v, Public: %v", name, public)
	if viper.IsSet(core.GetFlagName(c.NS, config.ArgPccId)) {
		...
		c.Printer.Infof("Property Pcc set: %v", pcc)
	}
...
func RunLanDelete(c *core.CommandConfig) error {
	...
	c.Printer.Infof("Lan with id: %v is deleting...", viper.GetString(core.GetFlagName(c.NS, config.ArgLanId)))
