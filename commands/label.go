...
func getLabelPrint(c *core.CommandConfig, s []v6.Label) printer.Result {
	r := printer.Result{}
	if c != nil {
		if s != nil {
			r.OutputJSON = s
...
func getLabels(Labels v6.Labels) []v6.Label {
	ss := make([]v6.Label, 0)
	if items, ok := Labels.GetItemsOk(); ok && items != nil {
		for _, s := range *items {
			ss = append(ss, v6.Label{Label: s})
		}
	}
	return ss
}

func getLabel(s *v6.Label) []v6.Label {
	ss := make([]v6.Label, 0)
	if s != nil {
		ss = append(ss, v6.Label{Label: s.Label})
	}
	return ss
}

func getLabelKVMaps(ss []v6.Label) []map[string]interface{} {
...
func getLabelKVMap(s v6.Label) map[string]interface{} {
