...
func TestAuthCmds(t *testing.T) {
	...
	t.Run(
		"login test user interactively, password as flag - valid token saved to config", func(t *testing.T) {
			login := cfg.LoginCmd()
			logout := cfg.LogoutCmd()

			login.Command.Flags().Set(constants.ArgPassword, GoodPassword)
			viper.Set(core.GetFlagName(login.NS, constants.ArgForce), true)
	...
	t.Run(
		"login test user, password as flag - valid token saved to config", func(t *testing.T) {
			login := cfg.LoginCmd()
			logout := cfg.LogoutCmd()
	...
	t.Run(
		"invalid user, password as flag - valid token saved to config - should still error out", func(t *testing.T) {
			login := cfg.LoginCmd()
			logout := cfg.LogoutCmd()
	...
	t.Run(
		"non existant config file should be created", func(t *testing.T) {
			login := cfg.LoginCmd()
			logout := cfg.LogoutCmd()
	...
	t.Run(
		"Pre-rework config file logout - Username and password removed from config file", func(t *testing.T) {
			logout := cfg.LogoutCmd()
	...
	// cfg location tests
	t.Run(
		"cfg location cmd returns valid location", func(t *testing.T) {
	...
	// cfg whoami tests
	t.Run(
		"cfg whoami returns current user - Env Username & Password", func(t *testing.T) {
			cmd := cfg.WhoamiCmd()
