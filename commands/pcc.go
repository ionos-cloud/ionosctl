...
func RunPccCreate(c *core.CommandConfig) error {
	...
	newUser := v6.PrivateCrossConnect{
...
func getPccInfo(oldUser *v6.PrivateCrossConnect, c *core.CommandConfig) *v6.PrivateCrossConnectProperties {
	...
	return &v6.PrivateCrossConnectProperties{
...
func peers() *core.Command {
	...
	*/
	...
	listPeers.AddStringFlag(config.ArgPccId, "", "", config.PccId, core.RequiredFlagOption())
...
func getPccPrint(resp *v6.Response, c *core.CommandConfig, pccs []v6.PrivateCrossConnect) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
			r.ApiResponse = resp
...
func getPccPeerPrint(c *core.CommandConfig, pccs []v6.Peer) printer.Result {
	r := printer.Result{}
	if c != nil {
		if pccs != nil {
			r.OutputJSON = pccs
...
func getPccs(pccs v6.PrivateCrossConnects) []v6.PrivateCrossConnect {
	u := make([]v6.PrivateCrossConnect, 0)
	if items, ok := pccs.GetItemsOk(); ok && items != nil {
		for _, item := range *items {
			u = append(u, v6.PrivateCrossConnect{PrivateCrossConnect: item})
		}
	}
	return u
}

func getPcc(u *v6.PrivateCrossConnect) []v6.PrivateCrossConnect {
	pccs := make([]v6.PrivateCrossConnect, 0)
	if u != nil {
		pccs = append(pccs, v6.PrivateCrossConnect{PrivateCrossConnect: u.PrivateCrossConnect})
	}
	return pccs
}

func getPccsKVMaps(us []v6.PrivateCrossConnect) []map[string]interface{} {
...
func getPccPeersKVMaps(ps []v6.Peer) []map[string]interface{} {
