...
func TestGetIpFailoverCols(t *testing.T) {
	...
	viper.Set(config.ArgOutput, config.DefaultOutputFormat)
	viper.Set(config.ArgQuiet, false)
	viper.Set(core.GetGlobalFlagName("ipfailover", config.ArgFormat), []string{"NicId"})
	getIpFailoverCols(core.GetGlobalFlagName("ipfailover", config.ArgFormat), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetIpFailoverColsErr(t *testing.T) {
	...
	viper.Set(config.ArgOutput, config.DefaultOutputFormat)
	viper.Set(config.ArgQuiet, false)
	viper.Set(core.GetGlobalFlagName("ipfailover", config.ArgFormat), []string{"Unknown"})
	getIpFailoverCols(core.GetGlobalFlagName("ipfailover", config.ArgFormat), w)
	err := w.Flush()
	assert.NoError(t, err)
