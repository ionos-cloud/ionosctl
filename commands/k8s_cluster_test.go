...
func TestRunK8sClusterCreate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgBucketNames), []string{testClusterVar})
		viper.Set(core.GetFlagName(cfg.NS, config.ArgSubnets), []string{testClusterVar})
...
func TestRunK8sClusterUpdate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgBucketNames), []string{testClusterNewVar})
		viper.Set(core.GetFlagName(cfg.NS, config.ArgSubnets), []string{testClusterNewVar})
		rm.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, nil)
