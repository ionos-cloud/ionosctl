...
var (
	defaultFirewallRuleCols = []string{
		"FirewallRuleId", "Name", "Protocol", "PortRangeStart", "PortRangeEnd", "Direction", "IPVersion", "State",
	}
	allFirewallRuleCols = []string{
		"FirewallRuleId", "Name", "Protocol", "SourceMac", "SourceIP", "DestinationIP", "PortRangeStart",
		"PortRangeEnd",
...
func FirewallruleCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultFirewallRuleCols, tabheaders.ColsMessage(allFirewallRuleCols),
	)
	_ = viper.BindPFlag(
		core.GetFlagName(firewallRuleCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols),
	)
	_ = firewallRuleCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, firewallRuleCmd, core.CommandBuilder{
	...
	*/
	...
	get.AddUUIDFlag(
		cloudapiv6.ArgFirewallRuleId, cloudapiv6.ArgIdShort, "", cloudapiv6.FirewallRuleId, core.RequiredFlagOption(),
	)
	_ = get.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgFirewallRuleId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.FirewallRulesIds(
				viper.GetString(core.GetFlagName(get.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(get.NS, cloudapiv6.ArgServerId)),
				viper.GetString(core.GetFlagName(get.NS, cloudapiv6.ArgNicId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
...
* Protocol`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgProtocol, "", "", "The Protocol for Firewall Rule: TCP, UDP, ICMP, ANY", core.RequiredFlagOption(),
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgProtocol,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return []string{"TCP", "UDP", "ICMP", "ANY"}, cobra.ShellCompDirectiveNoFileComp
		},
	)
	create.AddStringFlag(
		cloudapiv6.ArgSourceMac, "", "",
		"Only traffic originating from the respective MAC address is allowed. Valid format: aa:bb:cc:dd:ee:ff. Unset option allows all source MAC addresses",
	)
	create.AddIpFlag(
		cloudapiv6.ArgSourceIp, "", nil,
		"Only traffic originating from the respective IPv4 address is allowed. Not setting option allows all source IPs",
	)
	create.AddIpFlag(
		cloudapiv6.ArgDestinationIp, cloudapiv6.ArgDestinationIpShort, nil,
		"In case the target NIC has multiple IP addresses, only traffic directed to the respective IP address of the NIC is allowed. Not setting option allows all target/destination IPs. WARNING: This short-hand flag `-D` is deprecated.",
	)
	create.AddIntFlag(
		cloudapiv6.ArgIcmpType, "", 0,
		"Define the allowed type (from 0 to 254) if the protocol ICMP is chosen. Not setting option allows all types",
	)
	create.AddIntFlag(
		cloudapiv6.ArgIcmpCode, "", 0,
		"Define the allowed code (from 0 to 254) if protocol ICMP is chosen. Not setting option allows all codes",
	)
	create.AddIntFlag(
		cloudapiv6.ArgPortRangeStart, "", 1,
		"Define the start range of the allowed port (from 1 to 65534) if protocol TCP or UDP is chosen. Not setting portRangeStart and portRangeEnd allows all ports",
	)
	create.AddIntFlag(
		cloudapiv6.ArgPortRangeEnd, "", 1,
		"Define the end range of the allowed port (from 1 to 65534) if the protocol TCP or UDP is chosen. Not setting portRangeStart and portRangeEnd allows all ports",
	)
	create.AddStringFlag(
		cloudapiv6.ArgDirection, cloudapiv6.ArgDirectionShort, "INGRESS", "The type/direction of Firewall Rule",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
	...
	create.AddBoolFlag(
		...
		"Wait for Request for Firewall Rule creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Firewall Rule creation [seconds]",
	)
	...
	create.AddSetFlag(
		cloudapiv6.FlagIPVersion, "", "IPv4", []string{"IPv4", "IPv6"}, "The IP version for the Firewall Rule",
	)
...
* Firewall Rule Id`,
	...
	update.AddStringFlag(
		cloudapiv6.ArgSourceMac, "", "",
		"Only traffic originating from the respective MAC address is allowed. Valid format: aa:bb:cc:dd:ee:ff. Not setting option allows all source MAC addresses",
	)
	update.AddIpFlag(
		cloudapiv6.ArgSourceIp, "", nil,
		"Only traffic originating from the respective IPv4 address is allowed. Not setting option allows all source IPs",
	...
	update.AddIntFlag(
		cloudapiv6.ArgIcmpType, "", 0,
		"Redefine the allowed type (from 0 to 254) if the protocol ICMP is chosen. Not setting option allows all types",
	)
	update.AddIntFlag(
		cloudapiv6.ArgIcmpCode, "", 0,
		"Redefine the allowed code (from 0 to 254) if protocol ICMP is chosen. Not setting option allows all codes",
	)
	update.AddIntFlag(
		cloudapiv6.ArgPortRangeStart, "", 1,
		"Redefine the start range of the allowed port (from 1 to 65534) if protocol TCP or UDP is chosen. Not setting portRangeStart and portRangeEnd allows all ports",
	)
	update.AddIntFlag(
		cloudapiv6.ArgPortRangeEnd, "", 1,
		"Redefine the end range of the allowed port (from 1 to 65534) if the protocol TCP or UDP is chosen. Not setting portRangeStart and portRangeEnd allows all ports",
	)
	update.AddStringFlag(
		cloudapiv6.ArgDirection, cloudapiv6.ArgDirectionShort, "", "The type/direction of Firewall Rule",
	)
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.FirewallRulesIds(
				...
				viper.GetString(core.GetFlagName(update.NS, cloudapiv6.ArgNicId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for Request for Firewall Rule update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Firewall Rule update [seconds]",
	)
	...
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.FirewallRulesIds(
				viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv6.ArgServerId)),
				viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv6.ArgNicId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for Request for Firewall Rule deletion to be executed",
	)
	deleteCmd.AddBoolFlag(cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete all the Firewalls.")
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Firewall Rule deletion [seconds]",
	)
...
func PreRunFirewallRuleList(c *core.PreCommandConfig) error {
	if err := core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgServerId, cloudapiv6.ArgNicId,
	); err != nil {
		return err
	}
...
func PreRunDcServerNicIdsFRuleProtocol(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgServerId, cloudapiv6.ArgNicId, cloudapiv6.ArgProtocol,
	)
}
...
func RunFirewallRuleList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.FirewallRule, firewallRules.FirewallRules,
...
func RunFirewallRuleGet(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.FirewallRule, firewallRule.FirewallRule,
		tabheaders.GetHeaders(allFirewallRuleCols, defaultFirewallRuleCols, cols),
	)
	if err != nil {
		return err
...
func RunFirewallRuleDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete firewall rule", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Firewall Rule with id: %v...", fruleId),
	)
...
func getFirewallRulePropertiesSet(c *core.CommandConfig) resources.FirewallRuleProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgProtocol)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Protocol set: %v", protocol),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgSourceIp)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property SourceIp set: %v", sourceIp),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgSourceMac)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property SourceMac set: %v", sourceMac),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDestinationIp)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property TargetIp/DestinationIp set: %v", targetIp),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgIcmpCode)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property IcmpCode set: %v", icmpCode),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgIcmpType)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property IcmpType set: %v", icmpType),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgPortRangeStart)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property PortRangeStart set: %v", portRangeStart),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgPortRangeEnd)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property PortRangeEnd set: %v", portRangeEnd),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDirection)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property Type/Direction set: %v", firewallruleType),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.FlagIPVersion)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property IP Version set: %v", ipVersion),
		)
	}
...
func DeleteAllFirewallRules(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(constants.DatacenterId, datacenterId),
	)
	...
	firewallRules, resp, err := c.CloudApiV6Services.FirewallRules().List(
		datacenterId, serverId, nicId, cloudapiv6.ParentResourceListQueryParams,
	)
	if err != nil {
		return err
	}
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Firewall Rules",
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Deleting all the Firewall Rules..."),
	)
	...
	for _, firewall := range *firewallRulesItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Firewall Rule with id: %v...", *id),
		)
