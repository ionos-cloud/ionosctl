...
func RunFirewallRuleGet(c *core.CommandConfig) error {
	...
	c.Printer.Verbose(
		"Firewall Rule with id: %v is getting... ",
			...
			for _, firewall := range *firewallRulesItems {
				if id, ok := firewall.GetIdOk(); ok && id != nil {
					...
					resp, err = c.CloudApiV6Services.FirewallRules().Delete(
						datacenterId, serverId, nicId, *id, queryParams,
					)
					if resp != nil && printer.GetId(resp) != "" {
						c.Printer.Verbose(constants.MessageRequestInfo, printer.GetId(resp), resp.RequestTime)
					}
...
var (
	defaultFirewallRuleCols = []string{
		"FirewallRuleId", "Name", "Protocol", "PortRangeStart", "PortRangeEnd", "Direction", "State",
	}
...
func getFirewallRulePrint(
	resp *resources.Response, c *core.CommandConfig, rule []resources.FirewallRule,
) printer.Result {
	...
	if c != nil {
		...
		if rule != nil {
			...
			r.Columns = printer.GetHeaders(
				allFirewallRuleCols, defaultFirewallRuleCols,
