...
func TestRunServerCdromAttach(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().AttachCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
...
func TestRunServerCdromAttachErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().AttachCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testImage, nil, testCdromErr)
			err := RunServerCdromAttach(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunServerCdromsList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgDataCenterId), testCdromVar)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgServerId), testCdromVar)
			rm.CloudApiV6Mocks.Server.EXPECT().ListCdroms(
				testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testCdroms, &testResponse, nil)
...
func TestRunServerCdromsListListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().ListCdroms(
				testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(resources.Cdroms{}, &testResponse, nil)
			err := RunServerCdromsList(cfg)
			assert.NoError(t, err)
		},
	)
...
func TestRunServerCdromsListErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().ListCdroms(
				testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testCdroms, nil, testCdromErr)
			err := RunServerCdromsList(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunServerCdromGet(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgServerId), testCdromVar)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgCdromId), testCdromVar)
			rm.CloudApiV6Mocks.Server.EXPECT().GetCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testImage, &testResponse, nil)
...
func TestRunServerCdromDetach(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgCdromId), testCdromVar)
			viper.Set(core.GetFlagName(cfg.NS, constants.ArgWaitForRequest), false)
			rm.CloudApiV6Mocks.Server.EXPECT().DetachCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
...
func TestRunServerCdromDetachAll(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().ListCdroms(
				testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testCdromsList, &testResponse, nil)
			rm.CloudApiV6Mocks.Server.EXPECT().DetachCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, nil)
			rm.CloudApiV6Mocks.Server.EXPECT().DetachCdrom(
...
func TestRunServerCdromDetachAllListErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().ListCdroms(
				testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testCdromsList, nil, testCdromErr)
			err := RunServerCdromDetach(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunServerCdromDetachAllErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().DetachCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, testCdromErr)
			rm.CloudApiV6Mocks.Server.EXPECT().DetachCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, nil)
			err := RunServerCdromDetach(cfg)
...
func TestRunServerCdromDetachResponseErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Server.EXPECT().DetachCdrom(
				testCdromVar, testCdromVar, testCdromVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponseErr, testCdromErr)
			err := RunServerCdromDetach(cfg)
			assert.Error(t, err)
		},
	)
