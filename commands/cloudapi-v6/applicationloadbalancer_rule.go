...
var (
	defaultAlbForwardingRuleCols = []string{
		"ForwardingRuleId", "Name", "Protocol", "ListenerIp", "ListenerPort", "ServerCertificates", "State",
	}
	allAlbForwardingRuleCols = []string{
		"ForwardingRuleId", "Name", "Protocol", "ListenerIp", "ListenerPort", "ClientTimeout", "ServerCertificates",
		"State",
	}
)
...
func ApplicationLoadBalancerRuleCmd() *core.Command {
	...
	*/
	...
	list.AddStringSliceFlag(
		constants.ArgCols, "", defaultAlbForwardingRuleCols, tabheaders.ColsMessage(defaultAlbForwardingRuleCols),
	)
	_ = list.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
func RunApplicationLoadBalancerForwardingRuleList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.ApplicationLoadBalancerForwardingRule,
...
func RunApplicationLoadBalancerForwardingRuleGet(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.ApplicationLoadBalancerForwardingRule,
...
func RunApplicationLoadBalancerForwardingRuleDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete application load balancer forwarding rule",
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateLogOutput("Application Load Balancer Forwarding Rule successfully deleted"),
	)
...
func DeleteAllApplicationLoadBalancerForwardingRule(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Getting Application Load Balancer Forwarding Rules..."),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Forwarding Rules",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(),
		jsontabwriter.GenerateLogOutput("Application Load Balancer Forwarding Rules successfully deleted"),
	)
	return nil
}
...
func getAlbForwardingRulePropertiesSet(c *core.CommandConfig) *resources.ApplicationLoadBalancerForwardingRuleProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgClientTimeout)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Property Client Timeout set: %v",
				viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv6.ArgClientTimeout)),
			),
		)
	}
