...
func ApplicationLoadBalancerRuleCmd() *core.Command {
	...
	*/
	list := core.NewCommand(
		ctx, albRuleCmd, core.CommandBuilder{
	...
	list.AddStringSliceFlag(
		constants.ArgCols, "", defaultAlbForwardingRuleCols, printer.ColsMessage(defaultAlbForwardingRuleCols),
	)
	_ = list.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	_ = get.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.AlbForwardingRulesIds(
				os.Stderr, viper.GetString(core.GetFlagName(get.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(get.NS, cloudapiv6.ArgApplicationLoadBalancerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	get.AddStringSliceFlag(
		constants.ArgCols, "", defaultAlbForwardingRuleCols, printer.ColsMessage(defaultAlbForwardingRuleCols),
	)
	_ = get.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
...
* Listener Port`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Unnamed Forwarding Rule",
	...
	create.AddIpFlag(
		cloudapiv6.ArgListenerIp, "", nil,
		"Listening (inbound) IP. It must be assigned to the listener NIC of Application Load Balancer.",
		core.RequiredFlagOption(),
	)
	create.AddIntFlag(
		cloudapiv6.ArgListenerPort, "", 8080, "Listening (inbound) port number; valid range is 1 to 65535.",
		core.RequiredFlagOption(),
	)
	create.AddIntFlag(
		cloudapiv6.ArgClientTimeout, "", 50,
...
* Forwarding Rule Id`,
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.AlbForwardingRulesIds(
				os.Stderr, viper.GetString(core.GetFlagName(update.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(update.NS, cloudapiv6.ArgApplicationLoadBalancerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	update.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "", "The name of the Application Load Balancer forwarding rule.",
	)
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, cloudapiv6.DefaultWait,
...
* Forwarding Rule Id`,
	...
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.AlbForwardingRulesIds(
				os.Stderr, viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv6.ArgApplicationLoadBalancerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
...
func PreRunApplicationLoadBalancerForwardingRuleCreate(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgApplicationLoadBalancerId, cloudapiv6.ArgListenerIp,
...
func PreRunDcApplicationLoadBalancerForwardingRuleIds(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgApplicationLoadBalancerId, cloudapiv6.ArgRuleId,
	)
}
...
func RunApplicationLoadBalancerForwardingRuleGet(c *core.CommandConfig) error {
	...
	return c.Printer.Print(
		getAlbForwardingRulePrint(
			nil, c, []resources.ApplicationLoadBalancerForwardingRule{*applicationLoadBalancerForwardingRule},
		),
	)
}
...
func RunApplicationLoadBalancerForwardingRuleCreate(c *core.CommandConfig) error {
	...
	return c.Printer.Print(
		getAlbForwardingRulePrint(
			resp, c, []resources.ApplicationLoadBalancerForwardingRule{*applicationLoadBalancerForwardingRule},
		),
	)
}
...
func RunApplicationLoadBalancerForwardingRuleDelete(c *core.CommandConfig) error {
	...
	if viper.GetBool(core.GetFlagName(c.NS, cloudapiv6.ArgAll)) {
		...
		if err = utils.AskForConfirm(
			c.Stdin, c.Printer, "delete application load balancer forwarding rule",
		); err != nil {
			return err
		}
		c.Printer.Verbose(
...
func getAlbForwardingRulePropertiesSet(c *core.CommandConfig) *resources.ApplicationLoadBalancerForwardingRuleProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgListenerIp)) {
		...
		c.Printer.Verbose(
			"Property ListenerIp set: %v", viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgListenerIp)),
		)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgListenerPort)) {
		...
		c.Printer.Verbose(
			"Property ListenerPort set: %v", viper.GetInt32(core.GetFlagName(c.NS, cloudapiv6.ArgListenerPort)),
		)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgServerCertificates)) {
		...
		c.Printer.Verbose(
			"Property ServerCertificates set: %v",
			viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv6.ArgServerCertificates)),
		)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgClientTimeout)) {
		...
		c.Printer.Verbose(
			"Property Client Timeout set: %v", viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv6.ArgClientTimeout)),
		)
	}
...
var defaultAlbForwardingRuleCols = []string{
	"ForwardingRuleId", "Name", "Protocol", "ListenerIp", "ListenerPort", "ServerCertificates", "State",
}
var allAlbForwardingRuleCols = []string{
	"ForwardingRuleId", "Name", "Protocol", "ListenerIp", "ListenerPort", "ClientTimeout", "ServerCertificates", "State",
}
...
func getAlbForwardingRulePrint(
	resp *resources.Response, c *core.CommandConfig, ss []resources.ApplicationLoadBalancerForwardingRule,
) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
		...
		if ss != nil {
			...
			r.Columns = getAlbForwardingRulesCols(core.GetFlagName(c.NS, constants.ArgCols), c.Printer.GetStderr())
		}
	}
	return r
}
