...
func TestGetK8sClusterCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("k8s-cluster", constants.ArgCols), []string{"Name"})
	getK8sClusterCols(core.GetGlobalFlagName("k8s-cluster", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetK8sClusterColsErr(t *testing.T) {
	...
	clierror.ErrAction = func() {}
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("k8s-cluster", constants.ArgCols), []string{"Unknown"})
	getK8sClusterCols(core.GetGlobalFlagName("k8s-cluster", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
