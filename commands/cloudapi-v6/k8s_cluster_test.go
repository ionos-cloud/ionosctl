...
func TestRunK8sClusterGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, &testResponse, nil)
...
func TestRunK8sClusterGetWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, testClusterErr)
		err := RunK8sClusterGet(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterGetWait(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForState), true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, nil)
		err := RunK8sClusterGet(cfg)
		assert.NoError(t, err)
	})
...
func TestRunK8sClusterCreate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.K8s.EXPECT().CreateCluster(clusterTestPost, resources.QueryParams{}).Return(&clusterTest, &testResponse, nil)
...
func TestRunK8sClusterCreateWaitIdErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV6Mocks.K8s.EXPECT().CreateCluster(clusterTestPost, resources.QueryParams{}).Return(&clusterTest, nil, nil)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterCreateWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV6Mocks.K8s.EXPECT().CreateCluster(clusterTestPost, resources.QueryParams{}).Return(&clusterTestId, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, testClusterErr)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
...
func TestRunK8sClusterCreateWaitReqErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV6Mocks.K8s.EXPECT().CreateCluster(clusterTestPost, resources.QueryParams{}).Return(&clusterTestId, &testResponse, nil)
...
func TestRunK8sClusterCreateResponseErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV6Mocks.K8s.EXPECT().CreateCluster(clusterTestPost, resources.QueryParams{}).Return(&clusterTest, &testResponse, testClusterErr)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterCreateErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV6Mocks.K8s.EXPECT().CreateCluster(clusterTestPost, resources.QueryParams{}).Return(&clusterTest, nil, testClusterErr)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterUpdate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterNewTestPut, resources.QueryParams{}).Return(&clusterNew, &testResponse, nil)
...
func TestRunK8sClusterUpdateWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgApiSubnets), []string{testClusterNewVar})
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterNewTestPut, resources.QueryParams{}).Return(&clusterNew, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, testClusterErr)
		err := RunK8sClusterUpdate(cfg)
		assert.Error(t, err)
...
func TestRunK8sClusterUpdateOldUser(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForState), false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTest, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterTestPut, resources.QueryParams{}).Return(&clusterTest, nil, nil)
		err := RunK8sClusterUpdate(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunK8sClusterUpdateErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgApiSubnets), []string{testClusterNewVar})
		rm.CloudApiV6Mocks.K8s.EXPECT().GetCluster(testClusterVar, resources.QueryParams{}).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterNewTestPut, resources.QueryParams{}).Return(&clusterNew, nil, testClusterErr)
		err := RunK8sClusterUpdate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterDelete(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgVerbose, false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV6Mocks.K8s.EXPECT().DeleteCluster(testClusterVar, resources.QueryParams{}).Return(&testResponse, nil)
...
func TestRunK8sClusterDeleteAllErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgAll), true)
		rm.CloudApiV6Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(clustersList, &testResponse, nil)
		rm.CloudApiV6Mocks.K8s.EXPECT().DeleteCluster(testClusterVar, resources.QueryParams{}).Return(&testResponse, testClusterErr)
		rm.CloudApiV6Mocks.K8s.EXPECT().DeleteCluster(testClusterVar, resources.QueryParams{}).Return(&testResponse, nil)
		err := RunK8sClusterDelete(cfg)
		assert.Error(t, err)
	})
...
func TestRunK8sClusterDeleteErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgForce, true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV6Mocks.K8s.EXPECT().DeleteCluster(testClusterVar, resources.QueryParams{}).Return(nil, testClusterErr)
		err := RunK8sClusterDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterDeleteAskForConfirm(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.K8s.EXPECT().DeleteCluster(testClusterVar, resources.QueryParams{}).Return(nil, nil)
		err := RunK8sClusterDelete(cfg)
		assert.NoError(t, err)
	})
}
