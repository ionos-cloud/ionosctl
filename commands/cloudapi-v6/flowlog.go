...
func FlowlogCmd() *core.Command {
	...
	_ = flowLogCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, flowLogCmd, core.CommandBuilder{
			Namespace:  "flowlog",
			Resource:   "flowlog",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
...
* Target S3 Bucket Name`,
	...
	create.AddBoolFlag(
		...
		"Wait for Request for FlowLog creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for FlowLog creation [seconds]",
	)
	create.AddInt32Flag(
...
func PreRunFlowLogCreate(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgServerId, cloudapiv6.ArgNicId, cloudapiv6.ArgS3Bucket,
	)
}
...
func RunFlowLogList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Flowlog, flowLogs.FlowLogs,
...
func RunFlowLogGet(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("FlowLog with id: %v from Nic with id: %v is getting...", flowLogId, nicId),
	)
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.Flowlog, flowLog.FlowLog,
		tabheaders.GetHeadersAllDefault(defaultFlowLogCols, cols),
	)
	if err != nil {
		return err
...
func RunFlowLogDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete flow log", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting FlowLog with id: %v...", flowLogId),
	)
...
func getFlowLogPropertiesSet(c *core.CommandConfig) resources.FlowLogProperties {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Direction set: %v", direction),
	)
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgS3Bucket)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Bucket set: %v", bucketName),
		)
	}
...
func getFlowLogPropertiesUpdate(c *core.CommandConfig) resources.FlowLogProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgAction)) {
		...
		properties.SetAction(strings.ToUpper(action))
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Action set: %v", action),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDirection)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property Direction set: %v", direction),
		)
	}
...
func DeleteAllFlowlogs(c *core.CommandConfig) error {
	...
	flowlogs, resp, err := c.CloudApiV6Services.FlowLogs().List(
		dcId, serverId, nicId, cloudapiv6.ParentResourceListQueryParams,
	)
	if err != nil {
		return err
	}
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the flow logs",
	...
	for _, flowlog := range *flowlogsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Flowlog with id: %v...", *id),
		)
