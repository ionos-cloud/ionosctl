...
func TestRunGroupResourceList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.Group.EXPECT().ListResources(testResourceVar, testListQueryParam).Return(resourceGroupTest, &testResponse, nil)
...
func TestRunGroupResourceListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgGroupId), testResourceVar)
		rm.CloudApiV6Mocks.Group.EXPECT().ListResources(testResourceVar, testListQueryParam).Return(resourceGroupTest, nil, testResourceErr)
		err := RunGroupResourceList(cfg)
		assert.Error(t, err)
	})
}
