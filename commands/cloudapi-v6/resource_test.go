...
func TestRunResourceGetByTypeAndId(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().GetResourceByTypeAndId(
				testResourceVar, testResourceVar,
			).Return(&resourceTestGet, &testResponse, nil)
			err := RunResourceGet(cfg)
			assert.NoError(t, err)
		},
	)
...
func TestRunResourceGetByTypeAndIdErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().GetResourceByTypeAndId(
				testResourceVar, testResourceVar,
			).Return(&resourceTestGet, nil, testResourceErr)
			err := RunResourceGet(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunGroupResourceList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListResources(
				testResourceVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(resourceGroupTest, &testResponse, nil)
...
func TestRunGroupResourceListErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListResources(
				testResourceVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(resourceGroupTest, nil, testResourceErr)
			err := RunGroupResourceList(cfg)
			assert.Error(t, err)
		},
	)
