...
- 'ionosctl img u -i kolibri.iso -l fra --ftp-url ftp://myComplexFTPServer/locations/%s --crt-path certificates/my-servers-cert.crt --location Paris,Berlin,LA,ZZZ --skip-update': Upload the image to multiple FTP servers, with location embedding into URL.`,
	...
	upload.AddStringFlag(FlagFtpUser, "", "", "Override username for FTP server. If unset, defaults to API username")
	upload.AddStringFlag(FlagFtpPass, "", "", "Override password for FTP server. If unset, defaults to API password")
	...
	// Deprecated in favor of new name: FlagRenameImages
