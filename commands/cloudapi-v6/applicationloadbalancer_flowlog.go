...
func ApplicationLoadBalancerFlowLogCmd() *core.Command {
	...
	*/
	list := core.NewCommand(
		ctx, applicationloadbalancerFlowLogCmd, core.CommandBuilder{
	...
	*/
	...
	_ = get.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ApplicationLoadBalancerFlowLogsIds(
...
* Bucket Name`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Unnamed ALB Flow Log",
		"The name of the Application Load Balancer FlowLog.",
	)
	create.AddStringFlag(
		cloudapiv6.ArgAction, cloudapiv6.ArgActionShort, "ALL", "Specifies the traffic action pattern.",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
	...
	create.AddStringFlag(
		cloudapiv6.ArgDirection, cloudapiv6.ArgDirectionShort, "INGRESS", "Specifies the traffic direction pattern.",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
	...
	create.AddBoolFlag(
		...
		"Wait for the Request for Application Load Balancer FlowLog creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.LbTimeoutSeconds,
		"Timeout option for Request for Application Load Balancer FlowLog creation [seconds]",
	)
...
* Application Load Balancer FlowLog Id`,
	...
	update.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "", "The name of the Application Load Balancer FlowLog.",
	)
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Application Load Balancer FlowLog update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.LbTimeoutSeconds,
		"Timeout option for Request for Application Load Balancer FlowLog update [seconds]",
	)
...
* Application Load Balancer FlowLog Id`,
	...
	deleteCmd.AddBoolFlag(
		cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete all Application Load Balancer FlowLogs",
	)
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Application Load Balancer FlowLog deletion to be executed",
	)
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.LbTimeoutSeconds,
		"Timeout option for Request for Application Load Balancer FlowLog deletion [seconds]",
	)
...
func PreRunApplicationLoadBalancerFlowLogCreate(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgApplicationLoadBalancerId, cloudapiv6.ArgS3Bucket,
	)
}
...
func PreRunDcApplicationLoadBalancerFlowLogIds(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgApplicationLoadBalancerId, cloudapiv6.ArgFlowLogId,
	)
}
...
func RunApplicationLoadBalancerFlowLogList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Flowlog, applicationloadbalancerFlowLogs,
...
func RunApplicationLoadBalancerFlowLogCreate(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.Flowlog, ng.FlowLog, tabheaders.GetHeadersAllDefault(defaultFlowLogCols, cols),
	)
	if err != nil {
		return err
	}
...
func RunApplicationLoadBalancerFlowLogDelete(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateVerboseOutput(
			"FlowLog ID: %v", viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgFlowLogId)),
		),
	)

	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete application load balancer flowlog",
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateLogOutput("Application Load Balancers Flowlog successfully deleted"),
	)
...
func DeleteAllApplicationLoadBalancerFlowLog(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Getting Application Load Balancer FlowLogs..."),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Application Load Balancer FlowLogs",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Application Load Balancer FlowLogs..."),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(),
		jsontabwriter.GenerateLogOutput("Application Load Balancers Flowlogs successfully deleted"),
	)
	return nil
