...
func K8sNodeCmd() *core.Command {
	...
	globalFlags.StringSliceP(constants.FlagCols, "", defaultK8sNodeCols, tabheaders.ColsMessage(defaultK8sNodeCols))
	...
	*/
	...
	get.AddUUIDFlag(cloudapiv6.ArgK8sNodeId, cloudapiv6.FlagIdShort, "", cloudapiv6.K8sNodeId, core.RequiredFlagOption())
	...
	get.AddBoolFlag(constants.FlagWaitForState, constants.FlagWaitForStateShort, constants.DefaultWait, "Wait for specified Node to be in ACTIVE state")
	get.AddIntFlag(constants.FlagTimeout, constants.FlagTimeoutShort, cloudapiv6.K8sTimeoutSeconds, "Timeout option for waiting for Node to be in ACTIVE state [seconds]")
...
* K8s Node Id`,
	...
	recreate.AddUUIDFlag(cloudapiv6.ArgK8sNodeId, cloudapiv6.FlagIdShort, "", cloudapiv6.K8sNodeId, core.RequiredFlagOption())
	...
	recreate.AddInt32Flag(cloudapiv6.FlagDepth, cloudapiv6.FlagDepthShort, cloudapiv6.DefaultCreateDepth, cloudapiv6.FlagDepthDescription)
...
* K8s Node Id`,
	...
	deleteCmd.AddBoolFlag(cloudapiv6.FlagAll, cloudapiv6.FlagAllShort, false, "Delete all the Kubernetes Nodes within an existing Kubernetes NodePool in a Cluster.")
...
func RunK8sNodeRecreate(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(c.Command.Command.InOrStdin(), "recreate k8s node", viper.GetBool(constants.FlagForce)) {
...
func RunK8sNodeDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(c.Command.Command.InOrStdin(), "delete k8s node", viper.GetBool(constants.FlagForce)) {
		return fmt.Errorf(confirm.UserDenied)
	}
...
func DeleteAllK8sNodes(c *core.CommandConfig) error {
	...
	for _, dc := range *k8sNodesItems {
		...
		if !confirm.FAsk(c.Command.Command.InOrStdin(), fmt.Sprintf("Deleting Node with ID: %v from K8s NodePool ID: %v from K8s Cluster ID: %v...", *id, nodepoolId, clusterId), viper.GetBool(constants.FlagForce)) {
			return fmt.Errorf(confirm.UserDenied)
		}
