...
func GroupCmd() *core.Command {
	...
	_ = groupCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, groupCmd, core.CommandBuilder{
			Namespace:  "group",
			Resource:   "group",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
	...
	*/
	...
	create.AddBoolFlag(
		cloudapiv6.ArgCreateDc, "", false,
		"The group will be allowed to create Data Centers. E.g.: --create-dc=true, --create-dc=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgCreateSnapshot, "", false,
		"The group will be allowed to create Snapshots. E.g.: --create-snapshot=true, --create-snapshot=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgReserveIp, "", false,
		"The group will be allowed to reserve IP addresses. E.g.: --reserve-ip=true, --reserve-ip=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgAccessLog, "", false,
		"The group will be allowed to access the activity log. E.g.: --access-logs=true, --access-logs=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgCreatePcc, "", false,
		"The group will be allowed to create PCCs. E.g.: --create-pcc=true, --create-pcc=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgS3Privilege, "", false,
		"The group will be allowed to manage S3. E.g.: --s3privilege=true, --s3privilege=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgCreateBackUpUnit, "", false,
		"The group will be able to manage Backup Units. E.g.: --create-backup=true, --create-backup=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgCreateNic, "", false,
		"The group will be allowed to create NICs. E.g.: --create-nic=true, --create-nic=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgCreateK8s, "", false,
		"The group will be allowed to create K8s Clusters. E.g.: --create-k8s=true, --create-k8s=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgCreateFlowLog, "", false,
		"The group will be allowed to create Flow Logs. E.g.: --create-flowlog=true, --create-flowlog=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgAccessMonitoring, "", false,
		"Privilege for a group to access and manage monitoring related functionality using Monotoring-as-a-Service. E.g.: --access-monitoring=true, --access-monitoring=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgAccessCerts, "", false,
		"Privilege for a group to access and manage certificates. E.g.: --access-certs=true, --access-certs=false",
	)
	create.AddBoolFlag(cloudapiv6.ArgAccessDNS, "", false, "Privilege for a group to access and manage dns records")
	create.AddBoolFlag(
		cloudapiv6.ArgManageDbaas, "", false, "Privilege for a group to manage DBaaS related functionality",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgManageDataplatform, "", false, "Privilege for a group to access and manage the Data Platform",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgManageRegistry, "", false,
	...
	create.AddBoolFlag(
		...
		"Wait for Request for Group creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Group creation [seconds]",
	)
	create.AddInt32Flag(
...
* Group Id`,
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for Request for Group update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Group update [seconds]",
	)
	update.AddInt32Flag(
...
* Group Id`,
	...
	deleteCmd.AddUUIDFlag(
		cloudapiv6.ArgGroupId, cloudapiv6.ArgIdShort, "", cloudapiv6.GroupId, core.RequiredFlagOption(),
	)
...
func RunGroupList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Group, groups.Groups,
...
func RunGroupGet(c *core.CommandConfig) error {
	...
	u, resp, err := c.CloudApiV6Services.Groups().Get(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgGroupId)), queryParams,
	)
	if resp != nil {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.Group, u.Group, tabheaders.GetHeaders(allGroupCols, allGroupCols, cols),
	)
	if err != nil {
		return err
	}
...
func RunGroupUpdate(c *core.CommandConfig) error {
	...
	groupUpd, resp, err := c.CloudApiV6Services.Groups().Update(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgGroupId,
			),
		), newGroup, queryParams,
	)
	if resp != nil && request.GetId(resp) != "" {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.Group, groupUpd.Group, tabheaders.GetHeaders(allGroupCols, allGroupCols, cols),
	)
	if err != nil {
		return err
	}
...
func RunGroupDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete group", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Group with id: %v...", groupId),
	)
...
func getGroupCreateInfo(c *core.CommandConfig) *resources.GroupProperties {
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateVerboseOutput(
			"Properties set for creating the group: Name: %v, CreateDatacenter: %v, CreateSnapshot: %v, "+
			...
			createFlowLog, monitoring, certs,
...
func getGroupUpdateInfo(oldGroup *resources.Group, c *core.CommandConfig) *resources.GroupProperties {
	...
	if properties, ok := oldGroup.GetPropertiesOk(); ok && properties != nil {
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgName)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Name set: %v", groupName),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCreateDc)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property CreateDataCenter set: %v", createDc),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCreateSnapshot)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property CreateSnapshot set: %v", createSnap),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCreatePcc)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property CreatePcc set: %v", createPcc),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCreateK8s)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property CreateK8sCluster set: %v", createK8s),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCreateNic)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property CreateInternetAccess set: %v", createNic),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCreateBackUpUnit)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property CreateBackupUnit set: %v", createBackUp),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgReserveIp)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property ReserveIp set: %v", reserveIp),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgAccessLog)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property AccessActivityLog set: %v", accessLog),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgS3Privilege)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property S3Privilege set: %v", s3),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCreateFlowLog)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property CreateFlowLog set: %v", createFlowLog),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgAccessMonitoring)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property AccessAndManageMonitoring set: %v", monitoring),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgAccessCerts)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property AccessAndManageCertificates set: %v", certs),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgAccessDNS)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property AccessAndManageDNS set: %v", dns),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgManageDbaas)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property ManageDBaaS set: %v", manageDb),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgManageRegistry)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property ManageRegistry set: %v", manageReg),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgManageDataplatform)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property ManageDataplatform set: %v", manageData),
			)
		} else {
...
func DeleteAllGroups(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Groups",
	...
	for _, group := range *groupsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Group with id: %v...", *id),
		)
	...
	fmt.Fprintf(c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Groups successfully deleted"))
	return nil
}
