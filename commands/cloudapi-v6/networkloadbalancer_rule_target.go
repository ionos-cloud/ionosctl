...
func getRuleTargetPrint(
	resp *resources.Response, c *core.CommandConfig, ss []resources.NetworkLoadBalancerForwardingRuleTarget,
) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
		...
		if ss != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				defaultRuleTargetCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
...
func getRuleTargets(targets *[]ionoscloud.NetworkLoadBalancerForwardingRuleTarget) []resources.NetworkLoadBalancerForwardingRuleTarget {
	...
	if targets != nil {
		for _, s := range *targets {
			ss = append(
				ss, resources.NetworkLoadBalancerForwardingRuleTarget{
