...
func NlbRuleTargetCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultRuleTargetCols, tabheaders.ColsMessage(defaultRuleTargetCols),
	)
	_ = viper.BindPFlag(
		core.GetFlagName(nlbRuleTargetCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols),
	)
	_ = nlbRuleTargetCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, nlbRuleTargetCmd, core.CommandBuilder{
...
* Target Port`,
	...
	add.AddStringFlag(
		cloudapiv6.ArgPort, cloudapiv6.ArgPortShort, "", "Port of the balanced target service. Range: 1 to 65535",
		core.RequiredFlagOption(),
	)
	add.AddIntFlag(
		cloudapiv6.ArgWeight, cloudapiv6.ArgWeightShort, 1,
		"Weight parameter is used to adjust the target VM's weight relative to other target VMs. Maximum: 256",
	)
	add.AddIntFlag(
		cloudapiv6.ArgCheckInterval, "", 2000,
		"[Health Check] CheckInterval determines the duration (in milliseconds) between consecutive health checks",
	)
	add.AddBoolFlag(
		cloudapiv6.ArgCheck, "", true, "[Health Check] Check specifies whether the target VM's health is checked",
	)
	add.AddBoolFlag(
		cloudapiv6.ArgMaintenance, "", false,
		"[Health Check]  Maintenance specifies if a target VM should be marked as down, even if it is not",
	)
	add.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Forwarding Rule Target creation to be executed",
	)
	add.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.NlbTimeoutSeconds,
		"Timeout option for Request for Forwarding Rule Target creation [seconds]",
	)
	add.AddInt32Flag(
...
* Target Port`,
	...
	_ = removeCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ForwardingRulesIds(
				viper.GetString(core.GetFlagName(removeCmd.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(removeCmd.NS, cloudapiv6.ArgNetworkLoadBalancerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	removeCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Forwarding Rule Target deletion to be executed",
	)
	removeCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.NlbTimeoutSeconds,
		"Timeout option for Request for Forwarding Rule Target deletion [seconds]",
	)
...
func PreRunNetworkLoadBalancerRuleTarget(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		...
		cloudapiv6.ArgIp, cloudapiv6.ArgPort,
	)
}
...
func PreRunNetworkLoadBalancerRuleTargetRemove(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlagsSets(
		c.Command, c.NS,
		[]string{
			cloudapiv6.ArgDataCenterId, cloudapiv6.ArgNetworkLoadBalancerId, cloudapiv6.ArgRuleId,
			cloudapiv6.ArgTargetIp, cloudapiv6.ArgTargetPort,
		},
		[]string{
			cloudapiv6.ArgDataCenterId, cloudapiv6.ArgNetworkLoadBalancerId, cloudapiv6.ArgRuleId, cloudapiv6.ArgAll,
		},
	)
}
...
func RunNlbRuleTargetList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.NetworkLoadBalancerRuleTarget, *targets,
...
func RunNlbRuleTargetAdd(c *core.CommandConfig) error {
	...
	_, resp, err = c.CloudApiV6Services.NetworkLoadBalancers().UpdateForwardingRule(
		dcId, nlbId, ruleId, nlbForwardingRule, queryParams,
	)
	if err != nil {
		return err
	}
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.NetworkLoadBalancerRuleTarget,
...
func RunNlbRuleTargetRemove(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete forwarding rule target",
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateLogOutput("Network Load Balancer Forwarding Rule Target successfully deleted"),
	)
	return nil
}
...
func RemoveAllNlbRuleTarget(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("NetworkLoadBalancer ID: %v", nlbId),
	)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("NetworkLoadBalancerForwardingRule ID: %v", ruleId),
	)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Getting NetworkLoadBalancerForwardingRule..."),
	)

	forwardingRule, resp, err := c.CloudApiV6Services.NetworkLoadBalancers().GetForwardingRule(
		dcId, nlbId, ruleId, cloudapiv6.ParentResourceQueryParams,
	)
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateLogOutput("Forwarding Rule Targets to be removed:"),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "remove all the Forwarding Rule Targets",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Removing all the Forwarding Rule Targets..."),
	)
	...
	_, resp, err = c.CloudApiV6Services.NetworkLoadBalancers().UpdateForwardingRule(
		dcId, nlbId, ruleId, nlbFwRuleProp, queryParams,
	)
	if resp != nil && request.GetId(resp) != "" {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(),
		jsontabwriter.GenerateLogOutput("Network Load Balancer Forwarding Rule Targets successfully deleted"),
	)
	return nil
}
