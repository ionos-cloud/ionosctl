...
func TestGetSnapshotsCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("snapshot", constants.ArgCols), []string{"Name"})
	getSnapshotCols(core.GetGlobalFlagName("snapshot", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetSnapshotsColsErr(t *testing.T) {
	...
	clierror.ErrAction = func() {}
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("snapshot", constants.ArgCols), []string{"Unknown"})
	getSnapshotCols(core.GetGlobalFlagName("snapshot", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
