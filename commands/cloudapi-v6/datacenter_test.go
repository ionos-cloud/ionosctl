...
func TestGetDatacentersCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("datacenter", constants.ArgCols), []string{"Name"})
	getDataCenterCols(core.GetGlobalFlagName("datacenter", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetDatacentersColsErr(t *testing.T) {
	...
	clierror.ErrAction = func() { return }
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("datacenter", constants.ArgCols), []string{"Unknown"})
	getDataCenterCols(core.GetGlobalFlagName("datacenter", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
