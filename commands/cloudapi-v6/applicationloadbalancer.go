...
var (
	defaultApplicationLoadBalancerCols = []string{
		"ApplicationLoadBalancerId", "Name", "ListenerLan", "Ips", "TargetLan", "PrivateIps", "State",
	}
	allApplicationLoadBalancerCols = []string{
		"ApplicationLoadBalancerId", "DatacenterId", "Name", "ListenerLan", "Ips", "TargetLan", "PrivateIps", "State",
	}
)
...
func ApplicationLoadBalancerCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultApplicationLoadBalancerCols,
		tabheaders.ColsMessage(defaultApplicationLoadBalancerCols),
	)
	_ = viper.BindPFlag(
		core.GetFlagName(applicationloadbalancerCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols),
	)
	_ = applicationloadbalancerCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, applicationloadbalancerCmd, core.CommandBuilder{
	...
	*/
	...
	get.AddUUIDFlag(
		cloudapiv6.ArgApplicationLoadBalancerId, cloudapiv6.ArgIdShort, "", cloudapiv6.ApplicationLoadBalancerId,
	...
	get.AddBoolFlag(
		...
		"Wait for specified Application Load Balancer to be in AVAILABLE state",
	)
	get.AddInt32Flag(
	...
	get.AddIntFlag(
		...
		"Timeout option for waiting for Application Load Balancer to be in AVAILABLE state [seconds]",
	)
...
* Data Center Id`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Unnamed Application Load Balancer",
	...
	create.AddBoolFlag(
		...
		"Wait for the Request for Application Load Balancer creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.AlbTimeoutSeconds,
		"Timeout option for Request for Application Load Balancer creation [seconds]",
	)
	create.AddInt32Flag(
...
* Application Load Balancer Id`,
	...
	update.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Application Load Balancer",
		"The name of the Application Load Balancer.",
	)
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Application Load Balancer update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.LbTimeoutSeconds,
		"Timeout option for Request for Application Load Balancer update [seconds]",
	)
	update.AddInt32Flag(
...
* Application Load Balancer Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Application Load Balancer deletion to be executed",
	)
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.LbTimeoutSeconds,
		"Timeout option for Request for Application Load Balancer deletion [seconds]",
	)
...
func PreRunApplicationLoadBalancerList(c *core.PreCommandConfig) error {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgFilters)) {
		return query.ValidateFilters(
			c, completer.ApplicationLoadBalancersFilters(), completer.ApplicationLoadBalancersFiltersUsage(),
		)
	}
	return nil
}
...
func RunApplicationLoadBalancerListAll(c *core.CommandConfig) error {
	...
	for _, dc := range allDcs {
		...
		allApplicationLoadBalancers = append(
			allApplicationLoadBalancers, ApplicationLoadBalancers.ApplicationLoadBalancers,
		)
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		allApplicationLoadBalancers, allApplicationLoadBalancersConverted,
...
func RunApplicationLoadBalancerList(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Getting ApplicationLoadBalancers from Datacenter with ID: %v",
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.ApplicationLoadBalancer, applicationloadbalancers,
...
func RunApplicationLoadBalancerGet(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.ApplicationLoadBalancer, ng.ApplicationLoadBalancer,
		tabheaders.GetHeadersAllDefault(defaultApplicationLoadBalancerCols, cols),
	)
...
func RunApplicationLoadBalancerCreate(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Creating ApplicationLoadBalancer"),
	)
	...
	if resp != nil {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Request href: %v ", resp.Header.Get("location")),
		)
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput(constants.MessageRequestTime, resp.RequestTime),
...
func RunApplicationLoadBalancerUpdate(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Updating ApplicationLoadBalancer with ID: %v",
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgApplicationLoadBalancerId)),
		),
	)
...
func RunApplicationLoadBalancerDelete(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			constants.ApplicationLoadBalancerId,
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgApplicationLoadBalancerId)),
		),
	)

	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete application load balancer",
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting ApplicationLoadBalancer"),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(),
		jsontabwriter.GenerateLogOutput("Application Load Balancer successfully deleted"),
	)
...
func DeleteAllApplicationLoadBalancer(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateLogOutput("Getting Application Load Balancers..."),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Application Load Balancers",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Application Load Balancers..."),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(),
		jsontabwriter.GenerateLogOutput("Application Load Balancers successfully deleted"),
	)
	return nil
}
...
func getNewApplicationLoadBalancerInfo(c *core.CommandConfig) *resources.ApplicationLoadBalancerProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgIps)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Property IPs set: %v", viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv6.ArgIps)),
			),
		)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgListenerLan)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Property ListenerLan set: %v", viper.GetInt32(core.GetFlagName(c.NS, cloudapiv6.ArgListenerLan)),
			),
		)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgTargetLan)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Property TargetLan set: %v", viper.GetInt32(core.GetFlagName(c.NS, cloudapiv6.ArgTargetLan)),
			),
		)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgPrivateIps)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Property LbPrivateIps set: %v", viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv6.ArgPrivateIps)),
			),
		)
	}
