...
func TestRunServerVolumeDetachAllListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgAll), true)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForRequest), false)
		rm.CloudApiV6Mocks.Server.EXPECT().ListVolumes(testServerVar, testServerVar, testListQueryParam).Return(vsAttachedList, nil, testVolumeErr)
		err := RunServerVolumeDetach(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunServerVolumeDetachAllItemsErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgAll), true)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForRequest), false)
		rm.CloudApiV6Mocks.Server.EXPECT().ListVolumes(testServerVar, testServerVar, testListQueryParam).Return(resources.AttachedVolumes{}, nil, nil)
		err := RunServerVolumeDetach(cfg)
		assert.Error(t, err)
	})
}
