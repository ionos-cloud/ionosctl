...
func TestRunLabelList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(constants.ArgOutput, constants.DefaultOutputFormat)
			viper.Set(constants.ArgQuiet, false)
			rm.CloudApiV6Mocks.Label.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				testLabels, nil, nil,
			)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			).Return(testLabelResources, nil, nil)
			err := RunLabelList(cfg)
			assert.NoError(t, err)
		},
	)
...
func TestRunLabelGet(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			).Return(&testLabelResourceRes, nil, nil)
			err := RunLabelGet(cfg)
			assert.NoError(t, err)
		},
	)
