...
func TestLoadbalancersCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("loadbalancer", constants.ArgCols), []string{"Name"})
	getLoadbalancersCols(
		core.GetGlobalFlagName("loadbalancer", constants.ArgCols),
...
func TestGetLoadbalancersColsErr(t *testing.T) {
	...
	clierror.ErrAction = func() { return }
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("loadbalancer", constants.ArgCols), []string{"Unknown"})
	getLoadbalancersCols(
		core.GetGlobalFlagName("loadbalancer", constants.ArgCols),
		core.GetFlagName("loadbalancer", cloudapiv6.ArgAll),
		w,
