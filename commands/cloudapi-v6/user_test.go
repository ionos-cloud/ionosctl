...
func TestPreRunUserListFilters(t *testing.T) {
	...
	core.PreCmdConfigTest(t, w, func(cfg *core.PreCommandConfig) {
		...
		viper.Set(constants.ArgQuiet, false)
		viper.Set(constants.ArgOutput, constants.DefaultOutputFormat)
		cfg.Command.Command.Flags().Set(constants.FlagFilters, fmt.Sprintf("firstname=%s", testQueryParamVar))
		err := PreRunUserList(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunUserListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.User.EXPECT().List().Return(resources.Users{}, &testResponse, nil)
		err := RunUserList(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunGroupUserAdd(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgGroupId), testGroupVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgUserId), testUserVar)
		rm.CloudApiV6Mocks.Group.EXPECT().AddUser(testGroupVar, userGroupAdd).Return(&userTestGet, &testResponse, nil)
...
func TestRunGroupUserAddResponse(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgGroupId), testGroupVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgUserId), testUserVar)
		rm.CloudApiV6Mocks.Group.EXPECT().AddUser(testGroupVar, userGroupAdd).Return(&userTestGet, &testResponse, testUserErr)
		err := RunGroupUserAdd(cfg)
		assert.Error(t, err)
	})
}
