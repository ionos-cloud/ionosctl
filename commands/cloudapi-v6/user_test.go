...
func TestGetUsersCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("user", constants.ArgCols), []string{"Firstname"})
	getUserCols(core.GetGlobalFlagName("user", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetUsersColsErr(t *testing.T) {
	...
	clierror.ErrAction = func() {}
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("user", constants.ArgCols), []string{"Unknown"})
	getUserCols(core.GetGlobalFlagName("user", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
