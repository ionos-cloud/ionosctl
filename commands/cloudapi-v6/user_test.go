	...
	core.PreCmdConfigTest(
		t, w, func(cfg *core.PreCommandConfig) {
			...
			viper.Set(
				...
				[]string{fmt.Sprintf("firstname=%s", testQueryParamVar)},
			)
			err := PreRunUserList(cfg)
			assert.NoError(t, err)
		},
...
func TestRunUserListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				resources.Users{}, &testResponse, nil,
			)
			err := RunUserList(cfg)
			assert.NoError(t, err)
		},
...
func TestRunUserGet(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(constants.ArgOutput, constants.DefaultOutputFormat)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgUserId), testUserVar)
			rm.CloudApiV6Mocks.User.EXPECT().Get(
				testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userTestGet, &testResponse, nil)
			err := RunUserGet(cfg)
			assert.NoError(t, err)
		},
	)
...
func TestRunUserGetErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().Get(
				testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userTestGet, nil, testUserErr)
			err := RunUserGet(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunUserCreate(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgEmail), testUserVar)
			viper.Set(core.GetFlagName(cfg.NS, constants.ArgPassword), testUserVar)
			rm.CloudApiV6Mocks.User.EXPECT().Create(
				userTest, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userTestGet, &testResponse, nil)
...
func TestRunUserCreateResponseErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().Create(
				userTest, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userTestGet, &testResponse, testUserErr)
			err := RunUserCreate(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunUserUpdate(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().Get(
				testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userTestGet, nil, nil)
			rm.CloudApiV6Mocks.User.EXPECT().Update(
				testUserVar, userNewPut, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userNew, &testResponse, nil)
...
func TestRunUserUpdateErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().Update(
				testUserVar, userNewPut, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userNew, nil, testUserErr)
			err := RunUserUpdate(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunUserDelete(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(constants.ArgVerbose, false)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgUserId), testUserVar)
			rm.CloudApiV6Mocks.User.EXPECT().Delete(
				testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
...
func TestRunUserDeleteAllErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.User.EXPECT().Delete(
				testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, testUserErr)
			rm.CloudApiV6Mocks.User.EXPECT().Delete(
				testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, nil)
			err := RunUserDelete(cfg)
...
func TestRunGroupUserList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListUsers(
				testGroupVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(groupUsersTest, &testResponse, nil)
...
func TestRunGroupUserListErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListUsers(
				testGroupVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(groupUsersTest, nil, testGroupErr)
			err := RunGroupUserList(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunGroupUserAdd(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgGroupId), testGroupVar)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgUserId), testUserVar)
			rm.CloudApiV6Mocks.Group.EXPECT().AddUser(
				testGroupVar, groupUserTest, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&userTestGet, &testResponse, nil)
...
func TestRunGroupUserRemove(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgGroupId), testGroupVar)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgUserId), testUserVar)
			rm.CloudApiV6Mocks.Group.EXPECT().RemoveUser(
				testGroupVar, testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, nil)
...
func TestRunGroupUserRemoveAll(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListUsers(
				testGroupVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(groupUsersTestList, &testResponse, nil)
			rm.CloudApiV6Mocks.Group.EXPECT().RemoveUser(
				testGroupVar, testUserVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, nil)
			rm.CloudApiV6Mocks.Group.EXPECT().RemoveUser(
...
func TestRunGroupUserRemoveAllListErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListUsers(
				testGroupVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(groupUsersTestList, nil, testUserErr)
			err := RunGroupUserRemove(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunGroupUserRemoveAllItemsErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListUsers(
				testGroupVar, gomock.AssignableToTypeOf(testListQueryParam),
			).Return(resources.GroupMembers{}, &testResponse, nil)
			err := RunGroupUserRemove(cfg)
			assert.Error(t, err)
		},
	)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Group.EXPECT().ListUsers(
				...
				resources.GroupMembers{GroupMembers: ionoscloud.GroupMembers{Items: &[]ionoscloud.User{}}},
				&testResponse, nil,
			)
			err := RunGroupUserRemove(cfg)
			assert.Error(t, err)
