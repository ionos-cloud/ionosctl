...
func LoadBalancerCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultLoadbalancerCols, tabheaders.ColsMessage(allLoadbalancerCols),
	)
	_ = viper.BindPFlag(
		core.GetFlagName(loadbalancerCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols),
	)
	_ = loadbalancerCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, loadbalancerCmd, core.CommandBuilder{
	...
	*/
	...
	get.AddUUIDFlag(
		cloudapiv6.ArgLoadBalancerId, cloudapiv6.ArgIdShort, "", cloudapiv6.LoadBalancerId, core.RequiredFlagOption(),
	)
	_ = get.Command.RegisterFlagCompletionFunc(
...
* Data Center Id`,
	...
	create.AddBoolFlag(
		...
		"Indicates if the Load Balancer will reserve an IP using DHCP. E.g.: --dhcp=true, --dhcp=false",
	)
	create.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for Request for Load Balancer creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Load Balancer creation [seconds]",
	)
	create.AddInt32Flag(
...
* Load Balancer Id`,
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for Request for Load Balancer update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Load Balancer update [seconds]",
	)
	update.AddInt32Flag(
...
* Load Balancer Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for Request for Load Balancer deletion to be executed",
	)
	deleteCmd.AddBoolFlag(
		cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete all the LoadBlancers from a virtual Datacenter.",
	)
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Load Balancer deletion [seconds]",
	)
...
func RunLoadBalancerListAll(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		allLoadbalancers, allLoadbalancersConverted,
...
func RunLoadBalancerList(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Getting LoadBalancers from Datacenter with ID: %v...",
	...
	lbs, resp, err := c.CloudApiV6Services.Loadbalancers().List(
		viper.GetString(
			core.GetFlagName(
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.LoadBalancer, lbs.Loadbalancers,
		tabheaders.GetHeaders(allLoadbalancerCols, defaultLoadbalancerCols, cols),
	)
	if err != nil {
		return err
...
func RunLoadBalancerGet(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Load balancer with id: %v is getting...",
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.LoadBalancer, lb.Loadbalancer,
		tabheaders.GetHeaders(allLoadbalancerCols, defaultLoadbalancerCols, cols),
	)
	if err != nil {
		return err
...
func RunLoadBalancerDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete loadbalancer", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
...
func DeleteAllLoadBalancers(c *core.CommandConfig) error {
	...
	loadBalancers, resp, err := c.CloudApiV6Services.Loadbalancers().List(
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the LoadBalancers",
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Deleting all the LoadBalancers..."),
	)
	...
	for _, lb := range *loadBalancersItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Load balancer with id: %v ...", *id),
		)
