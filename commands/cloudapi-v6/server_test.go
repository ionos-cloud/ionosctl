...
func TestRunServerListAll(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.Server.EXPECT().List(testDatacenterVar).Return(ss, &testResponse, nil).Times(len(getDataCenters(dcs)))
...
func TestRunServerListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.Server.EXPECT().List(testServerVar).Return(resources.Servers{}, &testResponse, nil)
		err := RunServerList(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunServerGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgDataCenterId), testServerVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgServerId), testServerVar)
		rm.CloudApiV6Mocks.Server.EXPECT().Get(testServerVar, testServerVar).Return(&resources.Server{Server: s}, &testResponse, nil)
