...
func TestGetPccsCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("pcc", constants.ArgCols), []string{"Name"})
	getPccCols(core.GetGlobalFlagName("pcc", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetPccsColsErr(t *testing.T) {
	...
	clierror.ErrAction = func() {}
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("pcc", constants.ArgCols), []string{"Unknown"})
	getPccCols(core.GetGlobalFlagName("pcc", constants.ArgCols), w)
	err := w.Flush()
	assert.NoError(t, err)
