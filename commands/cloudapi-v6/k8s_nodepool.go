...
var (
	...
	allK8sNodePoolCols     = []string{
		...
		"CoresCount", "RamSize", "AvailabilityZone", "StorageSize", "MaintenanceWindow", "AutoScaling", "PublicIps",
...
func K8sNodePoolCmd() *core.Command {
	...
	*/
	...
	get.AddUUIDFlag(
		constants.FlagNodepoolId, cloudapiv6.ArgIdShort, "", cloudapiv6.K8sNodePoolId, core.RequiredFlagOption(),
	)
	_ = get.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.K8sNodePoolsIds(
				viper.GetString(
					core.GetFlagName(
						get.NS, constants.FlagClusterId,
					),
				),
	...
	get.AddBoolFlag(
		...
		"Wait for specified NodePool to be in ACTIVE state",
	)
	get.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.K8sTimeoutSeconds,
		"Timeout option for waiting for NodePool to be in ACTIVE state [seconds]",
	)
	get.AddInt32Flag(
	...
	*/
	...
	create := core.NewCommandWithJsonProperties(
		ctx, k8sCmd, jsonPropertiesExample, &nodepoolViaJsonPropertiesFlag, core.CommandBuilder{
			Namespace: "k8s",
			Resource:  "nodepool",
			Verb:      "create",
			Aliases:   []string{"c"},
...
* Datacenter Id`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "UnnamedNodePool", "The name for the K8s NodePool",
	)
	create.AddStringFlag(
		cloudapiv6.ArgK8sVersion, "", "", "The K8s version for the NodePool. If not set, the default one will be used",
	)
	...
	create.AddIntSliceFlag(
		cloudapiv6.ArgLanIds, "", []int{}, "Collection of LAN Ids of existing LANs to be attached to worker Nodes",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgLanIds,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	create.AddBoolFlag(
		...
		"Indicates if the Kubernetes Node Pool LANs will reserve an IP using DHCP. E.g.: --dhcp=true, --dhcp=false",
	)
	create.AddIntFlag(
		constants.FlagNodeCount, "", 1,
		"The number of worker Nodes that the Node Pool should contain. Min 1, Max: Determined by the resource availability",
	)
	create.AddIntFlag(constants.FlagCores, "", 2, "The total number of cores for the Node")
	create.AddStringFlag(
		constants.FlagRam, "", strconv.Itoa(2048),
		"RAM size for node, minimum size is 2048MB. Ram size must be set to multiple of 1024MB. e.g. --ram 2048 or --ram 2048MB",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		constants.FlagRam,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	create.AddStringFlag(
		constants.FlagAvailabilityZone, constants.FlagAvailabilityZoneShort, "AUTO",
		"The compute Availability Zone in which the Node should exist",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
	...
	_ = create.Command.RegisterFlagCompletionFunc(
		constants.FlagStorageType,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return []string{"HDD", "SSD"}, cobra.ShellCompDirectiveNoFileComp
		},
	)
	create.AddStringFlag(
		constants.FlagStorageSize, "", strconv.Itoa(cloudapiv6.DefaultVolumeSize),
		"The size of the Storage in GB. e.g.: --size 10 or --size 10GB. The maximum Volume size is determined by your contract limit",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		constants.FlagStorageSize,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return []string{"10GB", "20GB", "50GB", "100GB", "1TB"}, cobra.ShellCompDirectiveNoFileComp
		},
	)
	create.AddStringToStringFlag(
		constants.FlagLabels, constants.FlagLabelsShort, map[string]string{},
		"Labels to set on a NodePool. It will overwrite the existing labels, if there are any. Use the following format: --labels KEY=VALUE,KEY=VALUE",
	)
	create.AddStringToStringFlag(
		constants.FlagAnnotations, constants.FlagAnnotationsShort, map[string]string{},
		"Annotations to set on a NodePool. It will overwrite the existing annotations, if there are any. Use the following format: --annotations KEY=VALUE,KEY=VALUE",
	)
	create.AddBoolFlag(
		constants.ArgWaitForState, constants.ArgWaitForStateShort, constants.DefaultWait,
		"Wait for the new NodePool to be in ACTIVE state",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.K8sTimeoutSeconds,
		"Timeout option for waiting for NodePool to be in ACTIVE state[seconds]",
...
* K8s NodePool Id`,
	...
	update.AddStringFlag(
		cloudapiv6.ArgK8sVersion, "", "", "The K8s version for the NodePool. K8s version downgrade is not supported",
	)
	_ = update.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgK8sVersion,
	...
	update.AddIntFlag(
		cloudapiv6.ArgK8sMinNodeCount, "", 1,
		"The minimum number of worker Nodes that the managed NodePool can scale in. Should be set together with --max-node-count",
	)
	update.AddIntFlag(
		cloudapiv6.ArgK8sMaxNodeCount, "", 1,
		"The maximum number of worker Nodes that the managed NodePool can scale out. Should be set together with --min-node-count",
	)
	update.AddStringToStringFlag(
		constants.FlagLabels, constants.FlagLabelsShort, map[string]string{},
		"Labels to set on a NodePool. It will overwrite the existing labels, if there are any. Use the following format: --labels KEY=VALUE,KEY=VALUE",
	)
	update.AddStringToStringFlag(
	...
	update.AddStringFlag(
		cloudapiv6.ArgLabelKey, "", "", "Label key. Must be set together with --label-value",
		core.DeprecatedFlagOption("Use --labels, --annotations options instead!"),
	)
	update.AddStringFlag(
		cloudapiv6.ArgLabelValue, "", "", "Label value. Must be set together with --label-key",
		core.DeprecatedFlagOption("Use --labels, --annotations options instead!"),
	)
	update.AddStringFlag(
		cloudapiv6.ArgK8sAnnotationKey, "", "", "Annotation key. Must be set together with --annotation-value",
		core.DeprecatedFlagOption("Use --labels, --annotations options instead!"),
	)
	update.AddStringFlag(
		cloudapiv6.ArgK8sAnnotationValue, "", "", "Annotation value. Must be set together with --annotation-key",
		core.DeprecatedFlagOption("Use --labels, --annotations options instead!"),
	)
	update.AddStringFlag(
		cloudapiv6.ArgK8sMaintenanceDay, "", "",
		"The day of the week for Maintenance Window has the English day format as following: Monday or Saturday",
	)
	_ = update.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgK8sMaintenanceDay,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	update.AddStringFlag(
		cloudapiv6.ArgK8sMaintenanceTime, "", "",
		"The time for Maintenance Window has the HH:mm:ss format as following: 08:00:00",
	)
	update.AddStringSliceFlag(
		cloudapiv6.ArgPublicIps, "", []string{},
		"Reserved public IP address to be used by the Nodes. IPs must be from same location as the Data Center used for the Node Pool. Usage: --public-ips IP1,IP2",
	)
	update.AddIntSliceFlag(
		cloudapiv6.ArgLanIds, "", []int{},
		"Collection of LAN Ids of existing LANs to be attached to worker Nodes. It will be added to the existing LANs attached",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgDhcp, "", true,
		"Indicates if the Kubernetes Node Pool LANs will reserve an IP using DHCP. E.g.: --dhcp=true, --dhcp=false",
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.K8sNodePoolsIds(
				viper.GetString(
					core.GetFlagName(
						update.NS, constants.FlagClusterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
...
* K8s NodePool Id`,
	...
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.K8sNodePoolsIds(
				viper.GetString(
					core.GetFlagName(
						deleteCmd.NS, constants.FlagClusterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	deleteCmd.AddBoolFlag(
		...
		"Delete all the Kubernetes Node Pools within an existing Kubernetes Nodepools.",
	)
...
func RunK8sNodePoolCreateFromJSON(
	c *core.CommandConfig, propertiesFromJson ionoscloud.KubernetesNodePoolForPost,
) error {
	np, _, err := client.Must().CloudClient.KubernetesApi.K8sNodepoolsPost(
		context.Background(),
		viper.GetString(core.GetFlagName(c.NS, constants.FlagClusterId)),
	).KubernetesNodePool(propertiesFromJson).Execute()
	if err != nil {
...
func RunK8sNodePoolCreate(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Creating K8s NodePool in K8s Cluster with ID: %v",
			viper.GetString(core.GetFlagName(c.NS, constants.FlagClusterId)),
		),
	)

	u, resp, err := c.CloudApiV6Services.K8s().CreateNodePool(
		viper.GetString(
			core.GetFlagName(
				c.NS, constants.FlagClusterId,
			),
		), *newNodePool, queryParams,
	)
...
func handleApiResponseK8sNodepoolCreate(c *core.CommandConfig, pool ionoscloud.KubernetesNodePool) error {
	...
	if viper.GetBool(core.GetFlagName(c.NS, constants.ArgWaitForState)) {
		if id, ok := pool.GetIdOk(); ok && id != nil {
			...
			if pool, _, err = client.Must().CloudClient.KubernetesApi.K8sNodepoolsFindById(
				context.Background(),
...
func RunK8sNodePoolListAll(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		allNodePools, allNodePoolsConverted,
		tabheaders.GetHeaders(allK8sNodePoolCols, defaultK8sNodePoolCols, cols),
	)
...
func RunK8sNodePoolList(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Getting K8s NodePools from K8s Cluster with ID: %v",
			viper.GetString(core.GetFlagName(c.NS, constants.FlagClusterId)),
		),
	)
	...
	k8ss, resp, err := c.CloudApiV6Services.K8s().ListNodePools(
		viper.GetString(
			core.GetFlagName(
				c.NS, constants.FlagClusterId,
			),
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		k8ss.KubernetesNodePools, k8ssConverted,
		tabheaders.GetHeaders(allK8sNodePoolCols, defaultK8sNodePoolCols, cols),
	)
	if err != nil {
		return err
...
func RunK8sNodePoolGet(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		u.KubernetesNodePool, uConverted,
		tabheaders.GetHeaders(allK8sNodePoolCols, defaultK8sNodePoolCols, cols),
	)
	if err != nil {
		return err
...
func RunK8sNodePoolUpdate(c *core.CommandConfig) error {
	...
	queryParams := listQueryParams.QueryParams
	oldNodePool, _, err := c.CloudApiV6Services.K8s().GetNodePool(
		viper.GetString(core.GetFlagName(c.NS, constants.FlagClusterId)),
	...
	if err = waitfor.WaitForState(
		c, waiter.K8sNodePoolStateInterrogator, viper.GetString(core.GetFlagName(c.NS, constants.FlagNodepoolId)),
	); err != nil {
		return err
	}

	newNodePoolUpdated, _, err := c.CloudApiV6Services.K8s().GetNodePool(
		viper.GetString(core.GetFlagName(c.NS, constants.FlagClusterId)),
		viper.GetString(core.GetFlagName(c.NS, constants.FlagNodepoolId)), queryParams,
	)
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		newNodePoolUpdated.KubernetesNodePool, newNodePoolUpdatedConverted,
		tabheaders.GetHeaders(allK8sNodePoolCols, defaultK8sNodePoolCols, cols),
	)
	if err != nil {
		return err
...
func RunK8sNodePoolDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete k8s node pool", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Kubernetes Nodepool successfully deleted"),
	)
	return nil
}
...
func getNewK8sNodePool(c *core.CommandConfig) (*resources.K8sNodePoolForPost, error) {
	...
	storageSize, err := utils2.ConvertSize(
		viper.GetString(core.GetFlagName(c.NS, constants.FlagStorageSize)), utils2.GigaBytes,
	)
	if err != nil {
		return nil, err
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property K8sVersion set: %v", k8sversion),
	)

	nodePoolProperties.SetNodeCount(nodeCount)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property NodeCount set: %v", nodeCount),
	)

	nodePoolProperties.SetDatacenterId(dcId)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property DatacenterId set: %v", dcId),
	)

	nodePoolProperties.SetCpuFamily(cpuFamily)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property CPU Family set: %v", cpuFamily),
	)

	nodePoolProperties.SetCoresCount(cores)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property CoresCount set: %v", cores),
	)

	nodePoolProperties.SetRamSize(int32(ramSize))
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Property RAM Size set: %vMB", int32(ramSize)),
	)

	nodePoolProperties.SetAvailabilityZone(availabilityZone)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Property Availability Zone set: %v", availabilityZone),
	)

	nodePoolProperties.SetStorageSize(int32(storageSize))
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Property Storage Size set: %vGB", int32(storageSize)),
	)

	nodePoolProperties.SetStorageType(storageType)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Property Storage Type set: %v", storageType),
	)
	...
	if viper.IsSet(core.GetFlagName(c.NS, constants.FlagLabels)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property Labels set: %v", keyValueMapLabels),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, constants.FlagAnnotations)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property Annotations set: %v", keyValueMapAnnotations),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgLanIds)) {
		...
		for _, lanId := range lanIds {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Lan ID set: %v", id),
			)
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Dhcp set: %v", dhcp),
			)

			newLans = append(
				newLans, ionoscloud.KubernetesNodePoolLan{
					Id:   &id,
					Dhcp: &dhcp,
				},
			)
...
func getNewK8sNodePoolUpdated(oldUser *resources.K8sNodePool, c *core.CommandConfig) resources.K8sNodePoolForPut {
	...
	if properties, ok := oldUser.GetPropertiesOk(); ok && properties != nil {
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sVersion)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property K8sVersion set: %v", vers),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, constants.FlagNodeCount)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property NodeCount set: %v", nodeCount),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sMinNodeCount)) ||
			...
			if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sMinNodeCount)) {
				...
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateVerboseOutput("Property MinNodeCount set: %v", minCount),
				)
			} else {
			...
			if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sMaxNodeCount)) {
				...
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateVerboseOutput("Property MaxNodeCount set: %v", maxCount),
				)
			} else {
			...
			propertiesUpdated.SetAutoScaling(
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sMaintenanceDay)) ||
			...
			if maintenance, ok := properties.GetMaintenanceWindowOk(); ok && maintenance != nil {
				newMaintenanceWindow := getMaintenanceInfo(
					c, &resources.K8sMaintenanceWindow{
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sAnnotationKey)) &&
			...
			propertiesUpdated.SetAnnotations(
				map[string]string{
					key: value,
				},
			)
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property Annotations set: key: %v, value: %v", key, value),
			)
		}
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgLabelKey)) &&
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property Labels set: key: %v, value: %v", key, value),
			)
		}
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgLanIds)) {
			...
			for _, lanId := range lanIds {
				...
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Lans set: %v", id),
				)
			}
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgPublicIps)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property PublicIps set: %v", publicIps),
			)
		}
	}
...
func DeleteAllK8sNodepools(c *core.CommandConfig) error {
	...
	k8sNodePools, resp, err := c.CloudApiV6Services.K8s().ListNodePools(
		k8sClusterId, cloudapiv6.ParentResourceListQueryParams,
	)
	if err != nil {
		return err
	}
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the K8sNodePools",
	...
	for _, dc := range *k8sNodePoolsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting K8sNodePool with id: %v...", *id),
		)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Kubernetes Nodepools successfully deleted"),
	)
	return nil
}
