	...
	_ = create.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.LansIds(
				os.Stderr, viper.GetString(core.GetFlagName(create.NS, cloudapiv6.ArgDataCenterId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	create.AddBoolFlag(
	...
	_ = create.Command.RegisterFlagCompletionFunc(
		constants.FlagCpuFamily, func(*cobra.Command, []string, string) ([]string, cobra.ShellCompDirective) {
			...
			return completer.DatacenterCPUFamilies(
				create.Command.Context(), os.Stderr, datacenterId,
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	create.AddStringFlag(
...
* K8s NodePool Id`,
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(*cobra.Command, []string, string) ([]string, cobra.ShellCompDirective) {
			...
			return completer.K8sNodePoolUpgradeVersions(
				os.Stderr, clusterId, nodepoolId,
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.K8sNodePoolsIds(
				os.Stderr, viper.GetString(core.GetFlagName(update.NS, constants.FlagClusterId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	update.AddBoolFlag(
...
func RunK8sNodePoolCreate(c *core.CommandConfig) error {
	...
	if viper.GetBool(core.GetFlagName(c.NS, constants.ArgWaitForState)) {
		if id, ok := u.GetIdOk(); ok && id != nil {
			...
			if u, _, err = c.CloudApiV6Services.K8s().GetNodePool(
				viper.GetString(
					core.GetFlagName(
						c.NS, constants.FlagClusterId,
					),
...
func getNewK8sNodePool(c *core.CommandConfig) (*resources.K8sNodePoolForPost, error) {
	...
	storageSize, err := utils.ConvertSize(
		viper.GetString(core.GetFlagName(c.NS, constants.FlagStorageSize)), utils.GigaBytes,
	)
	if err != nil {
		return nil, err
	}
	...
	if c != nil {
		if k8ss != nil {
			...
			r.Columns = printer.GetHeaders(
				allK8sNodePoolCols, defaultK8sNodePoolCols,
