...
func LanCmd() *core.Command {
	...
	_ = lanCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, lanCmd, core.CommandBuilder{
			Namespace:  "lan",
			Resource:   "lan",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
...
* Data Center Id`,
	...
	create.AddBoolFlag(
		cloudapiv6.ArgPublic, cloudapiv6.ArgPublicShort, cloudapiv6.DefaultPublic,
		"Indicates if the LAN faces the public Internet (true) or not (false). E.g.: --public=true, --public=false",
	)
	...
	create.AddBoolFlag(
		...
		"Wait for the Request for LAN creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for LAN creation [seconds]",
	)
	create.AddInt32Flag(
...
* LAN Id`,
	...
	update.AddBoolFlag(
		cloudapiv6.ArgPublic, "", cloudapiv6.DefaultPublic, "Public option for LAN. E.g.: --public=true, --public=false",
	)
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for LAN update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for LAN update [seconds]",
	)
	update.AddInt32Flag(
	...
	update.AddStringFlag(
		cloudapiv6.FlagIPv6CidrBlock, "", "DISABLE", cloudapiv6.FlagIPv6CidrBlockDescriptionForLAN+
...
* LAN Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for Request for LAN deletion to be executed",
	)
	deleteCmd.AddBoolFlag(
		cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete all Lans from a Virtual Data Center.",
	)
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for LAN deletion [seconds]",
	)
...
func RunLanList(c *core.CommandConfig) error {
	...
	lans, resp, err := c.CloudApiV6Services.Lans().List(
		viper.GetString(
			core.GetFlagName(
...
func RunLanGet(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Lan with id: %v from Datacenter with id: %v is getting...",
...
func RunLanCreate(c *core.CommandConfig) error {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.FlagIPv6CidrBlock)) {
		...
		default:
			...
			dc, _, err := client.Must().CloudClient.DataCentersApi.DatacentersFindById(
				context.Background(), dcId,
			).Execute()
			if err != nil {
				return err
			}
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property IPv6 Cidr Block set: %v", cidr),
		)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Creating LAN in Datacenter with ID: %v...",
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgDataCenterId)),
		),
	)
...
func RunLanUpdate(c *core.CommandConfig) error {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgPublic)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Public set: %v", public),
		)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Updating LAN with ID: %v from Datacenter with ID: %v...",
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgLanId)),
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgDataCenterId)),
		),
	)
...
func RunLanDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete lan", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
...
func DeleteAllLans(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Lans", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	for _, lan := range *lansItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Lan with id: %v...", *id),
		)
