...
func IpfailoverCmd() *core.Command {
	...
	globalFlags.StringSliceP(constants.ArgCols, "", defaultIpFailoverCols, printer.ColsMessage(defaultIpFailoverCols))
	_ = viper.BindPFlag(
	...
	_ = listCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.LansIds(
				os.Stderr, viper.GetString(core.GetFlagName(listCmd.NS, cloudapiv6.ArgDataCenterId)),
...
* IP address`,
	...
	_ = addCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.LansIds(
				os.Stderr, viper.GetString(core.GetFlagName(addCmd.NS, cloudapiv6.ArgDataCenterId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
		...
		if properties, ok := oldLan.GetPropertiesOk(); ok && properties != nil {
			if ipfailovers, ok := properties.GetIpFailoverOk(); ok && ipfailovers != nil {
				_, resp, err := c.CloudApiV6Services.Lans().Update(
					dcId, lanId, removeIpFailoverInfo(c, ipfailovers), queryParams,
				)
				if resp != nil && printer.GetId(resp) != "" {
					c.Printer.Verbose(constants.MessageRequestInfo, printer.GetId(resp), resp.RequestTime)
				}
	...
	if c != nil {
		...
		if ips != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				defaultIpFailoverCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
