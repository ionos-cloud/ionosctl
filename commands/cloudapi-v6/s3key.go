	...
	_ = get.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.S3KeyIds(
				os.Stderr, viper.GetString(core.GetFlagName(get.NS, cloudapiv6.ArgUserId)),
...
* S3Key Active`,
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.S3KeyIds(
				os.Stderr, viper.GetString(core.GetFlagName(update.NS, cloudapiv6.ArgUserId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	update.AddBoolFlag(
	...
	*/
	...
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.S3KeyIds(
				os.Stderr, viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv6.ArgUserId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	if c != nil {
		...
		if s != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				defaultS3KeyCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
