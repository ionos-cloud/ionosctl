...
var (
	defaultUserCols = []string{
		"UserId", "Firstname", "Lastname", "Email", "S3CanonicalUserId", "Administrator", "ForceSecAuth",
		"SecAuthActive", "Active",
	}
)
...
func UserCmd() *core.Command {
	...
	*/
	list := core.NewCommand(
		ctx, userCmd, core.CommandBuilder{
			Namespace:  "user",
			Resource:   "user",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
...
* Password`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgEmail, cloudapiv6.ArgEmailShort, "", "The email for the User", core.RequiredFlagOption(),
	)
	create.AddStringFlag(
		cloudapiv6.ArgPassword, cloudapiv6.ArgPasswordShort, "",
		"The password for the User (must be at least 5 characters long)", core.RequiredFlagOption(),
	)
	create.AddBoolFlag(cloudapiv6.ArgAdmin, "", false, "Assigns the User to have administrative rights")
	create.AddBoolFlag(
		cloudapiv6.ArgForceSecAuth, "", false,
...
* User Id`,
	...
	update.AddStringFlag(
		cloudapiv6.ArgPassword, cloudapiv6.ArgPasswordShort, "",
		"The password for the User (must be at least 5 characters long)",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgAdmin, "", false,
		"Assigns the User to have administrative rights. E.g.: --admin=true, --admin=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgForceSecAuth, "", false,
		"Indicates if secure (two-factor) authentication should be forced for the User. E.g.: --force-secure-auth=true, --force-secure-auth=false",
	)
...
func PreRunUserNameEmailPwd(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgFirstName, cloudapiv6.ArgLastName, cloudapiv6.ArgEmail, cloudapiv6.ArgPassword,
	)
}
...
func RunUserList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.User, users.Users,
...
func RunUserGet(c *core.CommandConfig) error {
	...
	u, resp, err := c.CloudApiV6Services.Users().Get(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgUserId)), queryParams,
	)
	if resp != nil {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.User, u.User, tabheaders.GetHeadersAllDefault(defaultUserCols, cols),
	)
	if err != nil {
		return err
	}
...
func RunUserUpdate(c *core.CommandConfig) error {
	...
	queryParams := listQueryParams.QueryParams
	oldUser, resp, err := c.CloudApiV6Services.Users().Get(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgUserId,
			),
		), queryParams,
	)
	if err != nil {
	...
	userUpd, resp, err := c.CloudApiV6Services.Users().Update(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgUserId,
			),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.User, userUpd.User, tabheaders.GetHeadersAllDefault(defaultUserCols, cols),
	)
	if err != nil {
		return err
	}
...
func RunUserDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete user", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting User with id: %v...", userId),
	)
...
func getUserInfo(oldUser *resources.User, c *core.CommandConfig) *resources.UserPut {
	...
	if properties, ok := oldUser.GetPropertiesOk(); ok && properties != nil {
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgFirstName)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property FirstName set: %v", firstName),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgLastName)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property LastName set: %v", lastName),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgEmail)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Email set: %v", email),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgPassword)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property Password set: %v", password),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgForceSecAuth)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property ForceSecAuth set: %v", forceSecureAuth),
			)
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgAdmin)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Property Administrator set: %v", admin),
			)
...
func DeleteAllUsers(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Users", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	for _, user := range *usersItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting User with id: %v...", *id),
		)
...
func GroupUserCmd() *core.Command {
	...
	/*
		List Users Command
	*/
	listUsers := core.NewCommand(
		ctx, groupUserCmd, core.CommandBuilder{
			Namespace:  "group",
			Resource:   "user",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
	...
	_ = listUsers.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return defaultUserCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	*/
	...
	_ = addUser.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return defaultUserCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	*/
	...
	removeUser.AddBoolFlag(constants.ArgForce, constants.ArgForceShort, false, constants.DescForce)
	removeUser.AddStringSliceFlag(constants.ArgCols, "", defaultUserCols, tabheaders.ColsMessage(defaultUserCols))
	_ = removeUser.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return defaultUserCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	removeUser.AddUUIDFlag(
		cloudapiv6.ArgUserId, cloudapiv6.ArgIdShort, "", cloudapiv6.UserId, core.RequiredFlagOption(),
	)
	_ = removeUser.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgUserId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.GroupUsersIds(
				viper.GetString(
					core.GetFlagName(
						removeUser.NS, cloudapiv6.ArgGroupId,
					),
				),
...
func RunGroupUserList(c *core.CommandConfig) error {
	...
	users, resp, err := c.CloudApiV6Services.Groups().ListUsers(
		viper.GetString(
			core.GetFlagName(
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.User, users.GroupMembers,
		tabheaders.GetHeadersAllDefault(defaultUserCols, cols),
	)
	if err != nil {
		return err
...
func RunGroupUserAdd(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("User with id: %v is adding to group with id: %v...", id, groupId),
	)
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.User, userAdded.User, tabheaders.GetHeadersAllDefault(defaultUserCols, cols),
	)
	if err != nil {
		return err
	}
...
func RunGroupUserRemove(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "remove user from group",
...
func RemoveAllUsers(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "remove all the Users", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	for _, user := range *usersItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting removing User with id: %v...", *id),
		)
