...
func RunDataCenterLabelsList(c *core.CommandConfig) error {
	...
	labelDcs, resp, err := c.CloudApiV6Services.Labels().DatacenterList(
		listQueryParams, viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgDataCenterId)),
	)
	if resp != nil {
		fmt.Fprintf(
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Label, labelDcs.LabelResources,
...
func RunDataCenterLabelGet(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Getting label with key: %v for Datacenter with id: %v...", labelKey, dcId),
	)
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.Label, labelDc.LabelResource,
		tabheaders.GetHeadersAllDefault(defaultLabelCols, cols),
	)
	if err != nil {
		return err
...
func RunDataCenterLabelRemove(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Removing label with key: %v for Datacenter with id: %v...", labelKey, dcId),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Datacenter Label successfully deleted"),
	)
	return nil
}
...
func RemoveAllDatacenterLabels(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Getting Labels from Datacenter..."),
	)
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Labels to be removed from Datacenter with ID: %v", dcId),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Labels from Datacenter with Id: "+dcId,
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Labels from Datacenter with Id: %v...", dcId),
	)
	...
	for _, label := range *labelsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Label with id: %v...", *key),
		)
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateLogOutput(constants.MessageDeletingAll, c.Resource, *key),
		)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Datacenter Labels successfully deleted"),
	)
	return nil
}
...
func RunServerLabelGet(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Getting label with key: %v for Server with id: %v...", labelkey, serverId),
	)
...
func RemoveAllServerLabels(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Labels to be removed from Server with Id: %v", serverId),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Labels from Server with Id: "+serverId,
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Labels from Server with Id: %v...", serverId),
	)
...
func RunVolumeLabelGet(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Getting label with key: %v for Volume with id: %v...", labelKey, volumeId),
	)
...
func RemoveAllVolumeLabels(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Labels to be removed from Volume with Id: %v", volumeId),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Labels from Volume with Id: "+volumeId,
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Labels from Volume with Id: %v...", volumeId),
	)
...
func RunIpBlockLabelsList(c *core.CommandConfig) error {
	...
	labelDcs, resp, err := c.CloudApiV6Services.Labels().IpBlockList(
		listQueryParams, viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgIpBlockId)),
	)
	if resp != nil {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
...
func RunIpBlockLabelRemove(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Removing label with key: %v for IpBlock with id: %v...", labelKey, ipBlockId,
		),
	)
...
func RemoveAllIpBlockLabels(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Labels to be removed from IpBlock with Id: %v", ipBlockId),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Labels from IpBlock with Id: "+ipBlockId,
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Labels from IpBlock with Id: %v...", ipBlockId),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("IP Block Labels successfully deleted"),
	)
	return nil
}
...
func RunSnapshotLabelsList(c *core.CommandConfig) error {
	...
	labelDcs, resp, err := c.CloudApiV6Services.Labels().SnapshotList(
		listQueryParams, viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgSnapshotId)),
	)
	if resp != nil {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
...
func RemoveAllSnapshotLabels(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Labels to be removed from Snapshot with Id: %v", snapshotId),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Labels from Snapshot with Id: "+snapshotId,
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Labels from Snapshot with Id: %v...", snapshotId),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Snapshot Labels successfully deleted"),
	)
	return nil
}
