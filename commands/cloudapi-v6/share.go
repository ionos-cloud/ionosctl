	...
	_ = get.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.GroupResourcesIds(
				os.Stderr, viper.GetString(core.GetFlagName(get.NS, cloudapiv6.ArgGroupId)),
...
* Resource Id`,
	...
	_ = update.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.GroupResourcesIds(
				os.Stderr, viper.GetString(core.GetFlagName(update.NS, cloudapiv6.ArgGroupId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	update.AddBoolFlag(
...
* Group Id`,
	...
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.GroupResourcesIds(
				os.Stderr, viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv6.ArgGroupId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	if groupSharesItems, ok := groupShares.GetItemsOk(); ok && groupSharesItems != nil {
		if len(*groupSharesItems) > 0 {
			...
			for _, share := range *groupSharesItems {
				if id, ok := share.GetIdOk(); ok && id != nil {
					c.Printer.Verbose(
						"Starting deleting Share with Resource ID: %v from Group with ID: %v...",
						*id, groupId,
					)
	...
	if c != nil {
		...
		if groups != nil {
			...
			r.Columns = printer.GetHeadersListAll(
				allGroupShareCols, defaultGroupShareCols, "GroupId",
				viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv6.ArgCols)),
				viper.GetBool(core.GetFlagName(c.NS, cloudapiv6.ArgAll)),
			)
		}
	}
	return r
