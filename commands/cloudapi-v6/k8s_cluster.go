...
var (
	...
	allK8sClusterCols     = []string{
		"ClusterId", "Name", "K8sVersion", "State", "MaintenanceWindow", "AvailableUpgradeVersions",
		"ViableNodePoolVersions", "S3Bucket", "ApiSubnetAllowList",
	}
)
...
func K8sClusterCmd() *core.Command {
	...
	*/
	...
	get.AddUUIDFlag(
		constants.FlagClusterId, cloudapiv6.ArgIdShort, "", cloudapiv6.K8sClusterId, core.RequiredFlagOption(),
	)
	...
	get.AddBoolFlag(
		...
		"Wait for specified Cluster to be in ACTIVE state",
	)
	get.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.K8sTimeoutSeconds,
		"Timeout option for waiting for Cluster to be in ACTIVE state [seconds]",
	)
	get.AddInt32Flag(
...
You can wait for the Cluster to be in "ACTIVE" state using ` + "`" + `--wait-for-state` + "`" + ` flag together with ` + "`" + `--timeout` + "`" + ` option.`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgK8sVersion, "", "", "The K8s version for the Cluster. If not set, the default one will be used",
	)
	...
	create.AddStringSliceFlag(
		cloudapiv6.ArgApiSubnets, "", []string{""},
	...
	create.AddBoolFlag(
		...
		"Wait for the Request for Cluster creation to be executed",
	)
	create.AddBoolFlag(
		constants.ArgWaitForState, constants.ArgWaitForStateShort, constants.DefaultWait,
		"Wait for the new Cluster to be in ACTIVE state",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.K8sTimeoutSeconds,
		"Timeout option for waiting for Cluster/Request [seconds]",
	)
	create.AddInt32Flag(
...
* K8s Cluster Id`,
	...
	update.AddStringFlag(
		cloudapiv6.ArgS3Bucket, "", "", "S3 Bucket name configured for K8s usage. It will overwrite the previous value",
	)
	...
	update.AddBoolFlag(
		constants.ArgWaitForState, constants.ArgWaitForStateShort, constants.DefaultWait,
		"Wait for specified Cluster to be in ACTIVE state after updating",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.K8sTimeoutSeconds,
		"Timeout option for waiting for Cluster to be in ACTIVE state after updating [seconds]",
	)
	update.AddInt32Flag(
...
* K8s Cluster Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Cluster deletion to be executed",
	)
	...
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.K8sTimeoutSeconds,
		"Timeout option for waiting for Request [seconds]",
	)
...
func RunK8sClusterList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		k8ss.KubernetesClusters, k8ssConverted,
...
func RunK8sClusterGet(c *core.CommandConfig) error {
	...
	if err := waitfor.WaitForState(
		c, waiter.K8sClusterStateInterrogator, viper.GetString(core.GetFlagName(c.NS, constants.FlagClusterId)),
	); err != nil {
		return err
	}
	...
	u, resp, err := c.CloudApiV6Services.K8s().GetCluster(
		viper.GetString(
			core.GetFlagName(
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		u.KubernetesCluster, uConverted,
		tabheaders.GetHeaders(allK8sClusterCols, defaultK8sClusterCols, cols),
	)
	if err != nil {
		return err
...
func RunK8sClusterUpdate(c *core.CommandConfig) error {
	...
	queryParams := listQueryParams.QueryParams
	oldCluster, _, err := c.CloudApiV6Services.K8s().GetCluster(
		viper.GetString(
			core.GetFlagName(
				c.NS, constants.FlagClusterId,
			),
	...
	k8sUpd, resp, err := c.CloudApiV6Services.K8s().UpdateCluster(
		viper.GetString(
			core.GetFlagName(
				c.NS, constants.FlagClusterId,
			),
		), newCluster, queryParams,
	)
	if resp != nil && request.GetId(resp) != "" {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
	...
	if viper.GetBool(core.GetFlagName(c.NS, constants.ArgWaitForState)) {
		...
		if k8sUpd, _, err = c.CloudApiV6Services.K8s().GetCluster(
			viper.GetString(
				core.GetFlagName(
					c.NS, constants.FlagClusterId,
				),
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		k8sUpd.KubernetesCluster, k8sUpdConverted,
		tabheaders.GetHeaders(allK8sClusterCols, defaultK8sClusterCols, cols),
	)
	if err != nil {
		return err
...
func RunK8sClusterDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete k8s cluster", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting K8s cluster with id: %v...", k8sClusterId),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Kubernetes Cluster successfully deleted"),
	)
	return nil
}
...
func getNewK8sCluster(c *core.CommandConfig) (*resources.K8sClusterForPost, error) {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sVersion)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property K8sVersion set: %v", k8sversion),
		)
	} else {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgS3Bucket)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property S3Buckets set: %v", s3buckets),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgApiSubnets)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Property ApiSubnetAllowList set: %v",
				viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv6.ArgApiSubnets)),
			),
		)
	}
...
func getK8sClusterInfo(oldUser *resources.K8sCluster, c *core.CommandConfig) resources.K8sClusterForPut {
	...
	if properties, ok := oldUser.GetPropertiesOk(); ok && properties != nil {
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgName)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Name set: %v", n),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sVersion)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property K8sVersion set: %v", v),
			)
		} else {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgApiSubnets)) {
			propertiesUpdated.SetApiSubnetAllowList(
				viper.GetStringSlice(
					core.GetFlagName(
						c.NS, cloudapiv6.ArgApiSubnets,
					),
				),
			)
...
func DeleteAllK8sClusters(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the K8sClusters",
	...
	for _, k8sCluster := range *k8sClustersItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting K8sCluster with id: %v...", *id),
		)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Kubernetes Clusters successfully deleted"),
	)
	return nil
}
...
func getMaintenanceInfo(
	c *core.CommandConfig, maintenance *resources.K8sMaintenanceWindow,
) resources.K8sMaintenanceWindow {
	var day, time string
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sMaintenanceDay)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property DayOfTheWeek of MaintenanceWindow set: %v", day),
		)
	} else {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgK8sMaintenanceTime)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property Time of MaintenanceWindow set: %v", time),
		)
	} else {
