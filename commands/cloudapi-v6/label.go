...
func LabelCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultLabelResourceCols, printer.ColsMessage(defaultLabelResourceCols),
	)
	_ = viper.BindPFlag(core.GetFlagName(labelCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols))
	_ = labelCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	var (
		allowedValues = []string{
			cloudapiv6.DatacenterResource, cloudapiv6.VolumeResource, cloudapiv6.ServerResource,
			cloudapiv6.SnapshotResource, cloudapiv6.IpBlockResource,
		}
	)
	...
	*/
	list := core.NewCommand(
		ctx, labelCmd, core.CommandBuilder{
			Namespace:  "label",
			Resource:   "label",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
	...
	list.AddSetFlag(
		cloudapiv6.ArgResourceType, "", "", allowedValues, "Type of resource to list labels from",
	...
	*/
	...
	get.AddSetFlag(
		cloudapiv6.ArgResourceType, "", "", allowedValues, "Type of resource to get labels from",
		core.RequiredFlagOption(),
	)
	...
	*/
	...
	getByUrn.AddStringFlag(
		cloudapiv6.ArgLabelUrn, "", "", "URN for the Label [urn:label:<resource_type>:<resource_uuid>:<key>]",
		core.RequiredFlagOption(),
	)
	getByUrn.AddBoolFlag(constants.ArgNoHeaders, "", false, cloudapiv6.ArgNoHeadersDescription)
	getByUrn.AddInt32Flag(
		cloudapiv6.ArgDepth, cloudapiv6.ArgDepthShort, cloudapiv6.DefaultGetDepth, cloudapiv6.ArgDepthDescription,
	)
	...
	*/
	...
	_ = addLabel.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgDataCenterId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.DataCentersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = addLabel.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				os.Stderr, viper.GetString(core.GetFlagName(addLabel.NS, cloudapiv6.ArgDataCenterId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	addLabel.AddSetFlag(
		cloudapiv6.ArgResourceType, "", "", allowedValues, "Type of resource to add labels to",
		core.RequiredFlagOption(),
	)
	addLabel.AddInt32Flag(
	...
	*/
	...
	_ = removeLabel.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgDataCenterId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.DataCentersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = removeLabel.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				os.Stderr, viper.GetString(core.GetFlagName(removeLabel.NS, cloudapiv6.ArgDataCenterId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	removeLabel.AddSetFlag(
		cloudapiv6.ArgResourceType, "", "", allowedValues, "Type of resource to remove labels from",
		core.RequiredFlagOption(),
	)
	removeLabel.AddBoolFlag(cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Remove all Labels")
	removeLabel.AddInt32Flag(
		cloudapiv6.ArgDepth, cloudapiv6.ArgDepthShort, cloudapiv6.DefaultMiscDepth, cloudapiv6.ArgDepthDescription,
	)
	...
	return []core.FlagNameSetWithPredicate{
		{
			...
			FlagNameSet: append(
				[]string{
					cloudapiv6.ArgResourceType, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgVolumeId,
				}, extraFlags...,
			),
			Predicate:      funcResourceTypeSetAndMatches,
			...
			FlagNameSet: append(
				[]string{
					cloudapiv6.ArgResourceType, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgServerId,
				}, extraFlags...,
			),
			Predicate:      funcResourceTypeSetAndMatches,
...
func PreRunResourceTypeLabelKeyRemove(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlagsSetsIfPredicate(
		c.Command, c.NS,
		append(
...
func PreRunResourceTypeLabelKeyValue(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlagsSetsIfPredicate(
		c.Command, c.NS, generateFlagSets(c, cloudapiv6.ArgLabelKey, cloudapiv6.ArgLabelValue)...,
	)
}
