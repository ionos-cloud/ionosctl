...
func LabelCmd() *core.Command {
	...
	_ = addLabel.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				viper.GetString(
					core.GetFlagName(
						addLabel.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	*/
	...
	removeLabel.AddBoolFlag(constants.ArgForce, constants.ArgForceShort, false, constants.DescForce)
	...
	_ = removeLabel.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				viper.GetString(
					core.GetFlagName(
						removeLabel.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
...
func RunLabelList(c *core.CommandConfig) error {
	...
	default:
		...
		out, err = jsontabwriter.GenerateOutput(
			"items", jsonpaths.Label, labelDcs.Labels,
...
func RunLabelRemove(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateVerboseOutput("Label is removing from %v...", resourceType),
	)
