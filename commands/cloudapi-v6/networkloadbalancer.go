...
var (
	defaultNetworkLoadBalancerCols = []string{
		"NetworkLoadBalancerId", "Name", "ListenerLan", "Ips", "TargetLan", "LbPrivateIps", "State",
	}
	allNetworkLoadBalancerCols = []string{
		"NetworkLoadBalancerId", "Name", "ListenerLan", "Ips", "TargetLan", "LbPrivateIps", "State", "DatacenterId",
	}
)
...
func NetworkloadbalancerCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultNetworkLoadBalancerCols, tabheaders.ColsMessage(defaultNetworkLoadBalancerCols),
	)
	_ = viper.BindPFlag(
		core.GetFlagName(networkloadbalancerCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols),
	)
	_ = networkloadbalancerCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, networkloadbalancerCmd, core.CommandBuilder{
	...
	*/
	...
	get.AddUUIDFlag(
		cloudapiv6.ArgNetworkLoadBalancerId, cloudapiv6.ArgIdShort, "", cloudapiv6.NetworkLoadBalancerId,
	...
	get.AddBoolFlag(
		...
		"Wait for specified Network Load Balancer to be in AVAILABLE state",
	)
	get.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.NlbTimeoutSeconds,
		"Timeout option for waiting for Network Load Balancer to be in AVAILABLE state [seconds]",
	)
	get.AddInt32Flag(
...
* Data Center Id`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Network Load Balancer", "Name of the Network Load Balancer",
	)
	...
	create.AddBoolFlag(
		...
		"Wait for the Request for Network Load Balancer creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.NlbTimeoutSeconds,
		"Timeout option for Request for Network Load Balancer creation [seconds]",
	)
	create.AddInt32Flag(
...
* Network Load Balancer Id`,
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Network Load Balancer update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.NlbTimeoutSeconds,
		"Timeout option for Request for Network Load Balancer update [seconds]",
	)
	update.AddInt32Flag(
...
* Network Load Balancer Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Network Load Balancer deletion to be executed",
	)
	...
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, cloudapiv6.NlbTimeoutSeconds,
		"Timeout option for Request for Network Load Balancer deletion [seconds]",
	)
...
func RunNetworkLoadBalancerListAll(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		allNetworkLoadBalancers, allNetworkLoadBalancersConverted,
...
func RunNetworkLoadBalancerList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.NetworkLoadBalancer, networkloadbalancers.NetworkLoadBalancers,
...
func RunNetworkLoadBalancerGet(c *core.CommandConfig) error {
	...
	if err := waitfor.WaitForState(
		c, waiter.NetworkLoadBalancerStateInterrogator,
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Network Load Balancer with id: %v is getting...",
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgNetworkLoadBalancerId)),
		),
	)
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.NetworkLoadBalancer, ng.NetworkLoadBalancer,
		tabheaders.GetHeaders(allNetworkLoadBalancerCols, defaultNetworkLoadBalancerCols, cols),
	)
	if err != nil {
		return err
...
func RunNetworkLoadBalancerDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete network load balancer",
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Network Load Balancer with id: %v...", nlbId),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Network Load Balancer successfully deleted"),
	)
	return nil
}
...
func getNewNetworkLoadBalancerInfo(c *core.CommandConfig) *resources.NetworkLoadBalancerProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgListenerLan)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property ListenerLan set: %v", listenerLan),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgTargetLan)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property TargetLan set: %v", targetLan),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgPrivateIps)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property PrivateIps set: %v", privateIps),
		)
	}
...
func DeleteAllNetworkLoadBalancers(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Getting Network Load Balancers..."),
	)

	networkLoadBalancers, resp, err := c.CloudApiV6Services.NetworkLoadBalancers().List(
		dcId, cloudapiv6.ParentResourceListQueryParams,
	)
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateLogOutput("Network Load Balancers to be deleted:"),
	)
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Network Load Balancers",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Network Load Balancers..."),
	)
	...
	for _, networkLoadBalancer := range *nlbItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Network Load Balancer with id: %v...", *id),
		)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Network Load Balancers successfully deleted"),
	)
	return nil
}
