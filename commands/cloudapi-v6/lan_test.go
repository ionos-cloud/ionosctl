...
func TestGetLansCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("lan", constants.ArgCols), []string{"Name"})
	getLansCols(core.GetGlobalFlagName("lan", constants.ArgCols), core.GetFlagName("lan", cloudapiv6.ArgAll), w)
	err := w.Flush()
	assert.NoError(t, err)
}
