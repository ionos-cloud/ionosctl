...
var (
	...
	testRequests = resources.Requests{
		Requests: ionoscloud.Requests{
			Id: &testRequestVar,
			Items: &[]ionoscloud.Request{
				rq, testRequestUpdated, testRequestUpdatedPatch, testRequestDeleted, testRequestCreated,
			},
		},
	}
...
func TestRunRequestList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Request.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				testRequests, &testResponse, nil,
			)
...
func TestRunRequestListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Request.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				resources.Requests{}, &testResponse, nil,
			)
			err := RunRequestList(cfg)
			assert.NoError(t, err)
		},
...
func TestRunRequestListErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(constants.ArgQuiet, false)
			rm.CloudApiV6Mocks.Request.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				testRequests, nil, testRequestErr,
			)
			err := RunRequestList(cfg)
			assert.Error(t, err)
...
func TestRunRequestGet(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Request.EXPECT().Get(
				testRequestVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&req, &testResponse, nil)
