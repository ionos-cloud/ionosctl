...
func BackupunitCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultBackupUnitCols, tabheaders.ColsMessage(defaultBackupUnitCols),
	)
	_ = viper.BindPFlag(
		core.GetFlagName(backupUnitCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols),
	)
	_ = backupUnitCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, backupUnitCmd, core.CommandBuilder{
	...
	*/
	...
	get.AddUUIDFlag(
		cloudapiv6.ArgBackupUnitId, cloudapiv6.ArgIdShort, "", cloudapiv6.BackupUnitId, core.RequiredFlagOption(),
	)
	...
	*/
	...
	_ = getsso.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgBackupUnitId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.BackupUnitsIds(), cobra.ShellCompDirectiveNoFileComp
		},
...
* Password`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "", "Alphanumeric name you want to assign to the BackupUnit",
		core.RequiredFlagOption(),
	)
	create.AddStringFlag(
		cloudapiv6.ArgEmail, cloudapiv6.ArgEmailShort, "", "The e-mail address you want to assign to the BackupUnit",
		core.RequiredFlagOption(),
	)
	create.AddStringFlag(
		cloudapiv6.ArgPassword, cloudapiv6.ArgPasswordShort, "",
		"Alphanumeric password you want to assign to the BackupUnit", core.RequiredFlagOption(),
	)
	create.AddBoolFlag(
	...
	create.AddIntFlag(
		...
		"Timeout option for Request for BackupUnit creation [seconds]",
	)
	create.AddInt32Flag(
...
* BackupUnit Id`,
	...
	update.AddStringFlag(
		cloudapiv6.ArgPassword, cloudapiv6.ArgPasswordShort, "",
		"Alphanumeric password you want to update for the BackupUnit",
	)
	update.AddStringFlag(
		cloudapiv6.ArgEmail, cloudapiv6.ArgEmailShort, "", "The e-mail address you want to update for the BackupUnit",
	)
	update.AddUUIDFlag(
		cloudapiv6.ArgBackupUnitId, cloudapiv6.ArgIdShort, "", cloudapiv6.BackupUnitId, core.RequiredFlagOption(),
	)
	...
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for BackupUnit update [seconds]",
	)
	update.AddInt32Flag(
...
* BackupUnit Id`,
	...
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for BackupUnit deletion [seconds]",
	)
...
func RunBackupUnitList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.BackupUnit, backupUnits.BackupUnits,
...
func RunBackupUnitGet(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Backup unit with id: %v is getting... ",
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgBackupUnitId)),
		),
	)

	u, resp, err := c.CloudApiV6Services.BackupUnit().Get(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgBackupUnitId,
			),
		), queryParams,
	)
	if resp != nil {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.BackupUnit, u.BackupUnit,
		tabheaders.GetHeadersAllDefault(defaultBackupUnitCols, cols),
	)
	if err != nil {
		return err
...
func RunBackupUnitGetSsoUrl(c *core.CommandConfig) error {
	...
	u, resp, err := c.CloudApiV6Services.BackupUnit().GetSsoUrl(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgBackupUnitId,
			),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.BackupUnitSSOUrl, u.BackupUnitSSO,
...
func RunBackupUnitUpdate(c *core.CommandConfig) error {
	...
	backupUnitUpd, resp, err := c.CloudApiV6Services.BackupUnit().Update(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgBackupUnitId,
			),
		), *newProperties, queryParams,
	)
	if resp != nil && request.GetId(resp) != "" {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.BackupUnit, backupUnitUpd.BackupUnit,
		tabheaders.GetHeadersAllDefault(defaultBackupUnitCols, cols),
	)
...
func RunBackupUnitDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete backup unit", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Backup unit with id: %v...", backupunitId),
	)
...
func DeleteAllBackupUnits(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Backup Units",
	...
	for _, backupUnit := range *backupUnitsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Backup unit with id: %v...", *id),
		)
