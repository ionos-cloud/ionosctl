...
func K8sNodePoolLanCmd() *core.Command {
	...
	*/
	...
	_ = list.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.K8sNodePoolsIds(
				viper.GetString(
					core.GetFlagName(
						list.NS, constants.FlagClusterId,
					),
				),
	...
	list.AddStringSliceFlag(
		constants.ArgCols, "", defaultK8sNodePoolLanCols, tabheaders.ColsMessage(defaultK8sNodePoolLanCols),
	)
	_ = list.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
* Lan Id`,
	...
	_ = add.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.K8sNodePoolsIds(
				viper.GetString(
					core.GetFlagName(
						add.NS, constants.FlagClusterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	add.AddStringSliceFlag(
		cloudapiv6.ArgCols, "", defaultK8sNodePoolLanCols, tabheaders.ColsMessage(defaultK8sNodePoolLanCols),
	)
	_ = add.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	_ = removeCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.K8sNodePoolsIds(
				viper.GetString(
					core.GetFlagName(
						removeCmd.NS, constants.FlagClusterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
...
func RunK8sNodePoolLanList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.K8sNodePoolLan, lans,
...
func RunK8sNodePoolLanAdd(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.K8sNodePoolLan, getK8sNodePoolLansForPut(ngNew),
		tabheaders.GetHeadersAllDefault(defaultK8sNodePoolLanCols, cols),
	)
	if err != nil {
		return err
...
func RunK8sNodePoolLanRemove(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "remove node pool lan", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateLogOutput("Kubernetes Node Pool Lan successfully deleted"),
	)
	return nil
}
...
func RemoveAllK8sNodePoolsLans(c *core.CommandConfig) error {
	...
	for _, lan := range *lans {
		if id, ok := lan.GetIdOk(); ok && id != nil {
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateLogOutput("K8s NodePool Lan Id: "+string(*id)),
			)
		}
	}

	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "remove all the K8sNodePool Lans",
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Removing all the K8sNodePool Lans..."),
	)
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(),
		jsontabwriter.GenerateLogOutput("Kubernetes Node Pool Lans successfully deleted"),
	)
	return nil
}
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgLanId)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
					"Adding a Kubernetes NodePool LAN with id: %v and dhcp: %v", lanId, dhcp,
				),
			)
			...
			if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgNetwork)) {
				...
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateVerboseOutput("Property Network set: %v", network),
				)
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateVerboseOutput("Property GatewayIp set: %v", gatewayIp),
				)
...
func removeK8sNodePoolLanInfo(c *core.CommandConfig, oldNg *resources.K8sNodePool) resources.K8sNodePoolForPut {
	...
	if properties, ok := oldNg.GetPropertiesOk(); ok && properties != nil {
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgLanId)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateVerboseOutput("Removing a Kubernetes NodePool LAN with id: %v", lanId),
			)
