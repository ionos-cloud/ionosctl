...
func TestRunLocationList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Location.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				locations, &testResponse, nil,
			)
...
func TestRunLocationListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Location.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				resources.Locations{}, &testResponse, nil,
			)
			err := RunLocationList(cfg)
			assert.NoError(t, err)
		},
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Location.EXPECT().GetByRegionAndLocationId(
				testIds[0], testIds[1], testQueryParamOther,
...
func TestRunLocationGetErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Location.EXPECT().GetByRegionAndLocationId(
				testIds[0], testIds[1], testQueryParamOther,
			).Return(&loc, nil, testLocationErr)
			err := RunLocationGet(cfg)
			assert.Error(t, err)
		},
	)
