...
func PccCmd() *core.Command {
	...
	_ = pccCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, pccCmd, core.CommandBuilder{
			Namespace:  "pcc",
			Resource:   "pcc",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
	...
	*/
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Unnamed PrivateCrossConnect", "The name for the Cross-Connect",
	)
	create.AddStringFlag(
		cloudapiv6.ArgDescription, cloudapiv6.ArgDescriptionShort, "", "The description for the Cross-Connect",
	)
	create.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Cross-Connect creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Cross-Connect creation [seconds]",
	)
	create.AddInt32Flag(
...
* Pcc Id`,
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Cross-Connect update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Cross-Connect update [seconds]",
	)
	update.AddInt32Flag(
...
* Pcc Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Cross-Connect deletion to be executed",
	)
	deleteCmd.AddBoolFlag(cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete all Cross-Connects.")
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Cross-Connect deletion [seconds]",
	)
...
func RunPccList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", allPccJSONPaths, pccs.PrivateCrossConnects,
...
func RunPccGet(c *core.CommandConfig) error {
	...
	u, resp, err := c.CloudApiV6Services.Pccs().Get(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgPccId)), queryParams,
	)
	if resp != nil {
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", allPccJSONPaths, u.PrivateCrossConnect,
		tabheaders.GetHeadersAllDefault(defaultPccCols, cols),
	)
	if err != nil {
		return err
...
func RunPccUpdate(c *core.CommandConfig) error {
	...
	queryParams := listQueryParams.QueryParams
	oldPcc, resp, err := c.CloudApiV6Services.Pccs().Get(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgPccId)), queryParams,
	)
	if err != nil {
		return err
	...
	pccUpd, resp, err := c.CloudApiV6Services.Pccs().Update(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgPccId,
			),
	...
	out, err := jsontabwriter.GenerateOutput(
		"", allPccJSONPaths, pccUpd.PrivateCrossConnect,
		tabheaders.GetHeadersAllDefault(defaultPccCols, cols),
	)
	if err != nil {
		return err
...
func RunPccDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete Cross-Connect", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
...
func getPccInfo(
	oldUser *resources.PrivateCrossConnect, c *core.CommandConfig,
) *resources.PrivateCrossConnectProperties {
	...
	if properties, ok := oldUser.GetPropertiesOk(); ok && properties != nil {
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgName)) {
			...
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Name set: %v", namePcc),
			)
		} else {
...
func DeleteAllPccs(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Cross-Connects",
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Deleting all the PrivateCrossConnects..."),
	)
...
func PeersCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultPccPeersCols,
	...
	_ = peerCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	/*
		List Command
	*/
	listPeers := core.NewCommand(
		ctx, peerCmd, core.CommandBuilder{
			Namespace:  "pcc",
			Resource:   "peers",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
	...
	_ = listPeers.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgPccId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.PccsIds(), cobra.ShellCompDirectiveNoFileComp
		},
...
func RunPccPeersList(c *core.CommandConfig) error {
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Getting Peers from Cross-Connect with ID: %v...",
			viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgPccId)),
		),
	)
