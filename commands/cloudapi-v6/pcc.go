...
func PccCmd() *core.Command {
	...
	*/
	...
	create.AddStringFlag(
		cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Unnamed PrivateCrossConnect",
		"The name for the Private Cross-Connect",
	)
	create.AddStringFlag(
		cloudapiv6.ArgDescription, cloudapiv6.ArgDescriptionShort, "", "The description for the Private Cross-Connect",
	)
	create.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Private Cross-Connect creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Private Cross-Connect creation [seconds]",
	)
	create.AddInt32Flag(
...
* Pcc Id`,
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Private Cross-Connect update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Private Cross-Connect update [seconds]",
	)
	update.AddInt32Flag(
...
* Pcc Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Private Cross-Connect deletion to be executed",
	)
	...
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Private Cross-Connect deletion [seconds]",
	)
...
func RunPccPeersList(c *core.CommandConfig) error {
	c.Printer.Verbose(
		"Getting Peers from Private Cross-Connect with ID: %v...",
	...
	if c != nil {
		...
		if pccs != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				defaultPccCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
...
func getPccPeerPrint(c *core.CommandConfig, pccs []resources.Peer) printer.Result {
	...
	if c != nil {
		if pccs != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				defaultPccPeersCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
