...
var (
	...
	allDatacenterCols     = []string{
		"DatacenterId", "Name", "Location", "State", "Description", "Version",
...
func DatacenterCmd() *core.Command {
	...
	_ = viper.BindPFlag(
		core.GetFlagName(datacenterCmd.Name(), constants.ArgCols), globalFlags.Lookup(constants.ArgCols),
	)
	_ = datacenterCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, datacenterCmd, core.CommandBuilder{
...
` + completer.DataCentersFiltersUsage(),
	...
	*/
	...
	get.AddUUIDFlag(
		cloudapiv6.ArgDataCenterId, cloudapiv6.ArgIdShort, "", cloudapiv6.DatacenterId, core.RequiredFlagOption(),
	)
...
You can wait for the Request to be executed using ` + "`" + `--wait-for-request` + "`" + ` option.`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgDescription, cloudapiv6.ArgDescriptionShort, "", "Description of the Data Center",
	)
	...
	create.AddBoolFlag(
		...
		"Wait for the Request for Data Center creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Data Center creation [seconds]",
	)
	create.AddInt32Flag(
...
* Data Center Id`,
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Data Center update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Data Center update [seconds]",
	)
	update.AddInt32Flag(
...
func RunDataCenterList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Datacenter, datacenters.Datacenters,
...
func RunDataCenterGet(c *core.CommandConfig) error {
	...
	dc, resp, err := c.CloudApiV6Services.DataCenters().Get(
		viper.GetString(
			core.GetFlagName(
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.Datacenter, dc.Datacenter,
		tabheaders.GetHeaders(allDatacenterCols, defaultDatacenterCols, cols),
	)
	if err != nil {
		return err
...
func RunDataCenterCreate(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Properties set for creating the datacenter: Name: %v, Description: %v, Location: %v", name, description,
			loc,
		),
	)
...
func RunDataCenterDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete data center", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Datacenter with ID: %v...", dcId),
	)
...
func DeleteAllDatacenters(c *core.CommandConfig) error {
	...
	for _, dc := range *datacentersItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Datacenter with id: %v...", *id),
		)
