...
func TargetGroupTargetCmd() *core.Command {
	...
	*/
	...
	list.AddStringSliceFlag(
		constants.ArgCols, "", defaultTargetGroupTargetCols, printer.ColsMessage(defaultTargetGroupTargetCols),
	)
	_ = list.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
var defaultTargetGroupTargetCols = []string{
...
func getTargetGroupTargetPrint(
	resp *resources.Response, c *core.CommandConfig, s []resources.TargetGroupTarget,
) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
		...
		if s != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				defaultTargetGroupTargetCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
