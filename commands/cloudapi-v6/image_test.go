...
func TestRunImageGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgVerbose, false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgImageId), testImageVar)
		rm.CloudApiV6Mocks.Image.EXPECT().Get(testImageVar, resources.QueryParams{}).Return(&testImage, &testResponse, nil)
		err := RunImageGet(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunImageGetErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgQuiet, false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgImageId), testImageVar)
		rm.CloudApiV6Mocks.Image.EXPECT().Get(testImageVar, resources.QueryParams{}).Return(&testImage, nil, testImageErr)
		err := RunImageGet(cfg)
		assert.Error(t, err)
	})
}
