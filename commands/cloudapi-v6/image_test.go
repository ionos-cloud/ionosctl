...
func TestRunImageList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Image.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				testImages, &testResponse, nil,
			)
...
func TestRunImageListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Image.EXPECT().List(gomock.AssignableToTypeOf(testListQueryParam)).Return(
				resources.Images{}, &testResponse, nil,
			)
			err := RunImageList(cfg)
			assert.NoError(t, err)
		},
...
func TestRunImageGet(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(constants.ArgVerbose, false)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgImageId), testImageVar)
			rm.CloudApiV6Mocks.Image.EXPECT().Get(
				testImageVar, gomock.AssignableToTypeOf(testQueryParamOther),
...
func TestRunImageGetErr(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.Image.EXPECT().Get(
				testImageVar, gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testImage, nil, testImageErr)
			err := RunImageGet(cfg)
			assert.Error(t, err)
		},
	)
