...
var (
	...
	allVolumeCols     = []string{
		"VolumeId", "Name", "Size", "Type", "LicenceType", "State", "Image", "Bus", "AvailabilityZone", "BackupunitId",
...
func VolumeCmd() *core.Command {
	...
	_ = volumeCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, volumeCmd, core.CommandBuilder{
			Namespace:  "volume",
			Resource:   "volume",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
...
* Data Center Id`,
	...
	create.AddStringFlag(
		cloudapiv6.ArgSize, cloudapiv6.ArgSizeShort, strconv.Itoa(cloudapiv6.DefaultVolumeSize),
		"The size of the Volume in GB. e.g.: --size 10 or --size 10GB. The maximum Volume size is determined by your contract limit",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgSize,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	create.AddStringFlag(
		...
		"Availability zone of the Volume. Storage zone can only be selected prior provisioning",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
	...
	create.AddUUIDFlag(
		cloudapiv6.ArgBackupUnitId, "", "",
		"The unique Id of the Backup Unit that User has access to. It is mandatory to provide either 'public image' or 'imageAlias' in conjunction with this property",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgBackupUnitId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	create.AddUUIDFlag(
		...
		"The Image Id or Snapshot Id to be used as template for the new Volume. A password or SSH Key need to be set",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgImageId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	create.AddStringFlag(
		...
		"The Image Alias to set instead of Image Id. A password or SSH Key need to be set",
	)
	create.AddStringFlag(
		cloudapiv6.ArgPassword, cloudapiv6.ArgPasswordShort, "",
		"Initial password to be set for installed OS. Works with public Images only. Not modifiable. Password rules allows all characters from a-z, A-Z, 0-9",
	)
	create.AddStringFlag(
		cloudapiv6.ArgUserData, "", "",
		"The cloud-init configuration for the Volume as base64 encoded string. It is mandatory to provide either 'public image' or 'imageAlias' that has cloud-init compatibility in conjunction with this property",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgCpuHotPlug, "", false,
		"It is capable of CPU hot plug (no reboot required). E.g.: --cpu-hot-plug=true, --cpu-hot-plug=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgRamHotPlug, "", false,
		"It is capable of memory hot plug (no reboot required). E.g.: --ram-hot-plug=true, --ram-hot-plug=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgNicHotPlug, "", false,
		"It is capable of nic hot plug (no reboot required). E.g.: --nic-hot-plug=true, --nic-hot-plug=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgNicHotUnplug, "", false,
		"It is capable of nic hot unplug (no reboot required). E.g.: --nic-hot-unplug=true, --nic-hot-unplug=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgDiscVirtioHotPlug, "", false,
		"It is capable of Virt-IO drive hot plug (no reboot required). E.g.: --disc-virtio-plug=true, --disc-virtio-plug=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgDiscVirtioHotUnplug, "", false,
		"It is capable of Virt-IO drive hot unplug (no reboot required). This works only for non-Windows virtual Machines. E.g.: --disc-virtio-unplug=true, --disc-virtio-unplug=false",
	)
	create.AddStringFlag(
		cloudapiv6.ArgSshKeyPaths, cloudapiv6.ArgSshKeyPathsShort, "", "Absolute paths of the SSH Keys for the Volume",
	)
	create.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Volume creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Volume creation [seconds]",
	)
	create.AddInt32Flag(
...
* Volume Id`,
	...
	update.AddUUIDFlag(
		cloudapiv6.ArgVolumeId, cloudapiv6.ArgIdShort, "", cloudapiv6.VolumeId, core.RequiredFlagOption(),
	)
	_ = update.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgVolumeId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	update.AddStringFlag(
		cloudapiv6.ArgSize, "", "",
		"The size of the Volume in GB. e.g. 10 or 10GB. The maximum volume size is determined by your contract limit",
	)
	_ = update.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgSize,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Volume update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Volume update [seconds]",
	)
	update.AddInt32Flag(
...
* Volume Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Volume deletion to be executed",
	)
	deleteCmd.AddBoolFlag(
		cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete all Volumes from a virtual Datacenter.",
	)
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Volume deletion [seconds]",
	)
...
func RunVolumeListAll(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutputPreconverted(
		allVolumes, allVolumesConverted,
...
func RunVolumeList(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Listing Volumes from Datacenter with ID: %v",
	...
	volumes, resp, err := c.CloudApiV6Services.Volumes().List(
		viper.GetString(
			core.GetFlagName(
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Volume, volumes.Volumes,
		tabheaders.GetHeaders(allVolumeCols, defaultVolumeCols, cols),
	)
	if err != nil {
		return err
...
func RunVolumeCreate(c *core.CommandConfig) error {
	...
	vol, resp, err := c.CloudApiV6Services.Volumes().Create(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgDataCenterId,
			),
...
func RunVolumeUpdate(c *core.CommandConfig) error {
	...
	vol, resp, err := c.CloudApiV6Services.Volumes().Update(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgDataCenterId)),
...
func RunVolumeDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete volume", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Volume with id: %v...", volumeId),
	)
...
func getNewVolume(c *core.CommandConfig) (*resources.Volume, error) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Size set: %vGB", float32(size)),
	)
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgBackupUnitId)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property BackupUnitId set: %v", backupUnitId),
		)
	}
	...
	if (!viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgImageId)) &&
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property LicenceType set: %v", licenceType),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgImageId)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property Image set: %v", imageId),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgImageAlias)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property ImageAlias set: %v", imageAlias),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgUserData)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput("Property UserData set: %v", userData),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCpuHotPlug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property CpuHotPlug set: %v", cpuHotPlug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgRamHotPlug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property RamHotPlug set: %v", ramHotPlug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgNicHotPlug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property NicHotPlug set: %v", nicHotPlug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgNicHotUnplug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property NicHotUnplug set: %v", nicHotUnplug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDiscVirtioHotPlug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property DiscVirtioHotPlug set: %v", discVirtioHotPlug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDiscVirtioHotUnplug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property DiscVirtioHotUnplug set: %v", discVirtioHotUnplug),
		)
	}
...
func getVolumeInfo(c *core.CommandConfig) (*resources.VolumeProperties, error) {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgSize)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property Size set: %vGB", float32(size)),
		)
	}
...
func DeleteAllVolumes(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Volumes",
	...
	for _, volume := range *volumesItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Volume with id: %v is...", *id),
		)
...
func ServerVolumeCmd() *core.Command {
	...
	*/
	attachVolume := core.NewCommand(
		ctx, serverVolumeCmd, core.CommandBuilder{
			Namespace: "server",
			Resource:  "volume",
			Verb:      "attach",
			Aliases:   []string{"a"},
...
* Volume Id`,
	...
	_ = attachVolume.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allVolumeCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = attachVolume.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.VolumesIds(
				viper.GetString(
					core.GetFlagName(
						attachVolume.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	attachVolume.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Volume attachment to be executed",
	)
	attachVolume.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Volume attachment [seconds]",
	)

	/*
		List Volumes Command
	*/
	listVolumes := core.NewCommand(
		ctx, serverVolumeCmd, core.CommandBuilder{
			Namespace:  "server",
			Resource:   "volume",
			Verb:       "list",
	...
	_ = listVolumes.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allVolumeCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = listVolumes.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				viper.GetString(
					core.GetFlagName(
						listVolumes.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	listVolumes.AddInt32Flag(
		constants.FlagMaxResults, constants.FlagMaxResultsShort, cloudapiv6.DefaultMaxResults, constants.DescMaxResults,
	)
	listVolumes.AddInt32Flag(
		cloudapiv6.ArgDepth, cloudapiv6.ArgDepthShort, cloudapiv6.DefaultListDepth, cloudapiv6.ArgDepthDescription,
	...
	listVolumes.AddStringSliceFlag(
		cloudapiv6.ArgFilters, cloudapiv6.ArgFiltersShort, []string{""}, cloudapiv6.ArgFiltersDescription,
	)
	_ = listVolumes.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgFilters,
	...
	*/
	...
	_ = getVolumeCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allVolumeCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = getVolumeCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				viper.GetString(
					core.GetFlagName(
						getVolumeCmd.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	getVolumeCmd.AddUUIDFlag(
		cloudapiv6.ArgVolumeId, cloudapiv6.ArgIdShort, "", cloudapiv6.VolumeId, core.RequiredFlagOption(),
	)
	_ = getVolumeCmd.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgVolumeId,
		func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.AttachedVolumesIds(
				viper.GetString(
					core.GetFlagName(
						getVolumeCmd.NS, cloudapiv6.ArgDataCenterId,
					),
				), viper.GetString(core.GetFlagName(getVolumeCmd.NS, cloudapiv6.ArgServerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	getVolumeCmd.AddInt32Flag(
...
* Volume Id`,
	...
	detachVolume.AddBoolFlag(constants.ArgForce, constants.ArgForceShort, false, constants.DescForce)
	detachVolume.AddStringSliceFlag(constants.ArgCols, "", defaultVolumeCols, tabheaders.ColsMessage(allVolumeCols))
	_ = detachVolume.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allVolumeCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = detachVolume.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.AttachedVolumesIds(
				viper.GetString(core.GetFlagName(detachVolume.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(detachVolume.NS, cloudapiv6.ArgServerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	_ = detachVolume.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				viper.GetString(
					core.GetFlagName(
						detachVolume.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	detachVolume.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Volume detachment to be executed",
	)
	detachVolume.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Volume detachment [seconds]",
	)
	detachVolume.AddBoolFlag(cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Detach all Volumes.")
	detachVolume.AddInt32Flag(
		cloudapiv6.ArgDepth, cloudapiv6.ArgDepthShort, cloudapiv6.DefaultMiscDepth, cloudapiv6.ArgDepthDescription,
	)
...
func PreRunDcServerVolumeIds(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgServerId, cloudapiv6.ArgVolumeId,
	)
}
...
func RunServerVolumeAttach(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.Volume, attachedVol.Volume,
		tabheaders.GetHeaders(allVolumeCols, defaultVolumeCols, cols),
	)
	if err != nil {
		return err
...
func RunServerVolumesList(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Listing attached Volumes from Server with ID: %v...", serverId),
	)
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Volume, attachedVols.AttachedVolumes,
		tabheaders.GetHeaders(allVolumeCols, defaultVolumeCols, cols),
	)
	if err != nil {
		return err
...
func RunServerVolumeDetach(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "detach volume from server",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
...
func DetachAllServerVolumes(c *core.CommandConfig) error {
	...
	volumes, resp, err := c.CloudApiV6Services.Servers().ListVolumes(
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "detach all the Volumes",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	for _, volume := range *volumesItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting detaching Volume with id: %v...", *id),
		)
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateLogOutput(constants.MessageRemovingAll, c.Resource, *id),
		)
