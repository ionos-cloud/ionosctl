...
func TestRunContractGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.Contract.EXPECT().Get(testQueryParamOther).Return(testContracts, &testResponse, nil)
...
func TestRunContractGetErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		viper.Set(config.ArgQuiet, false)
		rm.CloudApiV6Mocks.Contract.EXPECT().Get(testQueryParamOther).Return(testContracts, nil, testContractErr)
		err := RunContractGet(cfg)
		assert.Error(t, err)
	})
}
