...
func AlbRuleHttpRuleCmd() *core.Command {
	...
	globalFlags.StringSliceP(
		constants.ArgCols, "", defaultAlbRuleHttpRuleCols, tabheaders.ColsMessage(allAlbRuleHttpRuleCols),
	)
	_ = viper.BindPFlag(
...
func RunAlbRuleHttpRuleList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.ApplicationLoadBalancerHTTPRule, *httpRules,
...
func RunAlbRuleHttpRuleAdd(c *core.CommandConfig) error {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Adding the new HttpRule to the existing HttpRules"),
	)

	httpRuleItems = append(httpRuleItems, httpRuleNew.ApplicationLoadBalancerHttpRule)
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Updating ForwardingRule with the new HttpRules"),
	)
	...
	out, err := jsontabwriter.GenerateOutput(
		"", jsonpaths.ApplicationLoadBalancerHTTPRule, httpRuleNew.ApplicationLoadBalancerHttpRule,
		tabheaders.GetHeaders(allAlbRuleHttpRuleCols, defaultAlbRuleHttpRuleCols, cols),
	)
	if err != nil {
		return err
...
func RunAlbRuleHttpRuleRemove(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "remove forwarding rule http rule",
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Removing the HTTP Rule from the existing HTTP Rules"),
	)
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Updating ForwardingRule with the new HTTP Rules"),
	)
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateLogOutput("Application Load Balancer HTTP Rule successfully deleted"),
	)
...
func RemoveAllHTTPRules(c *core.CommandConfig) (*resources.Response, error) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateLogOutput("Forwarding Rule HTTP Rules to be deleted:"),
	)
	...
	for _, httpRuleOk := range *httpRulesOk {
		if nameOk, ok := httpRuleOk.GetNameOk(); ok && nameOk != nil {
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateLogOutput("Forwarding Rule HTTP Rule Name: %v", *nameOk),
			)
		}
		...
		if typeOk, ok := httpRuleOk.GetTypeOk(); ok && typeOk != nil {
			fmt.Fprintf(
				c.Command.Command.ErrOrStderr(),
				jsontabwriter.GenerateLogOutput("Forwarding Rule HTTP Rule Type: %v", *typeOk),
			)
		}
	}

	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Forwarding Rule HTTP Rules",
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Deleting all the Forwarding Rule HTTP Rules..."),
	)
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateLogOutput("Application Load Balancer HTTP Rules successfully deleted"),
	)
	return resp, err
}
...
func getRuleHttpRuleConditionInfo(c *core.CommandConfig) resources.ApplicationLoadBalancerHttpRuleCondition {
	...
	if !strings.EqualFold(viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgCondition)), "EXISTS") {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
				"Property Condition Value set: %v",
				viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgConditionValue)),
			),
		)
	}
...
func getRuleHttpRulesRemove(
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Getting Properties from the Forwarding Rule"),
	)
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Getting HTTP Rules from the Forwarding Rule Properties"),
	)
	...
	for _, httpRuleItem := range *httpRules {
		...
		if nameOk, ok := httpRuleItem.GetNameOk(); ok && nameOk != nil {
			if *nameOk == viper.GetString(core.GetFlagName(c.NS, cloudapiv6.ArgName)) {
				...
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateVerboseOutput("Found HTTP Rule with Name: %v", *nameOk),
				)
			}
		}
