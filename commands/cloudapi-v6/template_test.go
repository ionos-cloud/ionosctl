...
func TestRunTemplateList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.Template.EXPECT().List(testListQueryParam).Return(templates, &testResponse, nil)
...
func TestRunTemplateListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		viper.Set(config.ArgQuiet, false)
		rm.CloudApiV6Mocks.Template.EXPECT().List(testListQueryParam).Return(templates, nil, testTemplateErr)
		err := RunTemplateList(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunTemplateGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgVerbose, false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgTemplateId), testTemplateVar)
		rm.CloudApiV6Mocks.Template.EXPECT().Get(testTemplateVar, testQueryParamOther).Return(&tpl, &testResponse, nil)
...
func TestRunTemplateGetErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgQuiet, false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgTemplateId), testTemplateVar)
		rm.CloudApiV6Mocks.Template.EXPECT().Get(testTemplateVar, testQueryParamOther).Return(&tpl, nil, testTemplateErr)
		err := RunTemplateGet(cfg)
		assert.Error(t, err)
	})
}
