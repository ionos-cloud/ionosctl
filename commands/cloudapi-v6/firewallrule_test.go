...
func TestRunFirewallRuleList(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgServerId), testFirewallRuleVar)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgNicId), testFirewallRuleVar)
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().List(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testFirewallRules, &testResponse, nil)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().List(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testListQueryParam),
			).Return(resources.FirewallRules{}, &testResponse, nil)
			err := RunFirewallRuleList(cfg)
			assert.NoError(t, err)
		},
	)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().List(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testFirewallRules, nil, testFirewallRuleErr)
			err := RunFirewallRuleList(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunFirewallRuleGet(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgNicId), testFirewallRuleVar)
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgFirewallRuleId), testFirewallRuleVar)
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Get(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Get(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testRule, nil, testFirewallRuleErr)
			err := RunFirewallRuleGet(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunFirewallRuleCreate(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Create(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testInputFirewallRule,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Create(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testInputFirewallRule,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testInputFirewallRule, &testResponse, testFirewallRuleErr)
			err := RunFirewallRuleCreate(cfg)
			assert.Error(t, err)
		},
	)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Create(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testInputFirewallRule,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testInputFirewallRule, nil, testFirewallRuleErr)
			err := RunFirewallRuleCreate(cfg)
			assert.Error(t, err)
		},
	)
...
func TestRunFirewallRuleUpdate(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgDirection), testFirewallRuleType)
			viper.Set(core.GetFlagName(cfg.NS, constants.ArgWaitForRequest), false)
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Update(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				testInputFirewallRuleProperties, gomock.AssignableToTypeOf(testQueryParamOther),
			).
				Return(&testRule, &testResponse, nil)
...
func TestRunFirewallRuleDelete(t *testing.T) {
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgFirewallRuleId), testFirewallRuleVar)
			viper.Set(core.GetFlagName(cfg.NS, constants.ArgWaitForRequest), false)
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Delete(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().List(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testFirewallRulesList, &testResponse, nil)
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Delete(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, nil)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().List(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testListQueryParam),
			).Return(testFirewallRulesList, nil, testFirewallRuleErr)
			err := RunFirewallRuleDelete(cfg)
			assert.Error(t, err)
		},
	)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().List(
				...
				resources.FirewallRules{FirewallRules: ionoscloud.FirewallRules{Items: &[]ionoscloud.FirewallRule{}}},
				nil, nil,
			)
			err := RunFirewallRuleDelete(cfg)
			assert.Error(t, err)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Delete(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, testFirewallRuleErr)
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Delete(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(&testResponse, nil)
	...
	core.CmdConfigTest(
		t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
			...
			rm.CloudApiV6Mocks.FirewallRule.EXPECT().Delete(
				testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar, testFirewallRuleVar,
				gomock.AssignableToTypeOf(testQueryParamOther),
			).Return(nil, testFirewallRuleErr)
			err := RunFirewallRuleDelete(cfg)
			assert.Error(t, err)
		},
	)
