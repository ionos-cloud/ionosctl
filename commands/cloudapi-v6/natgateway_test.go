...
func TestRunNatGatewayList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(constants.ArgOutput, constants.DefaultOutputFormat)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv6.ArgDataCenterId), testNatGatewayVar)
		viper.Set(core.GetGlobalFlagName(cfg.Resource, constants.ArgCols), defaultNatGatewayCols)
...
func TestGetNatGatewaysCols(t *testing.T) {
	...
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("natgateway", constants.ArgCols), []string{"Name"})
	getNatGatewaysCols(core.GetGlobalFlagName("natgateway", constants.ArgCols), core.GetFlagName("natgateway", cloudapiv6.ArgAll), w)
	err := w.Flush()
	assert.NoError(t, err)
}
...
func TestGetNatGatewaysColsErr(t *testing.T) {
	...
	clierror.ErrAction = func() {}
	w := bufio.NewWriter(&b)
	viper.Set(core.GetGlobalFlagName("natgateway", constants.ArgCols), []string{"Unknown"})
	getNatGatewaysCols(core.GetGlobalFlagName("natgateway", constants.ArgCols), core.GetFlagName("natgateway", cloudapiv6.ArgAll), w)
	err := w.Flush()
	assert.NoError(t, err)
