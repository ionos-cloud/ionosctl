...
func SnapshotCmd() *core.Command {
	...
	*/
	list := core.NewCommand(
		ctx, snapshotCmd, core.CommandBuilder{
			Namespace:  "snapshot",
			Resource:   "snapshot",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
	...
	*/
	...
	get.AddUUIDFlag(
		cloudapiv6.ArgSnapshotId, cloudapiv6.ArgIdShort, "", cloudapiv6.SnapshotId, core.RequiredFlagOption(),
	)
	_ = get.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgSnapshotId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
* Volume Id`,
	...
	create.AddBoolFlag(
		cloudapiv6.ArgSecAuthProtection, "", false,
		"Enable secure authentication protection. E.g.: --sec-auth-protection=true, --sec-auth-protection=false",
	)
	create.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Snapshot creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Snapshot creation [seconds]",
	)
	create.AddInt32Flag(
...
* Snapshot Id`,
	...
	update.AddBoolFlag(
		...
		"This volume is capable of CPU hot plug (no reboot required). E.g.: --cpu-hot-plug=true, --cpu-hot-plug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgCpuHotUnplug, "", false,
		"This volume is capable of CPU hot unplug (no reboot required). E.g.: --cpu-hot-unplug=true, --cpu-hot-unplug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgRamHotPlug, "", false,
		"This volume is capable of memory hot plug (no reboot required). E.g.: --ram-hot-plug=true, --ram-hot-plug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgRamHotUnplug, "", false,
		"This volume is capable of memory hot unplug (no reboot required). E.g.: --ram-hot-unplug=true, --ram-hot-unplug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgNicHotPlug, "", false,
		"This volume is capable of NIC hot plug (no reboot required). E.g.: --nic-hot-plug=true, --nic-hot-plug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgNicHotUnplug, "", false,
		"This volume is capable of NIC hot unplug (no reboot required). E.g.: --nic-hot-unplug=true, --nic-hot-unplug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgDiscVirtioHotPlug, "", false,
		"This volume is capable of VirtIO drive hot plug (no reboot required). E.g.: --disc-virtio-plug=true, --disc-virtio-plug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgDiscVirtioHotUnplug, "", false,
		"This volume is capable of VirtIO drive hot unplug (no reboot required). E.g.: --disc-virtio-unplug=true, --disc-virtio-unplug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgDiscScsiHotPlug, "", false,
		"This volume is capable of SCSI drive hot plug (no reboot required). E.g.: --disc-scsi-plug=true, --disc-scsi-plug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgDiscScsiHotUnplug, "", false,
		"This volume is capable of SCSI drive hot unplug (no reboot required). E.g.: --disc-scsi-unplug=true, --disc-scsi-unplug=false",
	)
	update.AddBoolFlag(
		cloudapiv6.ArgSecAuthProtection, "", false,
		"Enable secure authentication protection. E.g.: --sec-auth-protection=true, --sec-auth-protection=false",
	...
	*/
	...
	restore.AddBoolFlag(constants.ArgForce, constants.ArgForceShort, false, constants.DescForce)
	restore.AddUUIDFlag(
		cloudapiv6.ArgSnapshotId, cloudapiv6.ArgIdShort, "", cloudapiv6.SnapshotId, core.RequiredFlagOption(),
	)
	_ = restore.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgSnapshotId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.SnapshotIds(), cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = restore.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.VolumesIds(
				viper.GetString(
					core.GetFlagName(
						restore.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	restore.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Snapshot restore to be executed",
	)
	restore.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Snapshot restore [seconds]",
	)
	restore.AddInt32Flag(
	...
	*/
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for Snapshot deletion to be executed",
	)
	deleteCmd.AddBoolFlag(cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete all the Snapshots.")
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for Snapshot deletion [seconds]",
	)
...
func PreRunSnapshotIdDcIdVolumeId(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgVolumeId, cloudapiv6.ArgSnapshotId,
	)
}
...
func RunSnapshotList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Snapshot, ss.Snapshots,
...
func RunSnapshotGet(c *core.CommandConfig) error {
	...
	s, resp, err := c.CloudApiV6Services.Snapshots().Get(
		viper.GetString(
			core.GetFlagName(
				c.NS, cloudapiv6.ArgSnapshotId,
			),
		), queryParams,
	)
	if resp != nil {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(), jsontabwriter.GenerateVerboseOutput(
			"Properties set for creating the Snapshot: DatacenterId: %v, VolumeId: %v, Name: %v, Description: %v, "+
				"LicenseType: %v, SecAuthProtection: %v", dcId, volumeId, name, description, licenseType,
			secAuthProtection,
		),
	)

	s, resp, err := c.CloudApiV6Services.Snapshots().Create(
		dcId, volumeId, name, description, licenseType, secAuthProtection, queryParams,
	)
...
func RunSnapshotUpdate(c *core.CommandConfig) error {
	...
	s, resp, err := c.CloudApiV6Services.Snapshots().Update(
...
func RunSnapshotRestore(c *core.CommandConfig) error {
	...
	queryParams := listQueryParams.QueryParams
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "restore snapshot", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
...
func RunSnapshotDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete snapshot", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Snapshot with id: %v...", snapshotId),
	)
...
func getSnapshotPropertiesSet(c *core.CommandConfig) resources.SnapshotProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDescription)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property Description set: %v", description),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgCpuHotUnplug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property CpuHotUnplug set: %v", cpuHotUnplug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgRamHotUnplug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property RamHotUnplug set: %v", ramHotUnplug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDiscScsiHotPlug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property DiscScsiHotPlug set: %v", discScsiHotPlug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgDiscScsiHotUnplug)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property DiscScsiHotUnplug set: %v", discScsiHotUnplug),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgSecAuthProtection)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property SecAuthProtection set: %v", secAuthProtection),
		)
	}
...
func DeleteAllSnapshots(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Snapshots",
	...
	for _, snapshot := range *snapshotsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Snapshot with id: %v...", *id),
		)
