...
* Snapshot Id`,
	...
	*/
	...
	_ = restore.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.VolumesIds(
				os.Stderr, viper.GetString(core.GetFlagName(restore.NS, cloudapiv6.ArgDataCenterId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	restore.AddBoolFlag(
...
func RunSnapshotCreate(c *core.CommandConfig) error {
	...
	c.Printer.Verbose(
		"Properties set for creating the Snapshot: DatacenterId: %v, VolumeId: %v, Name: %v, Description: %v, LicenseType: %v, SecAuthProtection: %v",
	...
	if c != nil {
		...
		if s != nil {
			...
			r.Columns = printer.GetHeadersAllDefault(
				defaultSnapshotCols, viper.GetStringSlice(core.GetFlagName(c.Resource, constants.ArgCols)),
			)
		}
	}
	return r
