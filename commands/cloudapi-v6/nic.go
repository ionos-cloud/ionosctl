...
var (
	...
	allNicCols     = []string{
		"NicId", "Name", "Dhcp", "LanId", "Ips", "IPv6Ips", "State", "FirewallActive",
...
func NicCmd() *core.Command {
	...
	_ = nicCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
	...
	*/
	list := core.NewCommand(
		ctx, nicCmd, core.CommandBuilder{
			Namespace:  "nic",
			Resource:   "nic",
			Verb:       "list",
			Aliases:    []string{"l", "ls"},
...
* Server Id`,
	...
	create.AddBoolFlag(
		cloudapiv6.ArgDhcp, "", cloudapiv6.DefaultDhcp,
		"Set to false if you wish to disable DHCP on the NIC. E.g.: --dhcp=true, --dhcp=false",
	)
	create.AddBoolFlag(
		cloudapiv6.ArgFirewallActive, "", cloudapiv6.DefaultFirewallActive,
		"Activate or deactivate the Firewall. E.g.: --firewall-active=true, --firewall-active=false",
	)
	create.AddStringFlag(
		cloudapiv6.ArgFirewallType, "", "INGRESS", "The type of Firewall Rules that will be allowed on the NIC",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgFirewallType,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return []string{"BIDIRECTIONAL", "INGRESS", "EGRESS"}, cobra.ShellCompDirectiveNoFileComp
		},
	)
	create.AddIntFlag(
		cloudapiv6.ArgLanId, "", cloudapiv6.DefaultNicLanId,
		"The LAN ID the NIC will sit on. If the LAN ID does not exist it will be created",
	)
	_ = create.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgLanId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	create.AddBoolFlag(
		...
		"Wait for the Request for NIC creation to be executed",
	)
	create.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for NIC creation [seconds]",
	)
	create.AddInt32Flag(
	...
	create.AddBoolFlag(
		cloudapiv6.FlagDHCPv6, "", true,
		"Set to false if you wish to disable DHCPv6 on the NIC. E.g.: --dhcpv6=true, --dhcpv6=false",
	)
	create.AddStringSliceFlag(
		cloudapiv6.FlagIPv6IPs, "", nil, "IPv6 IPs assigned to the NIC. They need to be within the NIC IPv6 Cidr Block.",
	)
...
* NIC Id`,
	...
	update.AddBoolFlag(
		cloudapiv6.ArgDhcp, "", cloudapiv6.DefaultDhcp,
		"Boolean value that indicates if the NIC is using DHCP (true) or not (false). E.g.: --dhcp=true, --dhcp=false",
	)
	update.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for NIC update to be executed",
	)
	update.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for NIC update [seconds]",
	)
...
* NIC Id`,
	...
	deleteCmd.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for NIC deletion to be executed",
	)
	...
	deleteCmd.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for NIC deletion [seconds]",
	)
...
func RunNicDelete(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete nic", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
	...
	fmt.Fprintf(
		c.Command.Command.ErrOrStderr(),
		jsontabwriter.GenerateVerboseOutput("Starting deleting Nic with id: %v...", nicId),
	)
...
func DeleteAllNics(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "delete all the Nics", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	for _, nic := range *nicsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting deleting Nic with id: %v...", *id),
		)
...
func LoadBalancerNicCmd() *core.Command {
	...
	/*
		Attach Nic Command
	*/
	attachNic := core.NewCommand(
		ctx, loadbalancerNicCmd, core.CommandBuilder{
...
* NIC Id`,
	...
	_ = attachNic.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allNicCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	attachNic.AddUUIDFlag(
		cloudapiv6.ArgServerId, "", "",
		"The unique Server Id on which NIC is build on. Not required, but it helps in autocompletion",
	)
	_ = attachNic.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgServerId,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.ServersIds(
				viper.GetString(
					core.GetFlagName(
						attachNic.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = attachNic.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.NicsIds(
				viper.GetString(core.GetFlagName(attachNic.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(attachNic.NS, cloudapiv6.ArgServerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	attachNic.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for NIC attachment to be executed",
	)
	attachNic.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for NIC attachment [seconds]",
	)

	/*
		List Nics Command
	*/
	listNics := core.NewCommand(
		ctx, loadbalancerNicCmd, core.CommandBuilder{
			Namespace:  "loadbalancer",
			Resource:   "nic",
			Verb:       "list",
	...
	_ = listNics.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allNicCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = listNics.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.LoadbalancersIds(
				viper.GetString(
					core.GetFlagName(
						listNics.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	listNics.AddInt32Flag(
		constants.FlagMaxResults, constants.FlagMaxResultsShort, cloudapiv6.DefaultMaxResults, constants.DescMaxResults,
	)
	listNics.AddStringFlag(cloudapiv6.ArgOrderBy, "", "", cloudapiv6.ArgOrderByDescription)
	listNics.AddStringSliceFlag(
		cloudapiv6.ArgFilters, cloudapiv6.ArgFiltersShort, []string{""},
		"cloudapiv6.ArgOrderByDescription. Use the following format to set filters: --filters KEY1=VALUE1,KEY2=VALUE2.",
	)
	_ = listNics.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgFilters,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	*/
	...
	_ = getNicCmd.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allNicCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = getNicCmd.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.LoadbalancersIds(
				viper.GetString(
					core.GetFlagName(
						getNicCmd.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = getNicCmd.Command.RegisterFlagCompletionFunc(
		cloudapiv6.ArgNicId,
		func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.AttachedNicsIds(
				viper.GetString(core.GetFlagName(getNicCmd.NS, cloudapiv6.ArgDataCenterId)),
...
* NIC Id`,
	...
	detachNic.AddBoolFlag(constants.ArgForce, constants.ArgForceShort, false, constants.DescForce)
	detachNic.AddStringSliceFlag(constants.ArgCols, "", defaultNicCols, tabheaders.ColsMessage(allNicCols))
	_ = detachNic.Command.RegisterFlagCompletionFunc(
		constants.ArgCols,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return allNicCols, cobra.ShellCompDirectiveNoFileComp
		},
	...
	_ = detachNic.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.AttachedNicsIds(
				viper.GetString(core.GetFlagName(detachNic.NS, cloudapiv6.ArgDataCenterId)),
				viper.GetString(core.GetFlagName(detachNic.NS, cloudapiv6.ArgLoadBalancerId)),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	...
	_ = detachNic.Command.RegisterFlagCompletionFunc(
		...
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.LoadbalancersIds(
				viper.GetString(
					core.GetFlagName(
						detachNic.NS, cloudapiv6.ArgDataCenterId,
					),
				),
			), cobra.ShellCompDirectiveNoFileComp
		},
	)
	detachNic.AddBoolFlag(
		constants.ArgWaitForRequest, constants.ArgWaitForRequestShort, constants.DefaultWait,
		"Wait for the Request for NIC detachment to be executed",
	)
	detachNic.AddIntFlag(
		constants.ArgTimeout, constants.ArgTimeoutShort, constants.DefaultTimeoutSeconds,
		"Timeout option for Request for NIC detachment [seconds]",
	)
...
func PreRunLoadBalancerNicList(c *core.PreCommandConfig) error {
	if err := core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgLoadBalancerId,
	); err != nil {
		return err
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgFilters)) {
...
func PreRunDcNicLoadBalancerIds(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(
		c.Command, c.NS, cloudapiv6.ArgDataCenterId, cloudapiv6.ArgNicId, cloudapiv6.ArgLoadBalancerId,
	)
}
...
func RunLoadBalancerNicList(c *core.CommandConfig) error {
	...
	out, err := jsontabwriter.GenerateOutput(
		"items", jsonpaths.Nic, attachedNics.BalancedNics,
		tabheaders.GetHeaders(allNicCols, defaultNicCols, cols),
	)
	if err != nil {
		return err
...
func RunLoadBalancerNicDetach(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "detach nic from loadbalancer",
	...
	fmt.Fprintf(
		c.Command.Command.OutOrStdout(), jsontabwriter.GenerateLogOutput("Nic successfully detached from Load Balancer"),
	)
	return nil
}
...
func DetachAllNics(c *core.CommandConfig) error {
	...
	nics, resp, err := c.CloudApiV6Services.Loadbalancers().ListNics(
		dcId, lbId, cloudapiv6.ParentResourceListQueryParams,
	)
	if err != nil {
		return err
	}
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), "detach all the Nics", viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	for _, nic := range *nicsItems {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Starting detaching Nic with id: %v...", *id),
		)
	...
	fmt.Fprintf(
		...
		jsontabwriter.GenerateLogOutput("Nics successfully detached from Load Balancer"),
	)
	return nil
}
...
func getNicProperties(c *core.CommandConfig) resources.NicProperties {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgFirewallActive)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property FirewallActive set: %v", firewallActive),
		)
	}
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv6.ArgFirewallType)) {
		...
		fmt.Fprintf(
			c.Command.Command.ErrOrStderr(),
			jsontabwriter.GenerateVerboseOutput("Property FirewallType set: %v", firewallType),
		)
	}
