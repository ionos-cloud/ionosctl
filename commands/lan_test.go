...
func TestRunLanGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		rm.Lan.EXPECT().Get(testLanVar, testLanVar).Return(&v5.Lan{Lan: l}, &testResponse, nil)
		err := RunLanGet(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunLanCreate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		rm.Lan.EXPECT().Create(testLanVar, v5.LanPost{LanPost: lanPostTest}).Return(&v5.LanPost{LanPost: lp}, &testResponse, nil)
