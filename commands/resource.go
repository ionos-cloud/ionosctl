...
func getResourcePrint(c *core.CommandConfig, res []v6.Resource) printer.Result {
	r := printer.Result{}
	if c != nil {
		if res != nil {
			r.OutputJSON = res
...
func getResource(res *v6.Resource) []v6.Resource {
	ress := make([]v6.Resource, 0)
	if res != nil {
		ress = append(ress, v6.Resource{Resource: res.Resource})
	}
	return ress
}

func getResources(groups v6.Resources) []v6.Resource {
	u := make([]v6.Resource, 0)
	if items, ok := groups.GetItemsOk(); ok && items != nil {
		for _, item := range *items {
			u = append(u, v6.Resource{Resource: item})
		}
	}
	return u
}

func getResourceGroups(groups v6.ResourceGroups) []v6.Resource {
	u := make([]v6.Resource, 0)
	if items, ok := groups.GetItemsOk(); ok && items != nil {
		for _, item := range *items {
			u = append(u, v6.Resource{Resource: item})
...
func getResourcesKVMaps(rs []v6.Resource) []map[string]interface{} {
