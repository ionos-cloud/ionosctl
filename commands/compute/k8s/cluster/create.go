...
You can wait for the Cluster to be in "ACTIVE" state using ` + "`" + `--wait-for-state` + "`" + ` flag together with ` + "`" + `--timeout` + "`" + ` option.`,
		Example:    "ionosctl compute k8s cluster create --name NAME",
	...
	cmd.AddBoolFlag(constants.ArgWaitForRequest, "", constants.DefaultWait, "Wait for the Request for Cluster creation to be executed")
	cmd.AddBoolFlag(constants.ArgWaitForState, "", constants.DefaultWait, "Wait for the new Cluster to be in ACTIVE state")
