...
func TestPreRunUserKeyIds(t *testing.T) {
	...
	core.PreCmdConfigTest(t, w, func(cfg *core.PreCommandConfig) {
		...
		cfg.SetFlag(cloudapiv6.ArgUserId, testS3keyVar)
		cfg.SetFlag(cloudapiv6.ArgS3KeyId, testS3keyVar)
...
func TestRunUserS3KeyList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		cfg.SetFlag(cloudapiv6.ArgUserId, testS3keyVar)
		viper.Set(constants.ArgOutput, constants.DefaultOutputFormat)
		rm.CloudApiV6Mocks.S3Key.EXPECT().List(testS3keyVar).Return(s3keys, &testutil.TestResponse, nil)
...
func TestRunUserS3KeyGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		cfg.SetFlag(cloudapiv6.ArgUserId, testS3keyVar)
		cfg.SetFlag(cloudapiv6.ArgS3KeyId, testS3keyVar)
		rm.CloudApiV6Mocks.S3Key.EXPECT().Get(testS3keyVar, testS3keyVar).Return(&s3keyTestGet, &testutil.TestResponse, nil)
...
func TestRunUserS3KeyCreateResponseErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(constants.ArgOutput, constants.DefaultOutputFormat)
		cfg.SetFlag(cloudapiv6.ArgUserId, testS3keyVar)
		rm.CloudApiV6Mocks.S3Key.EXPECT().Create(testS3keyVar).Return(&s3keyTest, &testutil.TestResponse, testS3keyErr)
		err := RunUserS3KeyCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunUserS3KeyUpdate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		cfg.SetFlag(cloudapiv6.ArgUserId, testS3keyVar)
		cfg.SetFlag(cloudapiv6.ArgS3KeyId, testS3keyVar)
		cfg.SetFlag(cloudapiv6.ArgS3KeyActive, testS3keyBoolNewVar)
...
func TestRunUserS3KeyDeleteAll(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.S3Key.EXPECT().List(testS3keyVar).Return(s3keysList, &testutil.TestResponse, nil)
		rm.CloudApiV6Mocks.S3Key.EXPECT().Delete(testS3keyVar, testS3keyVar).Return(&testutil.TestResponse, nil)
		rm.CloudApiV6Mocks.S3Key.EXPECT().Delete(testS3keyVar, testS3keyVar).Return(&testutil.TestResponse, nil)
		err := RunUserS3KeyDelete(cfg)
		assert.NoError(t, err)
...
func TestRunUserS3KeyDeleteAllItemsErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		cfg.SetFlag(cloudapiv6.ArgUserId, testS3keyVar)
		cfg.SetFlag(cloudapiv6.ArgAll, true)
		rm.CloudApiV6Mocks.S3Key.EXPECT().List(testS3keyVar).Return(resources.S3Keys{}, &testutil.TestResponse, nil)
		err := RunUserS3KeyDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunUserS3KeyDeleteAllLenErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV6Mocks.S3Key.EXPECT().List(testS3keyVar).Return(
			resources.S3Keys{S3Keys: ionoscloud.S3Keys{Items: &[]ionoscloud.S3Key{}}}, &testutil.TestResponse, nil)
		err := RunUserS3KeyDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunUserS3KeyDeleteAllErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		cfg.SetFlag(cloudapiv6.ArgAll, true)
		rm.CloudApiV6Mocks.S3Key.EXPECT().List(testS3keyVar).Return(s3keysList, &testutil.TestResponse, nil)
		rm.CloudApiV6Mocks.S3Key.EXPECT().Delete(testS3keyVar, testS3keyVar).Return(&testutil.TestResponse, testS3keyErr)
		rm.CloudApiV6Mocks.S3Key.EXPECT().Delete(testS3keyVar, testS3keyVar).Return(&testutil.TestResponse, nil)
		err := RunUserS3KeyDelete(cfg)
		assert.Error(t, err)
	})
