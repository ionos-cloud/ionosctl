...
func ServerCdromListCmd() *core.Command {
	listCdroms := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		...
		Verb:       "list",
		Aliases:    []string{"l", "ls"},
		ShortDesc:  "List the CD-ROMs attached to a Server",
		LongDesc:   "Use this command to list the CD-ROM images currently attached to a Server. Use the returned CD-ROM Id with `server cdrom get`, `server cdrom detach`, or `server update --cdrom-id` to boot from it.\n\nYou can filter the results using `--filters` option. Use the following format to set filters: `--filters KEY1=VALUE1,KEY2=VALUE2`.\n" + completer.ImagesFiltersUsage() + "\n\nRequired values to run command:\n\n* Data Center Id\n* Server Id",
