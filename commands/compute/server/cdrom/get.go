...
func ServerCdromGetCmd() *core.Command {
	getCdromCmd := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		...
		Verb:       "get",
		Aliases:    []string{"g"},
		ShortDesc:  "Get a CD-ROM attached to a Server",
		LongDesc:   "Use this command to retrieve the image details of a single CD-ROM currently attached to a Server. The --cdrom-id must be one that is attached to this Server; list them with `server cdrom list`.\n\nRequired values to run command:\n\n* Data Center Id\n* Server Id\n* Cdrom Id",
