...
func ServerTokenGetCmd() *core.Command {
	get := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		Namespace:  "server",
		Resource:   "token",
		Verb:       "get",
		Aliases:    []string{"g"},
		ShortDesc:  "Get a Server's JWT access token",
		LongDesc:   "Use this command to retrieve the JSON Web Token (JWT) for a Server. This token authorizes access to that server's endpoints (such as the VNC remote console). It is a credential — keep it secret and avoid printing it into shared logs.\n\nRequired values to run command:\n\n* Data Center Id\n* Server Id",
