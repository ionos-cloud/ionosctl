...
func ServerVolumeGetCmd() *core.Command {
	getVolumeCmd := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		...
		Verb:       "get",
		Aliases:    []string{"g"},
		ShortDesc:  "Get a Volume attached to a Server",
		LongDesc:   "Use this command to retrieve the details (size, type, licence type, bus, state, image) of a single Volume attached to a Server. The --volume-id must be one currently attached to this Server; list them with `server volume list`.\n\nRequired values to run command:\n\n* Data Center Id\n* Server Id\n* Volume Id",
