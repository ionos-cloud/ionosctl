...
func NatgatewayListCmd() *core.Command {
	cmd := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		...
		LongDesc:   "Use this command to list NAT Gateways from a specified Virtual Data Center. Pass `--all` to list gateways across every datacenter in the account instead of a single one.\n\nYou can filter the results using `--filters` option. Use the following format to set filters: `--filters KEY1=VALUE1,KEY2=VALUE2`.\n" + completer.NATGatewaysFiltersUsage() + "\n\nRequired values to run command:\n\n* Data Center Id",
