...
func LoadBalancerNicListCmd() *core.Command {
	cmd := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		...
		Verb:       "list",
		Aliases:    []string{"l", "ls"},
		ShortDesc:  "List NICs attached to a Load Balancer",
		LongDesc:   "Use this command to list every NIC currently attached to a Load Balancer's backend pool, i.e. the servers that receive its balanced traffic. Each entry shows the NIC's inherited IP(s), LAN, DHCP setting and state.\n\nRequired values to run command:\n\n* Data Center Id\n* Load Balancer Id",
