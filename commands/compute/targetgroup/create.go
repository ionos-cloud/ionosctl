...
func TargetGroupCreateCmd() *core.Command {
	cmd := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		...
		LongDesc: `Create a Target Group: a reusable backend pool that an Application Load Balancer forwarding rule can point to.
...
Use ` + "`" + `--wait` + "`" + ` (` + "`" + `-w` + "`" + `) to wait for the resource to reach AVAILABLE state.`,
		Example: `# Minimal group (defaults: ROUND_ROBIN, HTTP, GET / expecting status 200)
...
ionosctl compute targetgroup create --name api-backends --algorithm SOURCE_IP \
  ...
  --path /status --match-type RESPONSE_BODY --response OK`,
	...
	cmd.AddStringFlag(cloudapiv6.ArgName, cloudapiv6.ArgNameShort, "Unnamed Target Group", "The name of the target group. Used only for display; does not need to be unique.")
