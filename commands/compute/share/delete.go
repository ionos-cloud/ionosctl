...
func ShareDeleteCmd() *core.Command {
	cmd := core.NewCommand(context.TODO(), nil, core.CommandBuilder{
		...
		Verb:      "delete",
		Aliases:   []string{"d"},
		ShortDesc: "Revoke a Group's access to a shared resource",
		LongDesc: `Delete a Share, revoking the Group's access to that specific resource. The resource itself is not deleted - only this Group loses access to it (members may retain access if it is shared with another of their Groups, or if they are admins/owners).
...
* Group Id`,
	...
	cmd.AddBoolFlag(cloudapiv6.ArgAll, cloudapiv6.ArgAllShort, false, "Delete every Share of the Group, revoking its access to all shared resources at once")
