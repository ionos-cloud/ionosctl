...
func RunNlbRuleTargetList(c *core.CommandConfig) error {
	...
	return c.Printer(allCols).Print(*targets)
}
...
func RunNlbRuleTargetAdd(c *core.CommandConfig) error {
	...
	c.Verbose("Adding NlbRuleTarget with id: %v to NetworkLoadBalancer with id: %v", ruleId, nlbId)
	...
	return c.Printer(allCols).Print(targetNew.NetworkLoadBalancerForwardingRuleTarget)
}
...
func RunNlbRuleTargetRemove(c *core.CommandConfig) error {
	...
	c.Verbose("NlbRuleTarget with id: %v is removing...", c.Flags().String(cloudapiv6.ArgRuleId))
	...
	c.Msg("Network Load Balancer Forwarding Rule Target successfully deleted")
	return nil
}
...
func RemoveAllNlbRuleTarget(c *core.CommandConfig) error {
	...
	c.Verbose("NetworkLoadBalancer ID: %v", nlbId)
	c.Verbose("NetworkLoadBalancerForwardingRule ID: %v", ruleId)
	c.Verbose("Getting NetworkLoadBalancerForwardingRule...")
	...
	c.Msg("Forwarding Rule Targets to be removed:")
	...
	c.Verbose("Removing all the Forwarding Rule Targets...")
	...
	c.Msg("Network Load Balancer Forwarding Rule Targets successfully deleted")
	return nil
}

func getRuleTargetInfo(c *core.CommandConfig) resources.NetworkLoadBalancerForwardingRuleTarget {
	targetIp := c.Flags().String(cloudapiv6.ArgIp)
	targetPort := c.Flags().Int32(cloudapiv6.ArgPort)
	weight := c.Flags().Int32(cloudapiv6.ArgWeight)
	...
	maintenance := c.Flags().Bool(cloudapiv6.ArgMaintenance)
	check := c.Flags().Bool(cloudapiv6.ArgCheck)
	checkInterval := c.Flags().Int32(cloudapiv6.ArgCheckInterval)
	...
	c.Verbose("Properties set for adding the NlbRuleTarget: Ip: %v, Port: %v, Weight: %v, Maintenance: %v, Check: %v, CheckInterval: %v",
...
func getRuleTargetsRemove(c *core.CommandConfig, frOld *resources.NetworkLoadBalancerForwardingRule) (*resources.NetworkLoadBalancerForwardingRuleProperties, error) {
	...
	for _, targetItem := range *targets {
		...
		if port, ok := targetItem.GetPortOk(); ok && port != nil {
			if *port == c.Flags().Int32(cloudapiv6.ArgPort) {
				removePort = true
				foundPort = true
			}
		}
