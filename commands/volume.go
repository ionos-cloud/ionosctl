...
* Volume Id`,
	...
	update.AddStringFlag(config.ArgSize, "", "", "The size of the Volume in GB. e.g. 10 or 10GB. The maximum volume size is determined by your contract limit")
...
func RunVolumeCreate(c *core.CommandConfig) error {
	...
	vol, resp, err := c.Volumes().Create(viper.GetString(core.GetFlagName(c.NS, config.ArgDataCenterId)), *input)
	if resp != nil {
...
func RunVolumeUpdate(c *core.CommandConfig) error {
	...
	vol, resp, err := c.Volumes().Update(viper.GetString(core.GetFlagName(c.NS, config.ArgDataCenterId)),
		viper.GetString(core.GetFlagName(c.NS, config.ArgVolumeId)), *input)
	if err != nil {
		return err
	}
	if err = utils.WaitForRequest(c, printer.GetRequestPath(resp)); err != nil {
...
func getNewVolume(c *core.CommandConfig) (*v6.Volume, error) {
	proper := v6.VolumeProperties{}
	...
	if viper.IsSet(core.GetFlagName(c.NS, config.ArgImageId)) ||
		...
		imagePassword := viper.GetString(core.GetFlagName(c.NS, config.ArgPassword))
...
func getVolumeInfo(c *core.CommandConfig) (*v6.VolumeProperties, error) {
	input := v6.VolumeProperties{}
...
func getVolumePrint(resp *v6.Response, c *core.CommandConfig, vols []v6.Volume) printer.Result {
	r := printer.Result{}
	if c != nil {
		if resp != nil {
			r.ApiResponse = resp
...
func getVolumes(volumes v6.Volumes) []v6.Volume {
	vs := make([]v6.Volume, 0)
	for _, s := range *volumes.Items {
		vs = append(vs, v6.Volume{Volume: s})
	}
	return vs
}

func getVolume(vol *v6.Volume) []v6.Volume {
	vols := make([]v6.Volume, 0)
	if vol != nil {
		vols = append(vols, v6.Volume{Volume: vol.Volume})
	}
	return vols
}

func getAttachedVolumes(volumes v6.AttachedVolumes) []v6.Volume {
	vs := make([]v6.Volume, 0)
...
func getVolumesKVMaps(vs []v6.Volume) []map[string]interface{} {
...
func getVolumesIds(outErr io.Writer, datacenterId string) []string {
	...
	clierror.CheckError(err, outErr)
	volumeSvc := v6.NewVolumeService(clientSvc.Get(), context.TODO())
