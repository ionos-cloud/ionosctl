...
func RunServerGet(c *core.CommandConfig) error {
	c.Printer.Infof("Server with id: %v is getting... ", viper.GetString(core.GetFlagName(c.NS, config.ArgServerId)))
...
func RunServerDelete(c *core.CommandConfig) error {
	...
	c.Printer.Infof("Server with id: %v from datacenter with id: %v is deleting... ", viper.GetString(core.GetFlagName(c.NS, config.ArgServerId)), viper.GetString(core.GetFlagName(c.NS, config.ArgDataCenterId)))
...
func RunServerStart(c *core.CommandConfig) error {
	...
	c.Printer.Info("Server is starting... ")
...
func RunServerStop(c *core.CommandConfig) error {
	...
	c.Printer.Info("Server is stopping... ")
...
func RunServerReboot(c *core.CommandConfig) error {
	c.Printer.Info("Server is rebooting... ")
...
func getNewServerInfo(c *core.CommandConfig) (*resources.ServerProperties, error) {
	...
	if viper.IsSet(core.GetFlagName(c.NS, config.ArgCPUFamily)) {
		...
		c.Printer.Infof("Property CpuFamily set: %v ", cpuFamily)
	...
	if viper.IsSet(core.GetFlagName(c.NS, config.ArgAvailabilityZone)) {
		...
		c.Printer.Infof("Property AvailabilityZone set: %v ", availabilityZone)
	...
	if viper.IsSet(core.GetFlagName(c.NS, config.ArgCores)) {
		...
		c.Printer.Infof("Property Cores set: %v ", cores)
	...
	if viper.IsSet(core.GetFlagName(c.NS, config.ArgRam)) {
		...
		c.Printer.Infof("property Ram set: %v ", int32(size))
