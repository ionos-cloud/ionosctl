...
func CertCreateCmd() *core.Command {
	...
	cmd.AddStringFlag(
		FlagCertChain, "", "", "Specify the certificate chain (required either this or --certificate-chain-path)",
	)
	cmd.AddStringFlag(FlagPrivateKey, "", "", "Specify the private key (required either this or --private-key-path)")

	cmd.AddStringFlag(
		FlagCertPath, "", "", "Specify the certificate itself from a file (required either this or --certificate)",
	)
	cmd.AddStringFlag(
		FlagCertChainPath, "", "",
		"Specify the certificate chain from a file (required either this or --certificate-chain)",
	)
	cmd.AddStringFlag(
		FlagPrivateKeyPath, "", "", "Specify the private key from a file (required either this or --private-key)",
	)
...
func PreCmdPost(c *core.PreCommandConfig) error {
	...
	if !viper.IsSet(core.GetFlagName(c.NS, FlagPrivateKey)) && !viper.IsSet(
		core.GetFlagName(
			c.NS, FlagPrivateKeyPath,
		),
	) {
