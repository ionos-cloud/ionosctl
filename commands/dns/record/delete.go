...
ionosctl dns r delete --record PARTIAL_NAME --zone ZONE`,
			PreCmdRun: func(c *core.PreCommandConfig) error {
				err := core.CheckRequiredFlagsSets(
					c.Command, c.NS, []string{constants.ArgAll}, // All with optional filters
					[]string{constants.FlagZone, constants.FlagRecord}, // Known resource
				)
				if err != nil {
					return fmt.Errorf(
						"either provide --%s and optionally filters, or --%s and --%s, or narrow down to one record with --%s and/or --%s: %w",
						constants.ArgAll, constants.FlagZone, constants.FlagRecord, constants.FlagName,
						constants.FlagZone, err,
					)
				}
				return nil
			},
			CmdRun: func(c *core.CommandConfig) error {
				...
				if fn := core.GetFlagName(c.NS, constants.FlagRecord); viper.IsSet(fn) {
					if _, ok := uuid.FromString(viper.GetString(fn)); ok != nil /* not ok (name is provided) */ {
						...
						r, _, err = client.Must().DnsClient.RecordsApi.ZonesRecordsFindById(
							context.Background(), zoneId, viper.GetString(fn),
						).Execute()
						if err != nil {
				...
				yes := confirm.FAsk(
					c.Command.Command.InOrStdin(), fmt.Sprintf(
						"Are you sure you want to delete record %s (type: '%s'; content: '%s')", *r.Metadata.Fqdn,
						*r.Properties.Type, *r.Properties.Content,
					),
				...
				_, err = client.Must().DnsClient.RecordsApi.ZonesRecordsDelete(
					context.Background(),
					*r.Metadata.ZoneId,
					*r.Id,
				).Execute()
	...
	cmd.AddBoolFlag(
		constants.ArgAll, constants.ArgAllShort, false, fmt.Sprintf(
			"Delete all records. You can optionally filter the deleted records using --%s (full name / ID) and --%s (partial name)",
			constants.FlagZone, constants.FlagRecord,
		),
	)
	cmd.AddStringFlag(
		constants.FlagZone, constants.FlagZoneShort, "", fmt.Sprintf(
			"The full name or ID of the zone of the containing the target record. If --%s is set this is applied as a filter - limiting to records within this zone",
			constants.ArgAll,
		),
	)
	...
	cmd.AddStringFlag(
		constants.FlagRecord, constants.FlagRecordShort, "", fmt.Sprintf(
			"The ID, or full name of the DNS record. Required together with --%s. Can also provide partial names, but must narrow down to a single record result if not using --%s. If using it, will however delete all records that match.",
			constants.FlagZone, constants.ArgAll,
		),
	)
	_ = cmd.Command.RegisterFlagCompletionFunc(
		constants.FlagRecord,
		func(c *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return RecordsProperty(
				func(r dns.RecordRead) string {
					return *r.Properties.Name
				}, FilterRecordsByZoneAndRecordFlags(cmd.NS),
...
func deleteAll(c *core.CommandConfig) error {
	...
	err = functional.ApplyAndAggregateErrors(
		*xs.GetItems(), func(r dns.RecordRead) error {
			yes := confirm.FAsk(
				c.Command.Command.InOrStdin(), fmt.Sprintf(
					"Are you sure you want to delete record %s (type: '%s'; content: '%s')", *r.Properties.Name,
					*r.Properties.Type, *r.Properties.Content,
				),
				viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
			)
			...
			if yes {
				_, delErr := client.Must().DnsClient.RecordsApi.ZonesRecordsDelete(
					c.Context, *r.Metadata.ZoneId, *r.Id,
				).Execute()
				if delErr != nil {
...
func deleteSingleWithFilters(c *core.CommandConfig) (dns.RecordRead, error) {
	...
	if recsLen == 0 {
		return dns.RecordRead{}, fmt.Errorf(
			"found no records matching given filters (--%s and/or --%s). They"+
				" must narrow down to a single result", constants.FlagRecord, constants.FlagZone,
		)
	}

	if recsLen > 1 {
		recsNames := functional.Fold(
			*recs.Items, func(acc []string, t dns.RecordRead) []string {
				return append(acc, *t.Properties.Name)
			}, []string{},
		)

		return dns.RecordRead{}, fmt.Errorf(
			"got %d but expected 1: %+v. The given filters (--%s and/or --%s) "+
