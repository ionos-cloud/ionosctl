...
func ZonesDeleteCmd() *core.Command {
		...
		context.Background(), nil, core.CommandBuilder{
			...
			PreCmdRun: func(c *core.PreCommandConfig) error {
				return core.CheckRequiredFlagsSets(
					c.Command, c.NS, []string{constants.ArgAll}, []string{constants.FlagZone},
				)
			},
			CmdRun: func(c *core.CommandConfig) error {
				...
				yes := confirm.FAsk(
					c.Command.Command.InOrStdin(), fmt.Sprintf(
						"Are you sure you want to delete zone %s (desc: '%s')", *z.Properties.ZoneName,
						*z.Properties.Description,
					),
	...
	cmd.AddStringFlag(
		constants.FlagZone, constants.FlagZoneShort, "",
		fmt.Sprintf("%s. Required or -%s", constants.DescZone, constants.ArgAllShort),
	)
	_ = cmd.Command.RegisterFlagCompletionFunc(
		constants.FlagZone,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return ZonesProperty(
	...
	cmd.AddBoolFlag(
		...
		fmt.Sprintf("Delete all zones. Required or -%s", constants.FlagZoneShort),
	)
...
func deleteAll(c *core.CommandConfig) error {
	...
	err = functional.ApplyAndAggregateErrors(
		*xs.GetItems(), func(z dns.ZoneRead) error {
			yes := confirm.FAsk(
				c.Command.Command.InOrStdin(), fmt.Sprintf(
					"Are you sure you want to delete zone %s (desc: '%s')", *z.Properties.ZoneName,
					*z.Properties.Description,
