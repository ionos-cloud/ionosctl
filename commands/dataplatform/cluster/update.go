		...
		context.TODO(), nil, core.CommandBuilder{
			...
			CmdRun: func(c *core.CommandConfig) error {
				...
				if viper.IsSet(core.GetFlagName(c.NS, constants.FlagVersion)) {
					updateProperties.SetDataPlatformVersion(
						viper.GetString(
							core.GetFlagName(
								c.NS, constants.FlagVersion,
							),
						),
					)
				}
				...
				cluster, _, err := client.Must().DataplatformClient.DataPlatformClusterApi.ClustersPatch(
					c.Context, clusterId,
				).PatchClusterRequest(sdkdataplatform.PatchClusterRequest{Properties: &updateProperties}).Execute()
				if err != nil {
					return err
				}
