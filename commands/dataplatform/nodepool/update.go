...
func NodepoolUpdateCmd() *core.Command {
	...
	cmd := core.NewCommand(
		context.TODO(), nil, core.CommandBuilder{
			...
			CmdRun: func(c *core.CommandConfig) error {
				...
				cr, _, err := client.Must().DataplatformClient.DataPlatformNodePoolApi.ClustersNodepoolsPatch(
					context.Background(), clusterId, npId,
				).PatchNodePoolRequest(input).Execute()
				if err != nil {
					return err
				}
	...
	cmd.AddUUIDFlag(
		constants.FlagNodepoolId, constants.FlagIdShort, "", "The UUID of the cluster the nodepool belongs to",
	)
	_ = cmd.Command.RegisterFlagCompletionFunc(
	...
	cmd.AddInt32Flag(
		constants.FlagNodeCount, "n", 0, "The number of nodes that make up the node pool", core.RequiredFlagOption(),
	)
