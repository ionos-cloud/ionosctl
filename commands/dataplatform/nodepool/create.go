...
func NodepoolCreateCmd() *core.Command {
	...
	cmd := core.NewCommand(
		context.TODO(), nil, core.CommandBuilder{
			...
			CmdRun: func(c *core.CommandConfig) error {
				...
				cr, _, err := client.Must().DataplatformClient.DataPlatformNodePoolApi.ClustersNodepoolsPost(
					context.Background(), clusterId,
				).CreateNodePoolRequest(input).Execute()
				if err != nil {
					return err
				}
	...
	cmd.AddUUIDFlag(
		constants.FlagClusterId, constants.FlagIdShort, "", "The UUID of the cluster the nodepool belongs to",
	)
	...
	cmd.AddStringVarFlag(
		&name, constants.FlagName, constants.FlagNameShort, "", "The name of your nodepool", core.RequiredFlagOption(),
	)
	_ = cmd.Command.RegisterFlagCompletionFunc(
		constants.FlagName,
		func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	cmd.AddInt32VarFlag(
		&nodeCount, constants.FlagNodeCount, "", 0, "The number of nodes that make up the node pool",
	...
	cmd.AddInt32Flag(
		constants.FlagStorageSize, "", 0, "The size of the volume in GB. The size must be greater than 10GB",
	)

	cmd.AddStringToStringFlag(
