...
func NodepoolDeleteCmd() *core.Command {
		...
		context.TODO(), nil, core.CommandBuilder{
			...
			PreCmdRun: func(c *core.PreCommandConfig) error {
				return core.CheckRequiredFlagsSets(
					c.Command, c.NS, []string{constants.ArgAll},
					[]string{constants.FlagClusterId, constants.FlagNodepoolId},
					[]string{constants.ArgAll, constants.FlagClusterId},
				)
			},
			CmdRun: func(c *core.CommandConfig) error {
				...
				np, _, err := client.Must().DataplatformClient.DataPlatformNodePoolApi.ClustersNodepoolsFindById(
					context.Background(), clusterId, nodepoolId,
				).Execute()
				if err != nil {
				...
				ok := confirm.FAsk(
					...
					fmt.Sprintf("delete nodepool %s (%s)", nodepoolId, *np.Properties.Name),
				...
				fmt.Fprintf(
					...
					jsontabwriter.GenerateVerboseOutput("Deleting nodepool: %s", nodepoolId),
				)
				_, _, err = client.Must().DataplatformClient.DataPlatformNodePoolApi.ClustersNodepoolsDelete(
					c.Context, clusterId, nodepoolId,
				).Execute()
				return err
			},
			InitClient: true,
	...
	cmd.AddUUIDFlag(
		constants.FlagNodepoolId, constants.FlagIdShort, "", "The unique ID of the nodepool", core.RequiredFlagOption(),
	)
	_ = cmd.Command.RegisterFlagCompletionFunc(
		constants.FlagNodepoolId,
		func(c *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return completer.DataplatformNodepoolsIds(
				viper.GetString(
					core.GetFlagName(
						cmd.NS, constants.FlagClusterId,
					),
				),
	...
	cmd.AddBoolFlag(
		constants.ArgAll, constants.ArgAllShort, false,
		"Delete all clusters. If cluster ID is provided, delete all nodepools in given cluster",
	)
...
func deleteAll(c *core.CommandConfig, clusterId string) error {
	...
	return functional.ApplyAndAggregateErrors(
		*ls.GetItems(), func(x ionoscloud.ClusterResponseData) error {
...
func deleteNodePools(c *core.CommandConfig, clusterId string) error {
	xs, _, err := client.Must().DataplatformClient.DataPlatformNodePoolApi.ClustersNodepoolsGet(
	...
	return functional.ApplyAndAggregateErrors(
		*xs.GetItems(), func(x ionoscloud.NodePoolResponseData) error {
			ok := confirm.FAsk(
				c.Command.Command.InOrStdin(), fmt.Sprintf("delete nodepool %s (%s)", *x.Id, *x.Properties.Name),
				viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
			)
			if !ok {
			...
			_, _, err := client.Must().DataplatformClient.DataPlatformNodePoolApi.ClustersNodepoolsDelete(
				context.Background(), clusterId, *x.Id,
			).Execute()
			return err
		},
	)
