...
func setup() error {
	...
	if resp.HttpNotFound() || len(*dcs.Items) < 1 {
		dc, _, err := client.Must().CloudClient.DataCentersApi.DatacentersPost(context.Background()).Datacenter(sdkcompute.Datacenter{Properties: &sdkcompute.DatacenterProperties{Name: shared.ToPtr(uniqueResourceName), Location: shared.ToPtr("de/fra")}}).Execute()
		if err != nil {
