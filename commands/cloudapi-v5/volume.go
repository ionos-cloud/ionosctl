...
func VolumeCmd() *core.Command {
	...
	*/
	...
	get.AddStringFlag(cloudapiv5.ArgVolumeId, cloudapiv5.ArgIdShort, "", cloudapiv5.VolumeId, core.RequiredFlagOption())
...
* Data Center Id`,
	...
	create.AddStringFlag(cloudapiv5.ArgBus, "", "VIRTIO", "The bus type of the Volume")
	_ = create.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgBus, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return []string{"VIRTIO", "IDE"}, cobra.ShellCompDirectiveNoFileComp
	})
	create.AddStringFlag(cloudapiv5.ArgLicenceType, "", "LINUX", "Licence Type of the Volume")
	...
	create.AddStringFlag(cloudapiv5.ArgType, "", "HDD", "Type of the Volume")
	_ = create.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgLicenceType, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return []string{"HDD", "SSD", "SSD Standard", "SSD Premium"}, cobra.ShellCompDirectiveNoFileComp
	})
	create.AddStringFlag(cloudapiv5.ArgAvailabilityZone, cloudapiv5.ArgAvailabilityZoneShort, "AUTO", "Availability zone of the Volume. Storage zone can only be selected prior provisioning")
	...
	create.AddStringFlag(cloudapiv5.ArgBackupUnitId, "", "", "The unique Id of the Backup Unit that User has access to. It is mandatory to provide either 'public image' or 'imageAlias' in conjunction with this property")
	_ = create.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgBackupUnitId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.BackupUnitsIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	create.AddStringFlag(cloudapiv5.ArgImageId, "", "", "The Image Id or Snapshot Id to be used as template for the new Volume")
...
* Volume Id`,
	...
	update.AddStringFlag(cloudapiv5.ArgVolumeId, cloudapiv5.ArgIdShort, "", cloudapiv5.VolumeId, core.RequiredFlagOption())
	...
	update.AddStringFlag(cloudapiv5.ArgName, cloudapiv5.ArgNameShort, "", "Name of the Volume")
	update.AddStringFlag(cloudapiv5.ArgSize, "", strconv.Itoa(cloudapiv5.DefaultVolumeSize), "The size of the Volume in GB. e.g. 10 or 10GB. The maximum volume size is determined by your contract limit")
	_ = update.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgSize, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return []string{"10GB", "20GB", "50GB", "100GB", "1TB"}, cobra.ShellCompDirectiveNoFileComp
	})
...
* Volume Id`,
	...
	deleteCmd.AddStringFlag(cloudapiv5.ArgVolumeId, cloudapiv5.ArgIdShort, "", cloudapiv5.VolumeId, core.RequiredFlagOption())
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgVolumeId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
func PreRunDcVolumeIds(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(c.Command, c.NS, cloudapiv5.ArgDataCenterId, cloudapiv5.ArgVolumeId)
}

func PreRunVolumeCreate(c *core.PreCommandConfig) error {
	err := core.CheckRequiredFlags(c.Command, c.NS, cloudapiv5.ArgDataCenterId)
	if err != nil {
		return err
	}
	// Validate flags
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgImageId)) || viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgImageAlias)) {
		return core.CheckRequiredFlagsSets(c.Command, c.NS,
			[]string{cloudapiv5.ArgDataCenterId, cloudapiv5.ArgImageId, cloudapiv5.ArgPassword},
			[]string{cloudapiv5.ArgDataCenterId, cloudapiv5.ArgImageId, cloudapiv5.ArgSshKeyPaths},
			[]string{cloudapiv5.ArgDataCenterId, cloudapiv5.ArgImageAlias, cloudapiv5.ArgPassword},
			[]string{cloudapiv5.ArgDataCenterId, cloudapiv5.ArgImageAlias, cloudapiv5.ArgSshKeyPaths})
	}
	return nil
}
...
func RunVolumeList(c *core.CommandConfig) error {
	c.Printer.Verbose("Listing Volumes from Datacenter with ID: %v", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)))
	...
	volumes, resp, err := c.CloudApiV5Services.Volumes().List(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)), listQueryParams)
	if resp != nil {
...
func RunVolumeGet(c *core.CommandConfig) error {
	...
	c.Printer.Verbose("Volume with id: %v is getting...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgVolumeId)))
	vol, resp, err := c.CloudApiV5Services.Volumes().Get(
...
func RunVolumeCreate(c *core.CommandConfig) error {
	...
	vol, resp, err := c.CloudApiV5Services.Volumes().Create(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)),
		*input,
	)
	if resp != nil {
...
func RunVolumeUpdate(c *core.CommandConfig) error {
	...
	c.Printer.Verbose("Datacenter ID: %v", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)))
	c.Printer.Verbose("Updating Volume with ID: %v...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgVolumeId)))
	vol, resp, err := c.CloudApiV5Services.Volumes().Update(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)),
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgVolumeId)),
		*input,
	)
...
func getNewVolume(c *core.CommandConfig) (*resources.Volume, error) {
	...
	bus := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgBus))
	volumeType := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgType))
	...
	size, err := utils.ConvertSize(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgSize)),
		utils.GigaBytes,
	)
	if err != nil {
		return nil, err
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgBackupUnitId)) {
		backupUnitId := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgBackupUnitId))
		proper.SetBackupunitId(backupUnitId)
		c.Printer.Verbose("Property BackupUnitId set: %v", backupUnitId)
	}
	if (!viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgImageId)) &&
		!viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgImageAlias))) ||
		viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgLicenceType)) {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgImageId)) {
		imageId := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgImageId))
		proper.SetImage(imageId)
		c.Printer.Verbose("Property Image set: %v", imageId)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgImageAlias)) {
		imageAlias := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgImageAlias))
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgPassword)) {
		password := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgPassword))
		proper.SetImagePassword(password)
		c.Printer.Verbose("Property Password set")
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgSshKeyPaths)) {
		sshKeyPaths := viper.GetStringSlice(core.GetFlagName(c.NS, cloudapiv5.ArgSshKeyPaths))
		sshKeys, err := getSshKeysFromPaths(sshKeyPaths)
		if err != nil {
			return nil, err
		}
		proper.SetSshKeys(sshKeys)
		c.Printer.Verbose("Property SshKeys set from paths: %v", sshKeyPaths)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgUserData)) {
		userData := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserData))
...
func getVolumeInfo(c *core.CommandConfig) (*resources.VolumeProperties, error) {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgBus)) {
		bus := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgBus))
		input.SetBus(bus)
		c.Printer.Verbose("Property Bus set: %v", bus)
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgSize)) {
		size, err := utils.ConvertSize(
			viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgSize)),
			utils.GigaBytes,
		)
...
* Volume Id`,
	...
	attachVolume.AddStringFlag(cloudapiv5.ArgDataCenterId, "", "", cloudapiv5.DatacenterId, core.RequiredFlagOption())
	_ = attachVolume.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgDataCenterId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.DataCentersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	attachVolume.AddStringFlag(cloudapiv5.ArgVolumeId, cloudapiv5.ArgIdShort, "", cloudapiv5.VolumeId, core.RequiredFlagOption())
	_ = attachVolume.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgVolumeId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.VolumesIds(os.Stderr, viper.GetString(core.GetFlagName(attachVolume.NS, cloudapiv5.ArgDataCenterId))), cobra.ShellCompDirectiveNoFileComp
	})
	attachVolume.AddStringFlag(cloudapiv5.ArgServerId, "", "", cloudapiv5.ServerId, core.RequiredFlagOption())
	_ = attachVolume.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgServerId, func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.ServersIds(os.Stderr, viper.GetString(core.GetFlagName(attachVolume.NS, cloudapiv5.ArgDataCenterId))), cobra.ShellCompDirectiveNoFileComp
	})
	...
	*/
	...
	listVolumes.AddStringFlag(cloudapiv5.ArgDataCenterId, "", "", cloudapiv5.DatacenterId, core.RequiredFlagOption())
	_ = listVolumes.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgDataCenterId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.DataCentersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	listVolumes.AddStringFlag(cloudapiv5.ArgServerId, "", "", cloudapiv5.ServerId, core.RequiredFlagOption())
	_ = listVolumes.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgServerId, func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.ServersIds(os.Stderr, viper.GetString(core.GetFlagName(listVolumes.NS, cloudapiv5.ArgDataCenterId))), cobra.ShellCompDirectiveNoFileComp
	})
	listVolumes.AddIntFlag(cloudapiv5.ArgMaxResults, cloudapiv5.ArgMaxResultsShort, 0, "The maximum number of elements to return")
	listVolumes.AddStringFlag(cloudapiv5.ArgOrderBy, "", "", "Limits results to those containing a matching value for a specific property")
	_ = listVolumes.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgOrderBy, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.VolumesFilters(), cobra.ShellCompDirectiveNoFileComp
	})
	listVolumes.AddStringSliceFlag(cloudapiv5.ArgFilters, cloudapiv5.ArgFiltersShort, []string{""}, "Limits results to those containing a matching value for a specific property. Use the following format to set filters: --filters KEY1=VALUE1,KEY2=VALUE2")
	_ = listVolumes.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgFilters, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.VolumesFilters(), cobra.ShellCompDirectiveNoFileComp
	})
	...
	*/
	...
	getVolumeCmd.AddStringFlag(cloudapiv5.ArgDataCenterId, "", "", cloudapiv5.DatacenterId, core.RequiredFlagOption())
	_ = getVolumeCmd.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgDataCenterId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.DataCentersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	getVolumeCmd.AddStringFlag(cloudapiv5.ArgServerId, "", "", cloudapiv5.ServerId, core.RequiredFlagOption())
	_ = getVolumeCmd.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgServerId, func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.ServersIds(os.Stderr, viper.GetString(core.GetFlagName(getVolumeCmd.NS, cloudapiv5.ArgDataCenterId))), cobra.ShellCompDirectiveNoFileComp
	})
	getVolumeCmd.AddStringFlag(cloudapiv5.ArgVolumeId, cloudapiv5.ArgIdShort, "", cloudapiv5.VolumeId, core.RequiredFlagOption())
	_ = getVolumeCmd.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgVolumeId, func(cmd *cobra.Command, ags []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.AttachedVolumesIds(os.Stderr, viper.GetString(core.GetFlagName(getVolumeCmd.NS, cloudapiv5.ArgDataCenterId)), viper.GetString(core.GetFlagName(getVolumeCmd.NS, cloudapiv5.ArgServerId))), cobra.ShellCompDirectiveNoFileComp
	})
...
* Volume Id`,
	...
	detachVolume.AddStringFlag(cloudapiv5.ArgDataCenterId, "", "", cloudapiv5.DatacenterId, core.RequiredFlagOption())
	_ = detachVolume.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgDataCenterId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.DataCentersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	detachVolume.AddStringFlag(cloudapiv5.ArgVolumeId, cloudapiv5.ArgIdShort, "", cloudapiv5.VolumeId, core.RequiredFlagOption())
	_ = detachVolume.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgVolumeId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.AttachedVolumesIds(os.Stderr, viper.GetString(core.GetFlagName(detachVolume.NS, cloudapiv5.ArgDataCenterId)),
			viper.GetString(core.GetFlagName(detachVolume.NS, cloudapiv5.ArgServerId))), cobra.ShellCompDirectiveNoFileComp
	})
	detachVolume.AddStringFlag(cloudapiv5.ArgServerId, "", "", cloudapiv5.ServerId, core.RequiredFlagOption())
	_ = detachVolume.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgServerId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.ServersIds(os.Stderr, viper.GetString(core.GetFlagName(detachVolume.NS, cloudapiv5.ArgDataCenterId))), cobra.ShellCompDirectiveNoFileComp
	})
...
func RunServerVolumeAttach(c *core.CommandConfig) error {
	...
	attachedVol, resp, err := c.CloudApiV5Services.Servers().AttachVolume(dcId, serverId, volumeId)
	if err != nil {
		return err
	}
...
func RunServerVolumesList(c *core.CommandConfig) error {
	...
	attachedVols, _, err := c.CloudApiV5Services.Servers().ListVolumes(dcId, serverId, listQueryParams)
	if err != nil {
		return err
	}
...
func RunServerVolumeGet(c *core.CommandConfig) error {
	...
	attachedVol, _, err := c.CloudApiV5Services.Servers().GetVolume(dcId, serverId, volumeId)
	if err != nil {
		return err
	}
	...
	for _, v := range vs {
		...
		if propertiesOk, ok := v.GetPropertiesOk(); ok && propertiesOk != nil {
			...
			if zoneOk, ok := propertiesOk.GetAvailabilityZoneOk(); ok && zoneOk != nil {
				volumePrint.AvailabilityZone = *zoneOk
			}
			...
			if imageAliasOk, ok := propertiesOk.GetImageAliasOk(); ok && imageAliasOk != nil {
				volumePrint.ImageAlias = *imageAliasOk
			}
