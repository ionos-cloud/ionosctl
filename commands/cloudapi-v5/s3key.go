...
func UserS3keyCmd() *core.Command {
	...
	*/
	...
	list.AddStringFlag(cloudapiv5.ArgUserId, "", "", cloudapiv5.UserId, core.RequiredFlagOption())
	_ = list.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgUserId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	*/
	...
	get.AddStringFlag(cloudapiv5.ArgUserId, "", "", cloudapiv5.UserId, core.RequiredFlagOption())
	...
	get.AddStringFlag(cloudapiv5.ArgS3KeyId, cloudapiv5.ArgIdShort, "", cloudapiv5.S3KeyId, core.RequiredFlagOption())
	_ = get.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgS3KeyId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.S3KeyIds(os.Stderr, viper.GetString(core.GetFlagName(get.NS, cloudapiv5.ArgUserId))), cobra.ShellCompDirectiveNoFileComp
	})
...
* User Id`,
	...
	create.AddStringFlag(cloudapiv5.ArgUserId, "", "", cloudapiv5.UserId, core.RequiredFlagOption())
	_ = create.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgUserId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.UsersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
...
* S3Key Active`,
	...
	update.AddStringFlag(cloudapiv5.ArgUserId, "", "", cloudapiv5.UserId, core.RequiredFlagOption())
	...
	update.AddBoolFlag(cloudapiv5.ArgS3KeyActive, "", false, "Enable or disable an User S3Key")
	update.AddStringFlag(cloudapiv5.ArgS3KeyId, cloudapiv5.ArgIdShort, "", cloudapiv5.S3KeyId, core.RequiredFlagOption())
	_ = update.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgS3KeyId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.S3KeyIds(os.Stderr, viper.GetString(core.GetFlagName(update.NS, cloudapiv5.ArgUserId))), cobra.ShellCompDirectiveNoFileComp
	})
	...
	*/
	...
	deleteCmd.AddStringFlag(cloudapiv5.ArgUserId, "", "", cloudapiv5.UserId, core.RequiredFlagOption())
	...
	deleteCmd.AddStringFlag(cloudapiv5.ArgS3KeyId, cloudapiv5.ArgIdShort, "", cloudapiv5.S3KeyId, core.RequiredFlagOption())
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgS3KeyId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.S3KeyIds(os.Stderr, viper.GetString(core.GetFlagName(deleteCmd.NS, cloudapiv5.ArgUserId))), cobra.ShellCompDirectiveNoFileComp
	})
	...
	deleteCmd.AddBoolFlag(cloudapiv5.ArgAll, cloudapiv5.ArgAllShort, false, "delete all the S3Keys of an User.")
...
func PreRunUserKeyIds(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(c.Command, c.NS, cloudapiv5.ArgUserId, cloudapiv5.ArgS3KeyId)
}

func PreRunUserKeyDelete(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlagsSets(c.Command, c.NS,
		[]string{cloudapiv5.ArgUserId, cloudapiv5.ArgS3KeyId},
		[]string{cloudapiv5.ArgUserId, cloudapiv5.ArgAll},
	)
}

func RunUserS3KeyList(c *core.CommandConfig) error {
	c.Printer.Verbose("Listing S3 Keys of User with ID: %v...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)))
	ss, resp, err := c.CloudApiV5Services.S3Keys().List(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)))
	if resp != nil {
...
func RunUserS3KeyGet(c *core.CommandConfig) error {
	c.Printer.Verbose("S3 keys with id: %v is getting...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgS3KeyId)))
	s, resp, err := c.CloudApiV5Services.S3Keys().Get(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)),
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgS3KeyId)),
	)
	if resp != nil {
		c.Printer.Verbose(config.RequestTimeMessage, resp.RequestTime)
	}
...
func RunUserS3KeyUpdate(c *core.CommandConfig) error {
	active := viper.GetBool(core.GetFlagName(c.NS, cloudapiv5.ArgS3KeyActive))
	...
	userId := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId))
	keyId := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgS3KeyId))
...
func RunUserS3KeyDelete(c *core.CommandConfig) error {
	...
	var err error
	var s3Keys resources.S3Keys
	userId := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId))
	s3KeyId := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgS3KeyId))
	allFlag := viper.GetBool(core.GetFlagName(c.NS, cloudapiv5.ArgAll))
	if allFlag {
		fmt.Printf("S3 keys to be deleted:\n")
		s3Keys, resp, err = c.CloudApiV5Services.S3Keys().List(userId)
		if err != nil {
			return err
		}
