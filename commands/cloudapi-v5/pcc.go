...
func PccCmd() *core.Command {
	...
	*/
	...
	get.AddStringFlag(cloudapiv5.ArgPccId, cloudapiv5.ArgIdShort, "", cloudapiv5.PccId, core.RequiredFlagOption())
	_ = get.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgPccId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	*/
	...
	create.AddStringFlag(cloudapiv5.ArgName, cloudapiv5.ArgNameShort, "Unnamed PrivateCrossConnect", "The name for the Private Cross-Connect")
	create.AddStringFlag(cloudapiv5.ArgDescription, cloudapiv5.ArgDescriptionShort, "", "The description for the Private Cross-Connect")
...
* Pcc Id`,
	...
	update.AddStringFlag(cloudapiv5.ArgName, cloudapiv5.ArgNameShort, "", "The name for the Private Cross-Connect")
	update.AddStringFlag(cloudapiv5.ArgDescription, cloudapiv5.ArgDescriptionShort, "", "The description for the Private Cross-Connect")
	update.AddStringFlag(cloudapiv5.ArgPccId, cloudapiv5.ArgIdShort, "", cloudapiv5.PccId, core.RequiredFlagOption())
...
* Pcc Id`,
	...
	deleteCmd.AddStringFlag(cloudapiv5.ArgPccId, cloudapiv5.ArgIdShort, "", cloudapiv5.PccId, core.RequiredFlagOption())
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgPccId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.PccsIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
...
func RunPccGet(c *core.CommandConfig) error {
	c.Printer.Verbose("Private cross connect with id: %v is getting...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgPccId)))
	u, resp, err := c.CloudApiV5Services.Pccs().Get(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgPccId)))
	if resp != nil {
		c.Printer.Verbose(config.RequestTimeMessage, resp.RequestTime)
	}
	if err != nil {
...
func RunPccUpdate(c *core.CommandConfig) error {
	oldPcc, resp, err := c.CloudApiV5Services.Pccs().Get(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgPccId)))
	if err != nil {
		return err
	}
...
func getPccInfo(oldUser *resources.PrivateCrossConnect, c *core.CommandConfig) *resources.PrivateCrossConnectProperties {
	...
	if properties, ok := oldUser.GetPropertiesOk(); ok && properties != nil {
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgName)) {
			namePcc = viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgName))
...
func peers() *core.Command {
	...
	*/
	...
	listPeers.AddStringFlag(cloudapiv5.ArgPccId, "", "", cloudapiv5.PccId)
...
func RunPccPeersList(c *core.CommandConfig) error {
	c.Printer.Verbose("Getting Peers from Private Cross-Connect with ID: %v...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgPccId)))
	u, resp, err := c.CloudApiV5Services.Pccs().GetPeers(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgPccId)))
	if resp != nil {
		c.Printer.Verbose(config.RequestTimeMessage, resp.RequestTime)
	}
	if err != nil {
