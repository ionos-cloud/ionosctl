...
func TestPreImageId(t *testing.T) {
	...
	core.PreCmdConfigTest(t, w, func(cfg *core.PreCommandConfig) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgImageId), testImageVar)
...
func TestRunImageListSort(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgQuiet, false)
		viper.Set(config.ArgServerUrl, config.DefaultApiURL)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgLocation), testImageVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgLicenceType), testImageVar)
...
func TestRunImageGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgServerUrl, config.DefaultApiURL)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgImageId), testImageVar)
		rm.CloudApiV5Mocks.Image.EXPECT().Get(testImageVar).Return(&testImage, &testResponse, nil)
		err := RunImageGet(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunImageGetErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgServerUrl, config.DefaultApiURL)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgImageId), testImageVar)
		rm.CloudApiV5Mocks.Image.EXPECT().Get(testImageVar).Return(&testImage, nil, testImageErr)
		err := RunImageGet(cfg)
		assert.Error(t, err)
	})
}
