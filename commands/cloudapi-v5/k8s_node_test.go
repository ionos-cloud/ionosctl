...
func TestPreRunK8sClusterNodesIds(t *testing.T) {
	...
	core.PreCmdConfigTest(t, w, func(cfg *core.PreCommandConfig) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodeId), testNodeVar)
		err := PreRunK8sClusterNodesIds(cfg)
		assert.NoError(t, err)
	})
}
func TestPreRunK8sClusterNodesDelete(t *testing.T) {
	var b bytes.Buffer
	w := bufio.NewWriter(&b)
	core.PreCmdConfigTest(t, w, func(cfg *core.PreCommandConfig) {
		viper.Reset()
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodeId), testNodeVar)
		err := PreRunK8sClusterNodesDelete(cfg)
		assert.NoError(t, err)
	})
}

func TestPreRunK8sClusterNodesDeleteAll(t *testing.T) {
	var b bytes.Buffer
	w := bufio.NewWriter(&b)
	core.PreCmdConfigTest(t, w, func(cfg *core.PreCommandConfig) {
		viper.Reset()
...
func TestRunK8sNodeList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, resources.ListQueryParams{}).Return(nodesTest, &testResponse, nil)
...
func TestRunK8sNodeListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, testListQueryParam).Return(resources.K8sNodes{}, &testResponse, nil)
		err := RunK8sNodeList(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunK8sNodeListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, resources.ListQueryParams{}).Return(nodesTest, nil, testNodeErr)
		err := RunK8sNodeList(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sNodeGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetNode(testNodeVar, testNodeVar, testNodeVar).Return(&nodeTestGet, &testResponse, nil)
...
func TestRunK8sNodeGetWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetNode(testNodeVar, testNodeVar, testNodeVar).Return(&nodeTestGet, nil, testNodeErr)
		err := RunK8sNodeGet(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sNodeGetWait(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetNode(testNodeVar, testNodeVar, testNodeVar).Return(&nodeTestGet, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetNode(testNodeVar, testNodeVar, testNodeVar).Return(&nodeTestGet, nil, nil)
		err := RunK8sNodeGet(cfg)
		assert.NoError(t, err)
	})
...
func TestRunK8sNodeRecreate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().RecreateNode(testNodeVar, testNodeVar, testNodeVar).Return(&testResponse, nil)
...
func TestRunK8sNodeRecreateErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().RecreateNode(testNodeVar, testNodeVar, testNodeVar).Return(nil, testNodeErr)
		err := RunK8sNodeRecreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sNodeRecreateAskForConfirm(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV5Mocks.K8s.EXPECT().RecreateNode(testNodeVar, testNodeVar, testNodeVar).Return(nil, nil)
		err := RunK8sNodeRecreate(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunK8sNodeDeleteAll(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, resources.ListQueryParams{}).Return(nodesTestList, &testResponse, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteNode(testNodeVar, testNodeVar, testNodeVar).Return(&testResponse, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteNode(testNodeVar, testNodeVar, testNodeVar).Return(&testResponse, nil)
		err := RunK8sNodeDelete(cfg)
		assert.NoError(t, err)
...
func TestRunK8sNodeDeleteAllListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, resources.ListQueryParams{}).Return(nodesTestList, &testResponse, testNodeErr)
		err := RunK8sNodeDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sNodeDeleteAllItemsErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, resources.ListQueryParams{}).Return(resources.K8sNodes{}, &testResponse, nil)
		err := RunK8sNodeDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sNodeDeleteAllLenErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, resources.ListQueryParams{}).Return(
...
func TestRunK8sNodeDeleteAllErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListNodes(testNodeVar, testNodeVar, resources.ListQueryParams{}).Return(nodesTestList, &testResponse, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteNode(testNodeVar, testNodeVar, testNodeVar).Return(&testResponse, testNodeErr)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteNode(testNodeVar, testNodeVar, testNodeVar).Return(&testResponse, testNodeErr)
		err := RunK8sNodeDelete(cfg)
		assert.Error(t, err)
	})
...
func TestRunK8sNodeDeleteErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sNodePoolId), testNodeVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteNode(testNodeVar, testNodeVar, testNodeVar).Return(nil, testNodeErr)
		err := RunK8sNodeDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sNodeDeleteAskForConfirm(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testNodeVar)
		cfg.Stdin = bytes.NewReader([]byte("YES\n"))
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteNode(testNodeVar, testNodeVar, testNodeVar).Return(nil, nil)
		err := RunK8sNodeDelete(cfg)
		assert.NoError(t, err)
	})
}
