...
func ImageCmd() *core.Command {
	...
	*/
	...
	get.AddStringFlag(cloudapiv5.ArgImageId, cloudapiv5.ArgIdShort, "", cloudapiv5.ImageId, core.RequiredFlagOption())
	_ = get.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgImageId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
func RunImageList(c *core.CommandConfig) error {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgLocation)) {
		images = sortImagesByLocation(images, viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgLocation)))
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgLicenceType)) {
		images = sortImagesByLicenceType(images, strings.ToUpper(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgLicenceType))))
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgType)) {
		images = sortImagesByType(images, strings.ToUpper(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgType))))
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgImageAlias)) {
		images = sortImagesByAlias(images, viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgImageAlias)))
	}
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgLatest)) {
		images = sortImagesByTime(images, viper.GetInt(core.GetFlagName(c.NS, cloudapiv5.ArgLatest)))
	}
...
func RunImageGet(c *core.CommandConfig) error {
	c.Printer.Verbose("Image with id: %v is getting...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgImageId)))
	img, resp, err := c.CloudApiV5Services.Images().Get(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgImageId)))
	if resp != nil {
		c.Printer.Verbose(config.RequestTimeMessage, resp.RequestTime)
	}
	if err != nil {
