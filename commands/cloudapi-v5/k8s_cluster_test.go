...
func TestRunK8sClusterList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(clusters, &testResponse, nil)
...
func TestRunK8sClusterListQueryParams(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(testListQueryParam).Return(resources.K8sClusters{}, &testResponse, nil)
		err := RunK8sClusterList(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunK8sClusterListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgServerUrl, config.DefaultApiURL)
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(clusters, nil, testClusterErr)
		err := RunK8sClusterList(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, &testResponse, nil)
...
func TestRunK8sClusterGetWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, testClusterErr)
		err := RunK8sClusterGet(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterGetWait(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForState), true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, nil)
		err := RunK8sClusterGet(cfg)
		assert.NoError(t, err)
	})
...
func TestRunK8sClusterCreate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgName), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sVersion), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().CreateCluster(clusterTestPost).Return(&clusterTest, &testResponse, nil)
...
func TestRunK8sClusterCreateWaitIdErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().CreateCluster(clusterTestPost).Return(&clusterTest, nil, nil)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterCreateWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().CreateCluster(clusterTestPost).Return(&clusterTestId, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, testClusterErr)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
...
func TestRunK8sClusterCreateWaitReqErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().CreateCluster(clusterTestPost).Return(&clusterTestId, &testResponse, nil)
...
func TestRunK8sClusterCreateVersion(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().GetVersion().Return(testClusterVar, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().CreateCluster(clusterTestPost).Return(&clusterTest, nil, nil)
		err := RunK8sClusterCreate(cfg)
		assert.NoError(t, err)
	})
...
func TestRunK8sClusterCreateVersionErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().GetVersion().Return(testClusterVar, nil, testClusterErr)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterCreateResponseErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().CreateCluster(clusterTestPost).Return(&clusterTest, &testResponseErr, nil)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterCreateErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().CreateCluster(clusterTestPost).Return(&clusterTest, nil, testClusterErr)
		err := RunK8sClusterCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterUpdate(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForState), false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testClusterVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgName), testClusterNewVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sVersion), testClusterNewVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sMaintenanceTime), testClusterNewVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sMaintenanceDay), testClusterNewVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), testClusterNewVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterNewVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterNewTestPut).Return(&clusterNew, &testResponse, nil)
...
func TestRunK8sClusterUpdateWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterNewVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterNewTestPut).Return(&clusterNew, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, testClusterErr)
		err := RunK8sClusterUpdate(cfg)
		assert.Error(t, err)
...
func TestRunK8sClusterUpdateOldUser(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForState), false)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTest, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterTestPut).Return(&clusterTest, nil, nil)
		err := RunK8sClusterUpdate(cfg)
		assert.NoError(t, err)
	})
}
...
func TestRunK8sClusterUpdateErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sMaintenanceTime), testClusterNewVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sMaintenanceDay), testClusterNewVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgS3Bucket), []string{testClusterNewVar})
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgApiSubnets), []string{testClusterNewVar})
		rm.CloudApiV5Mocks.K8s.EXPECT().GetCluster(testClusterVar).Return(&clusterTestGet, nil, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().UpdateCluster(testClusterVar, clusterNewTestPut).Return(&clusterNew, nil, testClusterErr)
		err := RunK8sClusterUpdate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterDeleteAll(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgForce, true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(clustersList, &testResponse, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteCluster(testClusterVar).Return(&testResponse, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteCluster(testClusterVar).Return(&testResponse, nil)
		err := RunK8sClusterDelete(cfg)
		assert.NoError(t, err)
...
func TestRunK8sClusterDeleteAllListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgForce, true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(clustersList, &testResponse, testClusterErr)
		err := RunK8sClusterDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterDeleteAllItemsErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgForce, true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(resources.K8sClusters{}, &testResponse, nil)
		err := RunK8sClusterDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterDeleteAllLenErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgForce, true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(
...
func TestRunK8sClusterDeleteAllErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgAll), true)
		rm.CloudApiV5Mocks.K8s.EXPECT().ListClusters(resources.ListQueryParams{}).Return(clustersList, &testResponse, nil)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteCluster(testClusterVar).Return(&testResponse, testClusterErr)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteCluster(testClusterVar).Return(&testResponse, nil)
		err := RunK8sClusterDelete(cfg)
		assert.Error(t, err)
	})
...
func TestRunK8sClusterDeleteErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgForce, true)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgK8sClusterId), testClusterVar)
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteCluster(testClusterVar).Return(nil, testClusterErr)
		err := RunK8sClusterDelete(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunK8sClusterDeleteAskForConfirm(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		rm.CloudApiV5Mocks.K8s.EXPECT().DeleteCluster(testClusterVar).Return(nil, nil)
		err := RunK8sClusterDelete(cfg)
		assert.NoError(t, err)
	})
}
