...
func TestRunServerStartWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgServerId), testServerVar)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForRequest), true)
		rm.CloudApiV5Mocks.Server.EXPECT().Start(testServerVar, testServerVar).Return(nil, nil)
		err := RunServerStart(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunServerStopWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgServerId), testServerVar)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForRequest), true)
		rm.CloudApiV5Mocks.Server.EXPECT().Stop(testServerVar, testServerVar).Return(nil, nil)
		err := RunServerStop(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunServerRebootWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgServerId), testServerVar)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgWaitForRequest), true)
		rm.CloudApiV5Mocks.Server.EXPECT().Reboot(testServerVar, testServerVar).Return(nil, nil)
		err := RunServerReboot(cfg)
		assert.Error(t, err)
	})
}
