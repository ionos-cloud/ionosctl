...
func UserCmd() *core.Command {
	...
	*/
	...
	get.AddStringFlag(cloudapiv5.ArgUserId, cloudapiv5.ArgIdShort, "", cloudapiv5.UserId, core.RequiredFlagOption())
	_ = get.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgUserId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
* Password`,
	...
	create.AddStringFlag(cloudapiv5.ArgFirstName, "", "", "The first name for the User", core.RequiredFlagOption())
	create.AddStringFlag(cloudapiv5.ArgLastName, "", "", "The last name for the User", core.RequiredFlagOption())
	create.AddStringFlag(cloudapiv5.ArgEmail, cloudapiv5.ArgEmailShort, "", "The email for the User", core.RequiredFlagOption())
	create.AddStringFlag(cloudapiv5.ArgPassword, cloudapiv5.ArgPasswordShort, "", "The password for the User (must be at least 5 characters long)", core.RequiredFlagOption())
	create.AddBoolFlag(cloudapiv5.ArgAdmin, "", false, "Assigns the User to have administrative rights")
	create.AddBoolFlag(cloudapiv5.ArgForceSecAuth, "", false, "Indicates if secure (two-factor) authentication should be forced for the User")
...
* User Id`,
	...
	update.AddStringFlag(cloudapiv5.ArgFirstName, "", "", "The first name for the User")
	update.AddStringFlag(cloudapiv5.ArgLastName, "", "", "The last name for the User")
	update.AddStringFlag(cloudapiv5.ArgEmail, cloudapiv5.ArgEmailShort, "", "The email for the User")
	update.AddStringFlag(cloudapiv5.ArgPassword, cloudapiv5.ArgPasswordShort, "", "The password for the User (must be at least 5 characters long)")
	update.AddBoolFlag(cloudapiv5.ArgAdmin, "", false, "Assigns the User to have administrative rights")
	update.AddBoolFlag(cloudapiv5.ArgForceSecAuth, "", false, "Indicates if secure (two-factor) authentication should be forced for the User")
	update.AddStringFlag(cloudapiv5.ArgUserId, cloudapiv5.ArgIdShort, "", cloudapiv5.UserId, core.RequiredFlagOption())
	_ = update.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgUserId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.UsersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
...
* User Id`,
	...
	deleteCmd.AddStringFlag(cloudapiv5.ArgUserId, cloudapiv5.ArgIdShort, "", cloudapiv5.UserId, core.RequiredFlagOption())
...
func PreRunUserNameEmailPwd(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlags(c.Command, c.NS, cloudapiv5.ArgFirstName, cloudapiv5.ArgLastName, cloudapiv5.ArgEmail, cloudapiv5.ArgPassword)
}
...
func RunUserList(c *core.CommandConfig) error {
	...
	users, resp, err := c.CloudApiV5Services.Users().List(listQueryParams)
	if resp != nil {
...
func RunUserGet(c *core.CommandConfig) error {
	c.Printer.Verbose("User with id: %v is getting...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)))
	u, resp, err := c.CloudApiV5Services.Users().Get(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)))
	if resp != nil {
		c.Printer.Verbose(config.RequestTimeMessage, resp.RequestTime)
	}
	if err != nil {
...
func RunUserCreate(c *core.CommandConfig) error {
	firstname := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgFirstName))
	lastname := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgLastName))
	...
	secureAuth := viper.GetBool(core.GetFlagName(c.NS, cloudapiv5.ArgForceSecAuth))
	admin := viper.GetBool(core.GetFlagName(c.NS, cloudapiv5.ArgAdmin))
	...
	u, resp, err := c.CloudApiV5Services.Users().Create(newUser)
	if resp != nil {
...
func RunUserUpdate(c *core.CommandConfig) error {
	oldUser, resp, err := c.CloudApiV5Services.Users().Get(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)))
	if resp != nil {
		c.Printer.Verbose(config.RequestTimeMessage, resp.RequestTime)
	}
	if err != nil {
	...
	c.Printer.Verbose("Updating User with ID: %v...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)))
	userUpd, resp, err := c.CloudApiV5Services.Users().Update(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgUserId)), *newUser)
	if err != nil {
		return err
	}
...
func getUserInfo(oldUser *resources.User, c *core.CommandConfig) *resources.UserPut {
	...
	if properties, ok := oldUser.GetPropertiesOk(); ok && properties != nil {
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgFirstName)) {
			firstName := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgFirstName))
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgLastName)) {
			lastName := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgLastName))
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgForceSecAuth)) {
			forceSecureAuth := viper.GetBool(core.GetFlagName(c.NS, cloudapiv5.ArgForceSecAuth))
		...
		if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgAdmin)) {
			admin := viper.GetBool(core.GetFlagName(c.NS, cloudapiv5.ArgAdmin))
...
func GroupUserCmd() *core.Command {
	...
	*/
	...
	listUsers.AddStringFlag(cloudapiv5.ArgGroupId, "", "", cloudapiv5.GroupId, core.RequiredFlagOption())
	_ = listUsers.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgGroupId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	*/
	...
	addUser.AddStringFlag(cloudapiv5.ArgGroupId, "", "", cloudapiv5.GroupId, core.RequiredFlagOption())
	_ = addUser.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgGroupId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.GroupsIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	addUser.AddStringFlag(cloudapiv5.ArgUserId, cloudapiv5.ArgIdShort, "", cloudapiv5.UserId, core.RequiredFlagOption())
	_ = addUser.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgUserId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.UsersIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	...
	*/
	...
	removeUser.AddStringFlag(cloudapiv5.ArgGroupId, "", "", cloudapiv5.GroupId, core.RequiredFlagOption())
	_ = removeUser.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgGroupId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.GroupsIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	removeUser.AddStringFlag(cloudapiv5.ArgUserId, cloudapiv5.ArgIdShort, "", cloudapiv5.UserId, core.RequiredFlagOption())
...
func RunGroupUserList(c *core.CommandConfig) error {
	users, resp, err := c.CloudApiV5Services.Groups().ListUsers(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgGroupId)))
	if resp != nil {
		c.Printer.Verbose(config.RequestTimeMessage, resp.RequestTime)
	}
	if err != nil {
