...
func LanCmd() *core.Command {
	...
	*/
	...
	get.AddStringFlag(cloudapiv5.ArgLanId, cloudapiv5.ArgIdShort, "", cloudapiv5.LanId, core.RequiredFlagOption())
	_ = get.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgLanId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.LansIds(os.Stderr, viper.GetString(core.GetFlagName(get.NS, cloudapiv5.ArgDataCenterId))), cobra.ShellCompDirectiveNoFileComp
	})
...
* Data Center Id`,
	...
	create.AddStringFlag(cloudapiv5.ArgName, cloudapiv5.ArgNameShort, "Unnamed LAN", "The name of the LAN")
	create.AddBoolFlag(cloudapiv5.ArgPublic, cloudapiv5.ArgPublicShort, cloudapiv5.DefaultPublic, "Indicates if the LAN faces the public Internet (true) or not (false). E.g.: --public=true, --public=false")
	create.AddStringFlag(cloudapiv5.ArgPccId, "", "", "The unique Id of the Private Cross-Connect the LAN will connect to")
	_ = create.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgPccId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
* LAN Id`,
	...
	update.AddStringFlag(cloudapiv5.ArgLanId, cloudapiv5.ArgIdShort, "", cloudapiv5.LanId, core.RequiredFlagOption())
	...
	update.AddStringFlag(cloudapiv5.ArgName, cloudapiv5.ArgNameShort, "", "The name of the LAN")
	update.AddStringFlag(cloudapiv5.ArgPccId, "", "", "The unique Id of the Private Cross-Connect the LAN will connect to")
	_ = update.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgPccId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return completer.PccsIds(os.Stderr), cobra.ShellCompDirectiveNoFileComp
	})
	update.AddBoolFlag(cloudapiv5.ArgPublic, cloudapiv5.ArgPublicShort, cloudapiv5.DefaultPublic, "Public option for LAN. E.g.: --public=true, --public=false")
...
* LAN Id`,
	...
	deleteCmd.AddStringFlag(cloudapiv5.ArgLanId, cloudapiv5.ArgIdShort, "", cloudapiv5.LanId, core.RequiredFlagOption())
	_ = deleteCmd.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgLanId, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
func RunLanGet(c *core.CommandConfig) error {
	c.Printer.Verbose("Lan with id: %v from Datacenter with id: %v is getting...",
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgLanId)), viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)))
	l, resp, err := c.CloudApiV5Services.Lans().Get(
...
func RunLanCreate(c *core.CommandConfig) error {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgPccId)) {
		pcc := viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgPccId))
...
func RunLanUpdate(c *core.CommandConfig) error {
	...
	c.Printer.Verbose("Updating LAN with ID: %v from Datacenter with ID: %v...",
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgLanId)), viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)))
	lanUpdated, resp, err := c.CloudApiV5Services.Lans().Update(
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgDataCenterId)),
		viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgLanId)),
		input,
	)
