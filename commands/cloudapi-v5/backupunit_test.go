...
func TestRunBackupUnitList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		rm.CloudApiV5Mocks.BackupUnit.EXPECT().List().Return(backupUnits, &testResponse, nil)
...
func TestRunBackupUnitListErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(config.ArgQuiet, false)
		viper.Set(config.ArgOutput, config.DefaultOutputFormat)
		rm.CloudApiV5Mocks.BackupUnit.EXPECT().List().Return(backupUnits, nil, testBackupUnitErr)
		err := RunBackupUnitList(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunBackupUnitCreateWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgEmail), testBackupUnitVar)
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgPassword), testBackupUnitVar)
		rm.CloudApiV5Mocks.BackupUnit.EXPECT().Create(backupUnitTest).Return(&backupUnitTest, nil, nil)
		err := RunBackupUnitCreate(cfg)
		assert.Error(t, err)
	})
}
...
func TestRunBackupUnitUpdateWaitErr(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocksTest) {
		...
		viper.Set(core.GetFlagName(cfg.NS, cloudapiv5.ArgEmail), testBackupUnitNewVar)
		viper.Set(config.ArgServerUrl, config.DefaultApiURL)
		rm.CloudApiV5Mocks.BackupUnit.EXPECT().Update(testBackupUnitVar, backupUnitProperties).Return(&backupUnitNew, nil, nil)
		err := RunBackupUnitUpdate(cfg)
		assert.Error(t, err)
	})
}
