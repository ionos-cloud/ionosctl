...
func ContractCmd() *core.Command {
	...
	*/
	...
	get.AddStringFlag(cloudapiv5.ArgResourceLimits, "", "", "Specify Resource Limits to see details about it")
	_ = get.Command.RegisterFlagCompletionFunc(cloudapiv5.ArgResourceLimits, func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
...
func RunContractGet(c *core.CommandConfig) error {
	c.Printer.Verbose("Getting properties from contract regarding resource limits: %v...", viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgResourceLimits)))
	contractResource, resp, err := c.CloudApiV5Services.Contracts().Get()
	if resp != nil {
	...
	if viper.IsSet(core.GetFlagName(c.NS, cloudapiv5.ArgResourceLimits)) {
		switch strings.ToUpper(viper.GetString(core.GetFlagName(c.NS, cloudapiv5.ArgResourceLimits))) {
		case "CORES":
