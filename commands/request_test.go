...
func TestRunRequestList(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		rm.Request.EXPECT().List().Return(testRequests, &testResponse, nil)
...
func TestRunRequestGet(t *testing.T) {
	...
	core.CmdConfigTest(t, w, func(cfg *core.CommandConfig, rm *core.ResourcesMocks) {
		...
		viper.Set(config.ArgVerbose, true)
		viper.Set(core.GetFlagName(cfg.NS, config.ArgRequestId), testRequestVar)
		req := v6.Request{Request: rq}
		rm.Request.EXPECT().Get(testRequestVar).Return(&req, &testResponse, nil)
