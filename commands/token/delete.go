...
* Token Id/Token/CURRENT/EXPIRED/ALL`,
	...
	cmd.AddBoolFlag(
		authservice.ArgCurrent, authservice.ArgCurrentShort, false,
		"Delete the Token that is currently used. This requires a token to be set for authentication via environment variable IONOS_TOKEN or via config file",
		core.RequiredFlagOption(),
	)
	cmd.AddBoolFlag(
		authservice.ArgExpired, authservice.ArgExpiredShort, false, "Delete the Tokens that are currently expired",
		core.RequiredFlagOption(),
	)
	cmd.AddBoolFlag(
		authservice.ArgAll, authservice.ArgAllShort, false, "Delete the Tokens under your account",
		core.RequiredFlagOption(),
	)
	cmd.AddIntFlag(
		authservice.ArgContractNo, "", 0,
		"Users with multiple contracts must provide the contract number, for which the tokens are deleted",
	)
...
func preRunTokenDelete(c *core.PreCommandConfig) error {
	return core.CheckRequiredFlagsSets(
		c.Command, c.NS, []string{authservice.ArgTokenId}, []string{authservice.ArgCurrent},
		[]string{authservice.ArgExpired}, []string{authservice.ArgAll}, []string{authservice.ArgToken},
	)
}
...
func runTokenDelete(c *core.CommandConfig) error {
	...
	if viper.IsSet(core.GetFlagName(c.NS, authservice.ArgCurrent)) && viper.GetBool(
		core.GetFlagName(
			c.NS, authservice.ArgCurrent,
		),
	) {
		return runTokenDeleteCurrent(c)
	}
	if viper.IsSet(core.GetFlagName(c.NS, authservice.ArgExpired)) && viper.GetBool(
		core.GetFlagName(
			c.NS, authservice.ArgExpired,
		),
	) {
		return runTokenDeleteExpired(c)
	}
	if viper.IsSet(core.GetFlagName(c.NS, authservice.ArgAll)) && viper.GetBool(
		core.GetFlagName(
			c.NS, authservice.ArgAll,
		),
	) {
...
func runTokenDeleteAll(c *core.CommandConfig) error {
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), fmt.Sprintf("delete all tokens"),
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByCriteria(
		"ALL", viper.GetInt32(core.GetFlagName(c.NS, authservice.ArgContractNo)),
	)
	if err != nil {
		return err
	}
		...
		if success, ok := tokenResponse.GetSuccessOk(); ok && success != nil {
			if *success {
				fmt.Fprintf(
					...
					jsontabwriter.GenerateLogOutput("Status: all tokens have been successfully deleted"),
				)
				return nil
			}
		}
...
func runTokenDeleteExpired(c *core.CommandConfig) error {
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), fmt.Sprintf("delete expired tokens"),
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByCriteria(
		"EXPIRED", viper.GetInt32(core.GetFlagName(c.NS, authservice.ArgContractNo)),
	)
	if err != nil {
		return err
	}
	...
	if tokenResponse != nil {
		if success, ok := tokenResponse.GetSuccessOk(); ok && success != nil {
			if *success {
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateLogOutput("Status: expired tokens have been successfully deleted"),
				)
				return nil
			}
		}
...
func runTokenDeleteCurrent(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), fmt.Sprintf("delete CURRENT token"),
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByCriteria(
		"CURRENT", viper.GetInt32(core.GetFlagName(c.NS, authservice.ArgContractNo)),
	)
	if err != nil {
		return err
	}
	...
	if tokenResponse != nil {
		if success, ok := tokenResponse.GetSuccessOk(); ok && success != nil {
			if *success {
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateLogOutput("Status: current token have been successfully deleted"),
				)
				return nil
			}
		}
...
func runTokenDeleteById(c *core.CommandConfig) error {
	...
	if !confirm.FAsk(
		c.Command.Command.InOrStdin(), fmt.Sprintf("delete token with ID: %s", tokenId),
		viper.GetBool(core.GetFlagName(c.NS, constants.ArgForce)),
	) {
		return fmt.Errorf(confirm.UserDenied)
	}
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByID(
		tokenId, viper.GetInt32(core.GetFlagName(c.NS, authservice.ArgContractNo)),
	)
	if err != nil {
		return err
	}
	if tokenResponse != nil {
		if success, ok := tokenResponse.GetSuccessOk(); ok && success != nil {
			if *success {
				fmt.Fprintf(
					c.Command.Command.ErrOrStderr(),
					jsontabwriter.GenerateLogOutput("Status: token has been successfully deleted"),
				)
				return nil
			}
		}
...
func runTokenDeleteByToken(c *core.CommandConfig) error {
	...
	tokenResponse, _, err := c.AuthV1Services.Tokens().DeleteByID(
		fmt.Sprintf("%v", tokenId), viper.GetInt32(core.GetFlagName(c.NS, authservice.ArgContractNo)),
	)
	if err != nil {
		return err
	}
