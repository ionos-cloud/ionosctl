...
### Added
- `dbaas in-memory-db-v2` commands: a new command tree backed by the In-Memory DB v3 API (`sdk-go-bundle/.../inmemorydb/v3`), mirroring the `dbaas postgres-v2` layout. Replica sets are now modeled as clusters. Includes `cluster` (list/create/get/update/delete/restore), `snapshot` (list/get) with `snapshot location` (list/get), and `version` (list/get). Snapshot scheduling is configurable at create time via `--snapshot-location`, `--retention-days`, and `--snapshot-hours`, and passwords are hashed client-side (SHA-256) before sending. The existing `dbaas in-memory-db` tree (v2 API) is unchanged.
