...
Within each layer, a token takes precedence over a username and password combination. For instance, if a token and a username/password pair are both defined in environment variables, ionosctl will prioritize the token. However, higher layers can override the use of a token from a lower layer. For example, username and password environment variables will supersede a token found in the config file.`
	...
	FlagLogProtocol      = "log-protocol"
	FlagLogLabels        = "log-labels"
	FlagLogType          = "log-type"
	FlagLogRetentionTime = "log-retention-time"

	FlagDomain                          = "domain"
	FlagDistributionID                  = "distribution-id"
	FlagDistributionRoutingRules        = "routing-rules"
	FlagDistributionRoutingRulesExample = "routing-rules-example"

	FlagBrokerAddresses   = "broker-addresses"
	FlagPartitions        = "partitions"
	FlagReplicationFactor = "replication-factor"
	FlagRetentionTime     = "retention-time"
	FlagSegmentBytes      = "segment-bytes"
	FlagTopicId           = "topic-id"
