...
// It respects --output, --quiet, --query, --wait, and --no-headers flags.
...
func (t *Table) Render(visibleCols []string) (string, error) {
	...
	// When --wait is set and we're not in the re-render pass, defer output.
	// The table is stored in globalwait so Execute() can re-render after waiting.
	if viper.GetBool(constants.ArgWait) && !globalwait.IsRerendering() {
		globalwait.CaptureHref(t.raw)
