...
// WithURLOverride adds only the server URL flag and populates viper with the final URL.
// It does not touch product-specific config or client overrides.
func WithURLOverride(c *Command, fallbackURL string) *Command {
	if fallbackURL == "" {
		fallbackURL = constants.DefaultApiURL
	}
	...
	c.Command.PersistentFlags().StringP(
		constants.ArgServerUrl, constants.ArgServerUrlShort, fallbackURL,
		fmt.Sprintf("Override default host URL. Preferred over the env var '%s'", constants.EnvServerUrl),
	)

	originalPreRun := c.Command.PersistentPreRunE
	c.Command.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		url := findOverridenURLNoConfig(cmd)
		if url == "" {
			url = fallbackURL
		}
		viper.Set(constants.ArgServerUrl, url)
...
// findOverridenURLNoConfig mirrors the flag/env portion of findOverridenURL
// but does not attempt to read product-specific overrides from client.Config.
func findOverridenURLNoConfig(cmd *cobra.Command) string {
	// Check if the --server-url flag is set
	if cmd.Flags().Changed(constants.ArgServerUrl) {
		serverURL, _ := cmd.Flags().GetString(constants.ArgServerUrl)
		viper.Set(constants.ArgServerUrl, serverURL)
