...
func TestNewK8sService(t *testing.T) {
	...
	t.Run("getcluster_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, _, err := k8sSvc.GetCluster(testK8sResourceVar, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("createcluster_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, _, err := k8sSvc.CreateCluster(K8sClusterForPost{}, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("updatecluster_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, _, err := k8sSvc.UpdateCluster(testK8sResourceVar, K8sClusterForPut{}, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("deletecluster_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, err := k8sSvc.DeleteCluster(testK8sResourceVar, QueryParams{})
		assert.Error(t, err)
	})
	...
	t.Run("getnodepool_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, _, err := k8sSvc.GetNodePool(testK8sResourceVar, testK8sResourceVar, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("createnodepool_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, _, err := k8sSvc.CreateNodePool(testK8sResourceVar, K8sNodePoolForPost{}, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("updatenodepool_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, _, err := k8sSvc.UpdateNodePool(testK8sResourceVar, testK8sResourceVar, K8sNodePoolForPut{}, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("deletenodepool_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, err := k8sSvc.DeleteNodePool(testK8sResourceVar, testK8sResourceVar, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("recreatenode_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, err := k8sSvc.RecreateNode(testK8sResourceVar, testK8sResourceVar, testK8sResourceVar, QueryParams{})
		assert.Error(t, err)
	})
	t.Run("getnode_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, _, err := k8sSvc.GetNode(testK8sResourceVar, testK8sResourceVar, testK8sResourceVar, QueryParams{})
		assert.Error(t, err)
	})
	...
	t.Run("deletenode_k8s_error", func(t *testing.T) {
		svc := getTestClient(t)
		k8sSvc := NewK8sService(svc, ctx)
		_, err := k8sSvc.DeleteNode(testK8sResourceVar, testK8sResourceVar, testK8sResourceVar, QueryParams{})
		assert.Error(t, err)
	})
