...
func TestNewClustersService(t *testing.T) {
	...
	t.Run("list_cluster_error", func(t *testing.T) {
		svc := getTestClient(t)
		clusterSvc := NewClustersService(svc.Get(), ctx)
		_, _, err := clusterSvc.List("")
		assert.Error(t, err)
	})
	t.Run("list_cluster_filter_error", func(t *testing.T) {
		svc := getTestClient(t)
		clusterSvc := NewClustersService(svc.Get(), ctx)
		_, _, err := clusterSvc.List(testClusterResourceVar)
		assert.Error(t, err)
	})
	t.Run("get_clusters_error", func(t *testing.T) {
		svc := getTestClient(t)
		clusterSvc := NewClustersService(svc.Get(), ctx)
		_, _, err := clusterSvc.Get(testClusterResourceVar)
		assert.Error(t, err)
	})
	t.Run("create_cluster_error", func(t *testing.T) {
		svc := getTestClient(t)
		clusterSvc := NewClustersService(svc.Get(), ctx)
		_, _, err := clusterSvc.Create(CreateClusterRequest{})
		assert.Error(t, err)
	})
	t.Run("update_cluster_error", func(t *testing.T) {
		svc := getTestClient(t)
		clusterSvc := NewClustersService(svc.Get(), ctx)
		_, _, err := clusterSvc.Update(testClusterResourceVar, PatchClusterRequest{})
		assert.Error(t, err)
	})
	t.Run("delete_cluster_error", func(t *testing.T) {
		svc := getTestClient(t)
		clusterSvc := NewClustersService(svc.Get(), ctx)
		_, _, err := clusterSvc.Delete(testClusterResourceVar)
		assert.Error(t, err)
	})
}
