...
```text
ionosctl compute applicationloadbalancer rule get [flags]
```
...
```text
ionosctl compute alb rule get --datacenter-id DATACENTER_ID --applicationloadbalancer-id APPLICATIONLOADBALANCER_ID -i FORWARDINGRULE_ID
```
