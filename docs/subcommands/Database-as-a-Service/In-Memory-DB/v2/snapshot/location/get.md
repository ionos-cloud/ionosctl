---
description: "Get an In-Memory DB Snapshot Location"
---

# DbaasInMemoryDbV2SnapshotLocationGet

## Usage

```text
ionosctl dbaas in-memory-db-v2 snapshot location get [flags]
```
...
Use this command to retrieve details about an In-Memory DB Snapshot Location by using its ID.
...
```text
  ...
  -i, --snapshot-location-id string   The ID of the In-Memory DB Snapshot Location (required)
...
```text
ionosctl dbaas in-memory-db-v2 snapshot location get --snapshot-location-id <snapshot-location-id>
```
