...
@test "Create Datacenter Label" {
    ...
    run ionosctl datacenter create --name "$datacenter_name" -w -o json
    assert_success
    ...
    run ionosctl label add --resource-type datacenter --datacenter-id "$datacenter_id" \
      --label-key "$label_key" --label-value "$label_value" -o json --wait
    assert_success
...
@test "Create Ipblock Label" {
    ...
    run ionosctl ipblock create --name "$ipblock_name" -w -o json
    assert_success
    ...
    run ionosctl label add --resource-type ipblock --ipblock-id "$ipblock_id" --label-key "$label_key" --label-value "$label_value" -o json -w
    assert_success
    assert_output -p "\"type\": \"label\""
    assert_output -p "\"key\": \"$label_key\""
    assert_output -p "\"value\": \"$label_value\""
