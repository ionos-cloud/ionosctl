...
EOF
    run ionosctl object-storage object tagging put --name "$TEST_BUCKET_NAME" --key "$TEST_KEY" --json-properties "$tmpfile"
    rm -f "$tmpfile"
    assert_success
...
@test "object-storage object tagging get: json output" {
    run ionosctl object-storage object tagging get --name "$TEST_BUCKET_NAME" --key "$TEST_KEY" -o json
    assert_success
