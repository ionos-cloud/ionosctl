...
@test "Create Datacenter" {
    run ionosctl compute datacenter create --name "natgw-test-$(randStr 8)" --location "es/vit" -w --timeout 60 -o json
    assert_success
...
@test "Reserve IPBlock for NAT Gateway" {
    run ionosctl compute ipblock create --location "es/vit" --size 1 --name "bats-natgw-$(randStr 8)" -w --timeout 60 -o json
    assert_success
...
@test "Create LAN for NAT Gateway" {
    run ionosctl compute lan create --datacenter-id "$(cat /tmp/bats_test/datacenter_id)" --name "bats-natgw-lan-$(randStr 8)" \
     --public=false -w --timeout 60 -o json
    assert_success
...
@test "Delete NAT Gateway rule" {
    ...
    run ionosctl compute natgateway rule delete --datacenter-id "$(cat /tmp/bats_test/datacenter_id)" \
     --natgateway-id "$(cat /tmp/bats_test/natgateway_id)" --rule-id "$(cat /tmp/bats_test/rule_id)" -f -w --timeout 300
    assert_success
}
...
@test "Delete NAT Gateway" {
    ...
    run ionosctl compute natgateway delete --datacenter-id "$(cat /tmp/bats_test/datacenter_id)" \
     --natgateway-id "$(cat /tmp/bats_test/natgateway_id)" -f -w --timeout 300
    assert_success
}
