...
@test "Create Datacenter" {
    run ionosctl compute datacenter create --name "natgw-test-$(randStr 8)" --location "es/vit" -w -o json
    assert_success
    echo "$output" | jq -r '.id' > /tmp/bats_test/datacenter_id
}

@test "Reserve IPBlock for NAT Gateway" {
    run ionosctl compute ipblock create --location "es/vit" --size 1 --name "bats-natgw-$(randStr 8)" -w -o json
    assert_success
...
@test "Delete NAT Gateway rule" {
    ...
    run ionosctl compute natgateway rule delete --datacenter-id "$(cat /tmp/bats_test/datacenter_id)" \
     --natgateway-id "$(cat /tmp/bats_test/natgateway_id)" --rule-id "$(cat /tmp/bats_test/rule_id)" -f -w
    assert_success
}
...
@test "Delete NAT Gateway" {
    ...
    run ionosctl compute natgateway delete --datacenter-id "$(cat /tmp/bats_test/datacenter_id)" \
     --natgateway-id "$(cat /tmp/bats_test/natgateway_id)" -f -w
    assert_success
}
