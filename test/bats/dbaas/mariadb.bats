...
@test "Find MariaDB Cluster" {
   ...
   # Wait until 'get' no longer returns a 404
    while true; do
        run ionosctl db mariadb cluster get --cluster-id "$cluster_id" -o json 2> /dev/null
        if [ "$status" -eq 0 ]; then
            break
        else
            echo "Cluster not found, retrying..."
            sleep 5
            counter=$((counter+1))
            if [ $counter -ge $max_retries ]; then
                echo "Failed to find the cluster after $max_retries attempts."
                return 1
            fi
        fi
    done

    # Use JQ to get cluster name
    cluster_name=$(echo "$output" | jq -r '.properties.name')

    echo "Got name $cluster_name"

    # TODO: Can't use get because it's flaky and fails with 404 until the cluster is fully created
...
@test "Assert DNS resolves to CIDR" {
    ...
    clusters_json=$(ionosctl db mariadb cluster list -M 1 -o json)
